	...
	*/
	...
	function tname_search_drug_master() {
		...
		$.getJSON(json_url, function(data) {
			...
			for (var i = 0; i < len; i++) {
				autoComplete.push(data[i]);
				// percentage = (i*100)/len;
				// $('#search_pharmacy').val('Loading Drugs... Please wait! '+percentage.toFixed(1));
				if(i == chkPoint){
					$('#search_pharmacy').val('');
	...
	function validateLabOrder(){
		...
		if ($('#advance_cb').is(":checked")){
			console.log('Advance checkbox checked');
			if($('#advance_tb').val() == ''){
...
function add_inventory_row(id,drug_val)
{
	...
	console.log("Drug after substr: "+drg+", Composition: "+comp);
	...
	if(investigation != ''){
		$.ajax({
			...
			success : function(investigationInfo) {
				...
				$("#"+id).append('<tr id="'+id+'_'+a[0]+'" data-toggle="tooltip" data-placement="top" title="'+a[1]+'"><td class="text-center">'+(count)+'.</td><td style="text-transform:UpperCase">'+investigation+'<input type="hidden" name="billing_line_items['+a[0]+'][item_information]" value="'+investigation+'"></td><td class="text-center">'+a[2]+'</td><td class="text-center">'+a[3]+'</td><td class="text-center">'+a[5]+'</td><td class="text-right"><span id="amt_span_'+a[0]+'" >'+a[4]+'</span><input type="hidden" name="billing_line_items['+a[0]+'][investigation_id]" value="'+a[0]+'" class="invgids" /><input type="hidden" name="billing_line_items['+a[0]+'][amount]" id="mrp_'+a[0]+'" value="'+a[4]+'" class="test" /></td><td class="text-center actions"><i class="fas fa-trash-alt deleteSmall" onclick="return removeInvestigation('+a[0]+','+a[6]+')"><i></td></tr>'); 
