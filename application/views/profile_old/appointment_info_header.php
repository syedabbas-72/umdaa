    ...
    <div class="row patientInfo">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="pid">
                            ...
                            &nbsp;&nbsp;&nbsp;
                            <b>Mob: </b><?=ucwords($appointmentInfo[0]->mobile)?>
                        </span>
                    </td>
                </tr>
            </table>
        </div>
        <div class="col-md-5 appointmentStatus text-center">
        ...
        <div class="col-md-2 text-right">
            ...
            if($appointment_id){
                ...
                {
                    ?>
                    <a href="<?php echo base_url('profile/closeAppointment/'.$appointment_id.'/'.$patient_id); ?>" class="btn btn-primary" style="margin-top: 8px">Check Out</a>
                    <?php
                }
            }
            ?>
...
if($appointment_id != '' || $appointment_id != NULL){
    if($appointmentInfo[0]->appointment_status == 'booked'){
        $appHeaderFlag = 1;
    }else{
        $appHeaderFlag = 0;
    }
...
if($appHeaderFlag == 1){
    ...
    for($cntr = 0; $cntr < count($appointmentInfo); $cntr++){
        ...
        if(in_array($appointmentInfo[$cntr]->appointment_status, $appointmentStatus)) {
            ...
            ?>
            <div class="row col-md-12 appointmentInfoDiv" id="appointment_info_<?php echo $cntr; ?>">        
                <div class='<?=$colClass;?>'>
                    <div class="panel-body appointmentInfoPanelBody">
                        ...
                        if($appointmentInfo[$cntr]->appointment_payment_status == 0 || $appointmentInfo[$cntr]->registration_payment_status == 0) {
                            if (!isset($paymentPage)) {
                                ?>
                                <a class="btn btn-primary" href="<?php echo base_url('patients/confirm_payment/'.$patient_id.'/'.$appointmentInfo[$cntr]->appointment_id); ?>" id="<?php echo $appointment_id; ?>">Collect Payment</a>
                                <?php
                        ...
                        if($appointmentInfo[$cntr]->appointment_status == 'booked') {
                            ?>
                            <a class="btn btn-danger" id="<?php echo $app_info->appointment_id; ?>" onclick="return dropAppointment('<?php echo $cntr; ?>','<?php echo $appointmentInfo[$cntr]->appointment_id; ?>','<?php echo $doctor; ?>')">Drop</a>
                            <a class="btn btn-warning" id="<?php echo $appointmentInfo[$cntr]->appointment_id; ?>" onclick="return rescheduleModal('<?=$appointmentInfo[$cntr]->appointment_id; ?>')">Reschedule</a>
                            <?php // close
                        }

                        $possibleCheckOutStatus = array('checked_in','vital_signs','waiting','in_consultation');
                        if(in_array($appointmentInfo[$cntr]->appointment_status, $possibleCheckOutStatus))
                        {
                            ?>
                            <a href="<?php echo base_url('profile/closeAppointment/'.$appointmentInfo[$cntr]->appointment_id.'/'.$appointmentInfo[$cntr]->patient_id); ?>" class="btn btn-primary" style="margin-top: 8px">Check Out</a>
                            <?php
                        }
                        ?>
                    </div>
                ...
                <?php if($prioritySection) { ?>
                    ...
                    <div class="col-md-3">
                        <div class="panel-body appointmentInfoPanelBody">
                            <div class="row appointmentInfo">
                                <div class="col-md-12 text-left">
                                    <?php $priority = array('disability','pregnancy','elderly','children','sick','none'); ?>
                                    <label style="margin-bottom: 5px">Priority</label>
