...
class Lab extends MY_Controller {
	...
	public function view_prescription($pid){
		...
		$data['table']=$this->db->query("SELECT * FROM patient_investigation,patient_investigation_line_items WHERE patient_investigation.patient_investigation_id=patient_investigation_line_items.patient_investigation_id AND patient_investigation.umr_no='".$finalumr."'")->result();
		...
		$price=$this->db->query("SELECT clinic_investigation_id FROM `clinic_investigations` WHERE clinic_id='.$clinic_id.' AND investigation_id='.$pri->investigation_id.'")->result();
		$data['p']=$this->db->query("SELECT clinic_investigation_id FROM `clinic_investigations` WHERE clinic_id='.$clinic_id.' AND investigation_id='.$pri->investigation_id.'")->result();
		$data['priceinv']=$this->db->query("SELECT * FROM `clinic_investigation_price` WHERE clinic_investigation_id='".$price->clinic_investigation_id."'")->result();
		$this->db->where('CI.clinic_id =',$clinic_id);
	...
	*/
	public function department_investigations(){
		...
		foreach($departments as $key => $value){
			...
			$investigations = $this->db->select('investigation_id, investigation')->from('investigations')->where('lab_department_id =',$key)->get()->result_array();
	...
	public function add_investigations(){
		...
		foreach($investigations as $key => $value){
			...
			*/
			...
			$investigationInfo = $this->db->select('investigation_id, lab_department_id, package, other_information')->from('investigations')->where('investigation_id =',$key)->get()->result_array();
	...
	*/
	public function createInvestigaton($investigation)
	{
		...
		$clinic_investigation_id = $this->Generic_model->getFieldValue('clinic_investigations','clinic_investigation_id',array('clinic_id'=>$investigation['clinic_id'],'investigation_id'=>$investigation['investigation_id']));
		...
		if($package == 0){
			...
			$rangeInfo = $this->db->select('investigation_id, sample_type, condition, method, low_range, high_range, units, remarks, result_condition')->from('investigation_range')->where('investigation_id =',$investigation['investigation_id'])->get()->result_array();

			// Get distinct sample types for pricing
			$sampleTypes = $this->db->select('sample_type')->distinct()->from('investigation_range')->where('investigation_id =',$investigation['investigation_id'])->get()->result_array();
	...
	public function getPackageInfo($investigation_id){
		...
		$packageInvestigations = $this->db->select('investigation_id')
		->from('lab_template_line_items')
		->where('lab_template_id =',$lab_template_id)->get()->result_array();
		...
		foreach($packageInvestigations as $invRecord){
			...
			$investigation = $this->db->select('investigation_id, lab_department_id, package, other_information')->from('investigations')->where('investigation_id =',$invRecord['investigation_id'])->get()->result_array();
	...
	public function delete_lab_investigation($id=''){
		...
		$this->db->query("delete from clinic_investigation_price where clinic_investigation_id='".$id."'");
		$this->db->query("delete from clinic_investigation_range where clinic_investigation_id='".$id."'");
	...
	public function investigations()
	{
		...
		if($_POST['submit']){
			...
			if($_POST['price'] > 0){
				$res = $this->Generic_model->updateData('clinic_investigation_price',array('price' => $_POST['price']),array('clinic_investigation_price_id' => $_POST['clinic_investigation_price_id']));	
			}
		...
		$investigations = $this->db->select('CI.clinic_investigation_id, CI.investigation_id,
		 I.investigation, I.investigation, I.item_code, I.package, I.template_type')
		...
		foreach($investigations as $inv){
			...
			->from('clinic_investigation_price')->where('investigation_id =',$inv['investigation_id'])
			...
			if(count($samples) > 0){
				...
				foreach($samples as $sample){
					if($sample['price'] == 0){
						...
						->from('clinic_investigation_range')->where('investigation_id =',$inv['investigation_id'])
	...
	*/
	public function view_package_info(){
		...
		$lineItemsInfo = $this->db->select('LI.lab_template_line_item_id, LI.lab_template_id, LI.parent_investigation_id, LI.investigation_id, LI.parameter, CI.clinic_investigation_id, CI.clinic_id, CI.price, CI.other_information, CIR.clinic_investigation_range_id, CIR.method, CIR.condition, CIR.low_range, CIR.high_range, CIR.units, CIR.remarks')->from('lab_template_line_items LI')->join('clinic_investigations CI','LI.investigation_id = CI.investigation_id','left')->join('clinic_investigation_range CIR','CI.clinic_investigation_id = CIR.clinic_investigation_id','inner')->where('CI.clinic_id =',$clinic_id)->where('LI.lab_template_id =',$templateInfo[0]['lab_template_id'])->get()->result_array();
	...
	public function get_template_info()
	{
		...
		*/
		...
		$lab_template_info = $this->db->select('lab_template_id, lab_department_id, template_name, template_type')->from('lab_templates')->where('investigation_id =',$investigation_id)->get()->row();
		...
		if($clinic_lab_template_id == 0){
			...
			$labTemplateLineItems = $this->db->distinct()->select('LI.lab_template_line_item_id, LI.parent_investigation_id, LI.investigation_id, LI.parameter, LI.position, CIR.remarks')->from('lab_template_line_items LI')->join('clinic_investigation_range CIR','LI.investigation_id = CIR.investigation_id','inner')->where('LI.lab_template_id =',$lab_template_info->lab_template_id)->order_by('LI.position','ASC')->get()->result_array();
		}else{
			// Get lab template line items concern to lab_template_id
			$labTemplateLineItems = $this->db->distinct()->select('CLI.clinic_lab_template_line_item_id as lab_template_line_item_id, CLI.parent_investigation_id, CLI.investigation_id, CLI.parameter, CLI.position, CIR.remarks')->from('clinic_lab_template_line_items CLI')->join('clinic_investigation_range CIR','CLI.investigation_id = CIR.investigation_id','inner')->where('CLI.clinic_lab_template_id =',$clinic_lab_template_id)->order_by('CLI.position','ASC')->get()->result_array();
		}
		...
		if($lab_template_info->template_type == 'Excel'){
			...
			$tinfo .= '<tr><th style="width:65%">Parameter</th><th class="text-center" style="width:20%">Value</th><th class="text-center" style="width:15%">Position</th></tr>';
			...
			$tinfo = '';
			$tinfo .= '<tr><th colspan="3">'.$lab_template_info->template_name.'<input type="hidden" id="position_update_tb" name="position_update" value=0><input type="hidden" id="clinic_lab_template_id_tb" name="clinic_lab_template_id" value="'.$clinic_lab_template_id.'"><span class="customBtn pull-right mt-0" name="template_excel" onclick="newParamGeneralModal(\''.$clinic_investigation_id.'\',\''.$billing_id.'\',\''.$investigation_id.'\',\''.$investigation_status_id.'\',\''.$report_entry_status.'\',\''.$investigation.'\');">Add New Parameter</span></th></tr>';
			$tinfo .= '<tr><th style="width:20%">Parameter</th><th class="text-center" style="width:65%">Value</th><th class="text-center" style="width:15%">Position</th></tr>';
			$i = 0;
			foreach($labTemplateLineItems as $tresult)
			{
	...
	*/
	public function createParameter(){
		...
		*/
		...
		$investigationInfo = $this->db->select('I.investigation_id, I.lab_department_id, I.investigation as template_name, I.template_type, LT.lab_template_id')->from('investigations I')->join('lab_templates LT','I.investigation_id = LT.investigation_id','inner')->where('I.investigation_id =', $parent_investigation_id)->get()->result_array();
		...
		if($investigation_id == 0) {
			...
			// Merge Arrays
			$invRangeData = array_merge($invRangeData,$extraData);

			print_r($invRangeData);

			// Create Investigation Range Record
			$investigation_range_id = $this->Generic_model->insertDataReturnId('investigation_range',$invRangeData);	
		}
		...
		// Get lab template line items w.r.to lab_template_id
		$labTemplateLineItems = $this->db->select('parent_investigation_id, investigation_id, parameter, position')->from('lab_template_line_items')->where('lab_template_id =',$investigationInfo[0]['lab_template_id'])->get()->result_array();
	...
	public function add_order($prescription_id = NULL)
	{
		...
		if($prescription_id != NULL){
			...
			$this->db->select('PIL.patient_investigation_line_item_id, PIL.investigation_name,
			 INV.item_code, INV.short_form, INV.investigation, INV.investigation_id, INV.package,
			 CI.clinic_investigation_id, CI.clinic_id, CIP.price, P.patient_id, P.first_name,
			 P.last_name, P.umr_no, P.mobile, P.alternate_mobile, PI.patient_investigation_id,
			 PI.appointment_id, A.appointment_date, A.appointment_time_slot, Doc.doctor_id,
		     Doc.first_name as doc_first_name, Doc.last_name as doc_last_name, Dep.department_id, 
			 Dep.department_name');
			$this->db->from('patient_investigation_line_items PIL');
			$this->db->join('investigations INV', 'PIL.investigation_id = INV.investigation_id','left');
			$this->db->join('clinic_investigations CI', 'INV.investigation_id = CI.investigation_id','left');
			...
			$data['investigation_cart'] =$test= $this->db->get()->result();
// 			echo '<pre>';
// print_r($test);
// echo '<pre>';
		}
	...
	public function save_order()
	{
		...
		foreach($billing_line_items as $lineItemInfo){
			...
			$billing_line_item_id = $this->Generic_model->insertDataReturnId('billing_line_items',$lineItemInfo);    	
