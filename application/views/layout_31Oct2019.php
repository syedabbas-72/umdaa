...
<script type="text/javascript">	
	...
	*/
	...
	{
		console.log('I am at finding investigation record');
		var base_url = '<?php echo base_url(); ?>';
		$.ajax({
			...
			success : function(rdata) { 
				...
				if($.trim(rdata) == "NA"){
					...
					if($('#'+id+'_'+a[1]+'_tr').length > 0){
						...
						tableRow += '<td><input type="hidden" class="disc form-control" name="disc[]" id="disc_'+a[1]+'" value="" onkeyup="return checkmax('+a[0]+',id,'+a[1]+')"></td>';
...
{
	console.log("Trying to add a package");
	console.log("ID: "+id);
	...
	if(package_name != ''){
		$.ajax({
			...
			success : function(packageInfo) {
				...
				$("#"+id).append('<tr id="pkg_'+a[0]+'" data-toggle="tooltip" data-placement="top" title="'+a[1]+'"><td class="text-center">'+counter+'.</td><td><span style="text-transform:UpperCase">'+a[1]+'</span><input type="hidden" name="billing_line_items['+counter+'][item_information]"></td><td class="text-center">'+a[2]+'</td><td class="text-center">Package</td><td class="text-center">- -</td><td class="text-right"><span>'+a[3]+'</span><input type="hidden" name="billing_line_items['+counter+'][clinic_investigation_id]" value="'+a[0]+'"><input type="hidden" name="billing_line_items['+counter+'][amount]" id="mrp_'+counter+'" value="'+a[3]+'"></td><td class="text-center actions"><i class="fas fa-trash-alt deleteSmall" onclick="return removeInvestigation(pkg_'+a[0]+','+a[4]+')"><i></td></tr>');  
...
{
	console.log("Trying to add an investigation");
	console.log("ID: "+id);
	...
	if(investigation != ''){
		$.ajax({
			...
			success : function(investigationInfo) {
				...
				$("#"+id).append('<tr id="inv_'+a[0]+'" data-toggle="tooltip" data-placement="top" title="'+a[1]+'"><td class="text-center">'+(count)+'.</td><td style="text-transform:UpperCase"><span>'+investigation+'</span><input type="hidden" name="billing_line_items['+counter+'][item_information]" value="'+investigation+'"></td><td class="text-center">'+a[2]+'</td><td class="text-center">'+a[3]+'</td><td class="text-center">'+a[5]+'</td><td class="text-right"><span id="amt_span_'+counter+'">'+a[4]+'</span><input type="hidden" name="billing_line_items['+counter+'][investigation_id]" value="'+a[7]+'" class="invgids" /><input type="hidden" name="billing_line_items['+counter+'][clinic_investigation_id]" value="'+a[0]+'" class="invgids" /><input type="hidden" name="billing_line_items['+counter+'][amount]" id="mrp_'+counter+'" value="'+a[4]+'" class="test" /></td><td class="text-center actions"><i class="fas fa-trash-alt deleteSmall" onclick="return removeInvestigation(inv_'+a[0]+','+a[6]+')"><i></td></tr>'); 
	...
	if(invg!=''){
		$.ajax({
			...
			success : function(drgid) {
				...
				// console.log("Investigation: ".investigationJson);
				// return false;
				// console.log("Template Type: "+investigationJson['template_type']);
				var investigationCount = investigationJson.length;
				// console.log("Count: "+investigationCount);
				var html = '';
				...
				for(i=0; i<investigationCount; i++){
					...
					if($("#"+investigationJson[i]['investigation_id']+"_div").length){
						...
						// console.log("Investigation - '"+investigationJson[i]['investigation']+"' already added");
						$("#exist_div").show();
						if(x > 0){
							existContent += ", ";
						}
						...
						// Price
						html += '<div class="col-md-2 headCell"><div class="form-group"><label for="mrp_'+investigationJson[i]['investigation_id']+'_tb" class="col-form-label">MRP <span class="imp">*</span></label><input class="form-control" type="text" name="clinic_investigation['+investigationJson[i]['investigation_id']+'][price]" id="mrp_'+investigationJson[i]['investigation_id']+'_tb" onkeypress="return decimal();" placeholder="MRP" required></div></div>';
						...
						// Method TextBox
						html += '<div class="col-md-2"><div class="form-group">';
						html += '<label for="method_'+investigationJson[i]['investigation_id']+'_tb" class="col-form-label">Method</label><input class="form-control" type="text" id="units_'+investigationJson[i]['investigation_id']+'_tb" name="clinic_investigation['+investigationJson[i]['investigation_id']+'][method]" onkeypress="return alpha();" placeholder="Method">';
						html += '</div></div>';
