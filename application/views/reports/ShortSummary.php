...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-md-4">
			<!-- <div class="row"> -->
		<b style="margin-left: 14px";>Name:</b><?php echo $clinicInfo->clinic_name?>
		<div>
			<p><b>Location:</b><?php 
...
<!-- </div> -->
	...
	<div class="row">
		...
		<div class="col-md-12" style="margin-left:0px;margin-left:20px;margin-top:16px;">
...
<?php if(count($vital_sign)>0){ ?>
		...
		<table cellspacing="0" cellpadding="0" style="margin-left:35px;font-size:15px;width: 100%;font-size: 12px;min-height:50px !important;padding-left: 0px !important,">
			...
			else
			{
				...
				// for($v=0;$v<count($res_vital_sign);$v++)
				// {
					//  echo $res_vital_sign[$v]->vital_sign;
					// if($res_vital_sign[$i]->vital_sign == 'DBP' && $res_vital_sign[$i]->vital_sign == 'SBP')
					if($res_vital_sign[1]->vital_sign == 'DBP')
					{?>
						<tr style="padding: 10px">
						<?php 
							$SBP = 0;
				...
				<?php
					...
					elseif($res_vital_sign[0]->vital_sign == 'DBP')
					{?>
		<tr style="padding: 10px">
						<?php 
							$SBP = 0;
							...
							{ 
								...
								// 		$vital_sign_results[0]['unit'] = $value->unit;
								// }
								if($value->vital_sign == "DBP" || $value->vital_sign == "SBP"){	
									if($value->vital_sign == "DBP"){
										$DBP = $value->vital_result;
									}elseif($value->vital_sign == "SBP"){
										$SBP = $value->vital_result;
									...
									if($SBP != 0 && $DBP != 0){
										$BP = $SBP."/".$DBP;
										$vital_sign_results[0]['vital_sign'] = 'BP';
										$vital_sign_results[0]['value'] = $BP;
										$vital_sign_results[0]['unit'] = $value->unit;
									}						
								}else{
									$vital_sign_results[$i]['vital_sign'] = $value->vital_sign;
...
<!-- <button type="button" onClick="window.print()" title="Print" style="font-size:25px;">Print <i class="fa fa-print fa-lg"></i></button> -->