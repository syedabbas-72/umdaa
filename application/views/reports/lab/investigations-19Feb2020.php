				...
				<div class="tabs">
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="home">							
							<div class="row">
								<div class="col-md-12">
									<?php if(count($newInvestigations) > 0) { ?>
										...
										<table id="investigationsList_tbl" class="table customTable">
											<thead>
												<tr>
													...
													<th style="width: 30%">Investigation Name (Item Code) (Method)</th>
													<th style="width: 12%" class="text-center">Short Form</th>
													<th style="width: 13%" class="text-center">Template type</th>
													<th style="width: 20%" class="text-center">Ranges (Low - High) (Units)</th>										
											...
											<tbody>
												...
												foreach ($newInvestigations as $value) {
													...
													<tr class="redRows">
														...
														<td>
															<span><?php echo strtoupper($value['investigation']).$short_form; ?></span><span class="code"><?=$value['item_code']?></span><?php if($value['package'] == 1){ ?><span class="package">GOI</span><?php } ?><br>
															<small><i><?php echo ucwords($value['method']); ?></i></small>
														</td>
														<td class="text-center"><?=$value['short_form']?></td>
														<td class="text-center"><?=$value['template_type']?></td>
														<td class="text-center">
															<?php
															if($value['low_range'] == '' && $value['high_range'] == ''){
																echo '-';
															}else{
																if($value['low_range'] == 0 && $value['high_range'] != 0){
																	echo "<span> < ".$value['high_range'].'</span>';	
																}elseif($value['low_range'] != 0 && ($value['high_range'] == 0 || $value['high_range'] == '')){
																	echo "<span> > ".$value['low_range'].'</span>';
																}else{
																	echo "<span>".$value['low_range'].'</span> - <span>'.$value['high_range'].'</span>';
																}																
															}
														...
														<td class="text-right">
															<i class="fas fa-rupee-sign currencySmall"></i><span class="priceSmall"><?php echo number_format($value['price'],2); ?></span></td>
														<td class="text-center actions">
															<!-- Read -->
															<?php if($value['package'] == 1){ ?>
																<?php if($crudInfo->p_read){ ?>
																	<i class="fas fa-search viewSmall" onclick="return openPopup('viewPackageModal','package','<?=$value["investigation_id"]?>','<?=$value["clinic_investigation_id"]?>','<?=$value["price"]?>','<?=$value['template_type']?>');" title="View Packaged Investigations &amp; Edit"></i>
																<?php } ?>
															<?php } ?>
															...
															<?php if($value['package'] == 0){ ?>
																<?php if($crudInfo->p_update){ ?>
																	<i class="fas fa-pencil-alt editSmall" title="Edit Investigation" onclick="return openPopup('viewInvestigationModal','investigation','<?=$value["investigation_id"]?>','<?=$value["clinic_investigation_id"]?>','<?=$value["price"]?>','<?=$value['template_type']?>');"></i>
																<?php } ?>
															<?php } ?>	
												...
												foreach ($investigations as $value) {
													...
													<tr>
														<td class="text-center"><?php echo $i;?></td>
														<td>
															<span><?php echo strtoupper($value['investigation']).$short_form; ?></span><span class="code"><?=$value['item_code']?></span><?php if($value['package'] == 1){ ?><span class="package">PACKAGE</span><?php } ?>
															<br>
															<small><i><?php echo ucwords($value['method']); ?></i></small>
														</td>
														<td class="text-center"><?=$value['short_form']?></td>
		...
		<div class="modal-content">	
			<form class="form" action="<?php echo base_url('Lab/investigations');?>" method="post" role="form">
				...
				<div class="modal-footer text-center">	   	
					<input type="submit" name="submit" value="Submit" class="btn customBtn" onclick="" />
				</div>
			</form>
		</div>
		<!-- Modal content-->
...
<script type="text/javascript">
	...
	function openPopup(modal, type, investigation_id, clinic_investigation_id, price, template_type){
		...
		$.ajax({
            ...
            success : function(result) {
            	...
            	html += '<th colspan="'+colspan+'" class="title">'+investigationsInfo['package'].template_name+'<input type="hidden" name="clinic_investigation_id" value="'+clinic_investigation_id+'"><input type="hidden" name="template_type" value="'+template_type+'"></th>';
            	if(template_type == 'General'){
            		colspan = 1;
            	}else{
            		colspan = 2
            	}
            	html += '<th colspan="'+colspan+'" class="text-right"><div class="pull-right"><div style="display:table-cell"><span>Price: </span></div><div style="display:table-cell; padding-left:10px;"><input type="text" name="price" value="'+price+'" class="form-control" onkeypress="return decimal();" placeholder="M.R.P." style="width:100px"></div></div></th>';
            	html += '</tr>';
            	html += '<tr>';
            	...
            	for(var i=0; i<itemCount; i++){
            		...
            		html += '<tr>';
            		html += '<td>'+sno+'</td>';
            		html += '<td>'+investigationsInfo['packageItems'][i].parameter+'<input type="hidden" name="range['+i+'][clinic_investigation_range_id]" value="'+investigationsInfo['packageItems'][i].clinic_investigation_range_id+'"></td>';
            		...
            		if(template_type == 'Excel'){
	            		...
	            		if(investigationsInfo['packageItems'][i].low_range == null){ investigationsInfo['packageItems'][i].low_range = '';}
	            		...
	            		if(investigationsInfo['packageItems'][i].high_range == null){ investigationsInfo['packageItems'][i].high_range = '';}
	            		...
	            		if(investigationsInfo['packageItems'][i].other_information == null) { investigationsInfo['packageItems'][i].other_information = '';}
	            		html += '<td><textarea name="clinic_investigation['+i+'][other_information]" rows="1" cols="25" class="form-control">'+investigationsInfo['packageItems'][i].other_information+'</textarea></td>';
	            	}else if(template_type == 'General'){
