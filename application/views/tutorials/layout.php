		...
		$.ajax({
			...
			success : function(packageInfo) {
				...
				if(a.length > 6)
				{
					...
					{
						console.log("test");
						console.log("Master Investigations Ids: "+a[$i]);
					}
				}
	...
	if(investigation != ''){
		$.ajax({
			...
			success : function(investigationInfo) {
				...
				chkCounttt = $("#pkg_"+3).length;
				console.log("manuall:"+chkCounttt);

				// $result = mapping(a[0],'Investigation');
				// console.log("success2"+$result);

				// var feedd = {name: "normal", id: a[0]};
				// arrayTestt=[];
				// arrayTestt.push(feedd);

				// console.log(arrayTestt);
				...
				// $i=1;
				$("#"+id).append('<tr id="inv_'+a[0]+'" data-toggle="tooltip" data-placement="top"  class="invDataTr"><td class="text-center" style="text-transform:UpperCase"><span>'+counter+'</span></td><td class="text-center"><span style="text-transform:UpperCase">'+a[1]+'</span></td><td class="text-center" style="text-transform:UpperCase"><span class="code">'+a[2]+'</span><input type="hidden" name="billing_line_items['+counter+'][item_information]" value="'+investigation+'"></td><td class="text-center"><i class="fas fa-rupee-sign"></i> &nbsp;<span id="amt_span_'+a[0]+'">'+a[3]+'</span><input type="hidden" name="billing_line_items['+counter+'][investigation_id]" value="'+a[6]+'" class="invgids" /><input type="hidden" name="billing_line_items['+counter+'][clinic_investigation_id]" value="'+a[0]+'" class="invgids" /> <input type="hidden" name="billing_line_items['+counter+'][amount]" id="mrp_tb_'+a[0]+'" value="'+a[3]+'" class="test" /></td><td class="text-center actions"><i class="fas fa-trash-alt deleteSmall" onclick="return removeInvestigation(\''+a[0]+'\',\'inv_'+a[0]+'\',\''+a[5]+'\',\''+$("#referral_doctor_lab_discount_id").val()+'\')"><i></td></tr>'); 
