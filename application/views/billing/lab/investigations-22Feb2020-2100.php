														...
														foreach($conditions as $condition){
															if($j < $count){
																...
																<tr class="redRows">
																	...
																	<td <?php if($j == (int)$count - 1) { echo 'class="conditionRecEnd"'; }else{ echo 'class="conditionRec"'; } ?>>&nbsp;</td>
	...
	function openPopup(modal, type, investigation_id, clinic_investigation_id, price, clinic_investigation_price_id, template_type){
		...
		$.ajax({
            ...
            success : function(result) {
            	...
            	var template = investigationsInfo['package']['template'];
            	var methodsCount = investigationsInfo['methods'].length; // Populate Methods
            	var unitsCount = investigationsInfo['units'].length; // populate Units      

            	var method = 0;

            	if(type == 'investigation'){
            		method = template['lineItems'][0].method;
            	}

            	console.log("Method: "+method);
            	
            	
            	var itemCount = template['lineItems'].length;  
            	var template_type = template.template_type;

            	console.log(itemCount+", "+template_type);
            	
            	// if(template['package'] == 0){
            	// 	method = template['lineItems'][0].method;	
            	// }          	

            	// console.log(method);
            	// console.log("Template:"+investigationsInfo);
            	...
            	if(template_type == 'Excel'){
            		html += '<th colspan="3" class="title">'+template.template_name+'<input type="hidden" name="clinic_investigation_id" value="'+clinic_investigation_id+'"><input type="hidden" name="clinic_investigation_price_id" value="'+clinic_investigation_price_id+'"><input type="hidden" name="template_type" value="'+template_type+'"></th>';
