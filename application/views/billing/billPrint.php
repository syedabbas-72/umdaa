...
<table cellpadding="0" cellspacing="0" style="margin: 0px;font-size: 12px; padding: 0px; width: 100%;">
    ...
    foreach ($billing_line_items as $bill) {
        ...
        $paid = 0;
        if ($bill->discount==0) {
            $ndiscAmount = $bill->discount;
        } elseif ($bill->discount!=0) {
            if ($bill->discount_unit == "INR") {
                $ndiscAmount = $bill->discount;
            } elseif ($bill->discount_unit == "%") {
                $ndiscAmount = (($bill->amount*$bill->discount)/100);
            }
        }
        $npaid = $bill->amount-$ndiscAmount;
        $ntotal += $bill->amount;
        $nptotal += $npaid;
    }
	// foreach($billing as $bill)
	// {
	// 	$discAmount = 0;https://umdaa.co/clinic/uploads/billings/121041011618313451.pdf
    // 	$paid = 0;
    // 	if($bill->item_information == "Pharmacy" || $bill->item_information == "Lab")
    // 		continue;
    // 	if($bill->status==2)
	...
	?>
    <!-- <h4>NTotal: <?=$ntotal?></h4>
    <h4>NPTotal: <?=$nptotal?></h4>
    <h4>total_paid_amount: <?=$total_paid_amount?></h4> -->
	<table cellspacing="0" cellpadding="0" style="width: 100%; padding: 5px 10px">
		<tr>
			<td style="width: 60%; font-size: 12px ">
				...
				<span style="line-height: 20px; font-size: 12px">Received with thanks, amount of <span style="font-weight: bold"><?php echo number_format($nptotal,2); ?></span> INR towards the following:</span>
			</td>
			<td style="width: 40%;">
	...
	<table class="app-invoice" cellspacing="0" cellpadding="0" style="width: 100%;">
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				Amount Paid : <span style="color: #000; font-weight: 600;"><?php echo number_format($ptotal,2); ?> </span> 
			</td>
		</tr>
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				Amount Due : <span style="color: #000; font-weight: 600;"><?php echo number_format($billing_master->osa,2); ?> </span> 
			</td>
		</tr>
		<tr>
