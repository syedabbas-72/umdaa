...
<table cellpadding="0" cellspacing="0" style="margin: 0px;font-size: 12px; padding: 0px; width: 100%;">
	<tr>
		<td style="width:30%;font-size: 12px">
			...
			?>
			<span style="font-weight: bold;">Patient Name:</span><?php echo strtoupper($billingInfo->guest_name); ?><br>
			<span style="font-weight: bold;">Patient Age:</span>&nbsp;<?php echo $billingInvoice->age != '0'?$billingInvoice->age.' Y':'-'; ?><?php echo ','.$billingInvoice->gender ; ?><br>
		...
		<td style="width:30%;text-align: right;vertical-align: top;font-size: 12px">
			...
			<span style="font-weight: bold;font-size: 12px"><b><?php echo strtoupper($doctor_name); ?></b></span><br><span><?php echo strtoupper($doctorInfo->qualification. ", ". $department_name); ?> </span><br>
	...
	<table cellspacing="0" cellpadding="0" style="width: 100%; padding: 5px 10px">
		<tr>
			<td style="width: 60%; font-size: 12px ">
				...
				<span style="line-height: 20px; font-size: 12px">Received with thanks, amount of <span style="font-weight: bold">Rs. <?=number_format($billingInvoiceInfo->invoice_amount,2)?> /-</span> INR towards the following:</span>
			</td>
			<td style="width: 40%;">
				<table cellspacing="0" cellpadding="0" style="width: 100%">
					...
					<!-- <td style="text-align: right; padding-top:10px;font-size: 12px">Date : <span style="font-weight: 600; padding-top:10px;font-size: 12px"><?php echo $billing_date; ?></span></td> -->
						<td style="text-align: right; padding-top:10px;font-size: 12px">Date : <span style="font-weight: 600; padding-top:10px;font-size: 12px"><?php echo date('d/m/Y',strtotime($billingInfo->billing_date_time)); ?></span></td>
					</tr>
					<tr>
						<td style="text-align: right;font-size: 12px">Invoice Number : <span style="font-weight: 600;font-size: 12px"><?php echo $billingInfo->invoice_no_alias; ?></span></td>
					</tr>
				</table>
			</td>
		</tr>
	...
	<table class="app-invoice" cellspacing="0" cellpadding="0" style="width: 100%;">
					...
					foreach($billingLineItemsInfo as $bills) {
						...
						$ptotal += $paid;
							...
							<tr>
								...
								<td style="font-size: 12px; width: 75%; padding:10px;text-align: left;"><label class="font-weight-bold"><?php echo $bills->item_information; ?></label></td>
								<?php  
								...
								?>
								<td style="font-size: 12px; text-align: right; width: 20%; padding:10px; border-bottom: 1px solid #ccc; border-right:1px solid #ccc;text-align: right;" class="text-right"><span><?php echo number_format($data->price,2); ?></span></td>
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				Total Amount : <span style="font-weight: 600;"><?php echo number_format($billingInfo->total_amount,2); ?></span> 
			</td>
		</tr>

		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				Total Savings : <?php echo number_format($billingInfo->total_amount - $billingInfo->billing_amount,2); ?>
			</td>
		</tr>
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				Balance Amount : <?php echo number_format($billingInfo->billing_amount- $billingInvoice_records[0]->invoice_amount,2); ?>
			</td>
		</tr>
			<?php } ?>
		<?php } ?>
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
				 Amount Paid : <?php echo number_format($billingInvoice_records[0]->invoice_amount,2); ?> on <?php echo date('d/m/Y',strtotime($billingInvoice_records[0]->invoice_date));?>
			</td>
		</tr>
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
		...
		<tr>
			<td colspan="5" style="text-align: right; padding: 10px;font-size: 12px">
			Balance Amount Paid : <?php echo number_format($billingInvoice_records[1]->invoice_amount,2); ?>  on <?php echo date('d/m/Y',strtotime($billingInvoice_records[1]->invoice_date));?>
			</td>
		</tr>
		</tr>
		...
		<tr>
			<td colspan="5" style="padding: 10px; text-align: left; padding: 10px;font-size: 12px">
				Mode of Payment : <span style="font-weight: 600;"><?php echo ucfirst($billingInvoice->payment_mode); ?></span>
			</td>
		</tr>
		<tr>
