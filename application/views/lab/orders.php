						...
						<div class="row">
							<div class="col-sm-12">
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											<label for="pname" class="col-form-label">Select Doctor <span class="imp">*</span></label>
											<select class="form-control" id="sel1" name='doctor_id' value="<?php if($cartCount > 0) { echo ucwords($investigation_cart[0]->doc_first_name); } ?>">
											   <option value="">Select Doctor</option>
												<?php foreach($clinic_doctors as $row):?>
												<?php $doc_name = $this->db->select('*')->from('doctors')->where('doctor_id =',$row->doctor_id)->get()->row();?>
												 <option value="<?php echo $row->doctor_id;?>" <?=($investigation_cart[0]->doc_first_name == $doc_name->first_name)?'selected':''?>><?php echo "Dr.".$doc_name->first_name.' '.$doc_name->last_name;?></option>
												<?php endforeach;?>
											</select>
											<!-- <input class="form-control" type="text" autocomplete="off" onkeypress="return alpha();" name="billing[guest_name]" style="text-transform: capitalize;" placeholder="Gender" required value="<?php if($cartCount > 0) { echo ucwords($investigation_cart[0]->first_name.' '.$lat_name); } ?>"> -->
										</div>
											...
											<td style="width:35%;" class="text-left form-group">
												<select name="referral_doctor_id" id="referral_doctor_id_sb" class="form-control" onchange="return adjustDiscount(0,'<?=$lab_discount?>','<?=$referral_doctor_lab_discount?>');">
													...
													<?php foreach($referral_doctors as $doctorRec){
														...
														?>
														<option value="<?=$doctorRec['rfd_id']?>"><?=ucwords($doctorRec['doctor_name'])?></option>
														<?php
													}
													?>
												</select>
