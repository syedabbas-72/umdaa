	...
	$.ajax({
		...
		success : function(rdata) { 
			...
			if($.trim(rdata) == "NA"){
				...
				// alert("Scheduled Drug Info: "+a[4]+", Stock Available: "+a[5]);
				// return false;
				//alert(a[5]);
				...
				if($('#'+id+'_'+a[1]+'_tr').length > 0){
					...
					tableRow += '<td><span class="mrp"> '+trade_name+' '+a[4]+'</span><span class="formulation">'+a[2]+'</span></td>';

					if(a[5] == 0){
						tableRow += '<td><input type="text" name="qty[]" id="qty_'+a[1]+'" class="form-control batchTxt noStock" readonly placeholder="No Stock available"></td>';
					}else{
						tableRow += '<td><a onclick="get_batch_details('+a[1]+');" data-toggle="modal" placeholder="quantity" data-target="#myModal"><input type="text" name="qty[]" id="qty_'+a[1]+'" class="form-control batchTxt stock" readonly placeholder="Click and Place Required Quantity"> </a></td>';
					}					
					
					tableRow += '<td><span id="actual_amt_span_'+a[1]+'" class="mrp"></span></td>';
					tableRow += '<td><input type="hidden" class="disc text-center" name="disc[]" id="disc_'+a[1]+'" value="'+a[0]+'" style="width:80px" onkeyup="return checkmax('+a[0]+',id,'+a[1]+')"></td>';
					...
					tableRow += '<td><i class="fa fa-trash delete remove_drug_p" id="'+id+'_'+a[1]+'_tr"><i></td>';
					tableRow += '</tr>';
					$("#"+id).append(tableRow);
...
function enable_discount(){
	...
	$(".totrw").each(function () {
		...
		if(!isNaN(index)){
			if ( $('#apdis').is(":checked")){
				$('.disc').prop('type','number');
				var max = $("#disc_tb_"+index).val();
				var id = $("#disc_"+index).val();
				if($('#disc_'+index).val() == 0){
					$('#disc_'+index).val($("#disc_tb_"+index).val());	
				}
				
				// $('.disc').attr('min','0');
				// $('.disc').attr('max', max);
				// $('.disc').attr("onkeyup","checkmax(max,id,"+index+")");
				var amt = $("#toamt_"+index).val();
				var dis = $("#disc_"+index).val();
				var dec = (dis/100).toFixed(2);
				var mult = amt*dec;
