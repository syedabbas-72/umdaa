...
public function get_dashboard_details()
{
	...
	if($did=='all')
	{
		$registrations = $this->db->query("select count(patient_id) as pcnt from patients where created_date_time between '".$start."%' and '".$end."%'")->row();
		$consultations = $this->db->query("select sum(amount) as camt from billing_line_items a inner join billing b on a.billing_id=b.billing_id where clinic_id=".$cid." and (b.created_date_time between '".$start."%' and '".$end."%') and item_information='Consultation'")->row();
	}
	else{
		$registrations = $this->db->query("select count(patient_id) as pcnt from patients where created_date_time between '".$start."%' and '".$end."%'")->row();
		$consultations = $this->db->query("select sum(amount) as camt from billing_line_items a inner join billing b on a.billing_id=b.billing_id where clinic_id=".$cid." and doctor_id=".$did." and (b.created_date_time between '".$start."%' and '".$end."%') and item_information='Consultation'")->row();
	}
...
public function pharmacy_dashboard()
{
	...
	//$data['drug'] = $this->db->query("select sum(amount) as camt from billing_line_items where created_date_time like '".$tdate."%' and (item_information!='Consultation' and item_information!='Registration' and item_information is Not NULL)")->row();
...
public function get_drug_info()
{
	...
	if(count($dinfo)>0)
	{
		...
		if(count($invinfo)>0)
			echo $dinfo->drug_id.":".$dinfo->formulation.":".$dinfo->composition.":".$invinfo->reorder_level.":".$invinfo->igst.":".$invinfo->cgst.":".$invinfo->sgst.":".$invinfo->max_discount_percentage;
		else
		{
			echo $dinfo->drug_id.":".$dinfo->formulation.":".$dinfo->composition.":"."".":".$dinfo->igst.":".$dinfo->cgst.":".$dinfo->sgst.":"."";
		}
	}
	else
	{		
