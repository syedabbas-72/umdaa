...
class Api extends REST_Controller {
    ...
    public function doctor_registration($parameters, $method, $user_id){
        ...
        $docRec = $this->db->select('doctor_id, mobile, email')->from('doctors')->where('mobile =',$mobile)->or_where('registration_code =',$registration_code)->or_where('email =',$email)->get()->row();
        ...
        if(count($docRec) == 0){
            ...
            if($docRec->mobile == $mobile){
                $message = "The mobile number ".$mobile." already registered, please try with an other number";
            }

            if($docRec->email == $email){
                $message = "The email id ".$email." already registered, please try with an other id";
            }
    ...
    if (isset($parameters['mobile'])) {
        ...
        if (count($patients) > 0) {
            ...
            foreach ($patients as $patient) {
                ...
                if($patient->email_id != '' || $patient->email_id != NULL)
                    $patient_val['appointment_list'][$i]['email'] = DataCrypt($patient->email_id,'decrypt');
                else
                    $patient_val['appointment_list'][$i]['email'] = null;
...
//inventory outwards insert
    ...
    public function getConsentforms($parameters, $method, $user_id) {
        ...
        $consent_forms = $this->db->select("a.consent_form_id, a.consent_form_title")->from("consent_form a")->join("consent_form_department b","a.consent_form_id = b.consent_form_id")->where("b.department_id='" . $parameters['department_id'] . "'")->order_by('a.consent_form_title','ASC')->get()->result();
