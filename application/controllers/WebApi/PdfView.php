...
public function labReport($billing_id, $investigation_id){
    ...
    $data['billingLineItemsInfo'] = $this->db->select("*")->from("billing_line_items")->where("billing_id",$billing_id)->get()->result();
    $data['patientInfo'] = $this->db->select("*")->from("patients")->where("patient_id",$data['billingInfo']->patient_id)->get()->row();
    $clinic_id = $data['billingInfo']->clinic_id;
    if($data['billingInfo']->doctor_id != 0)
    {
    ...
    $data['view'] = 'pdfViews/view_lab_report';
    $this->load->view('pdfViews/pdfLayout',$data);
 }

 public function eo_webview($expert_opinion_id){
    $data['header'] = 0;
    $data['expInfo'] = $this->db->query("select * from expert_opinion where expert_opinion_id='".$expert_opinion_id."'")->row();
    $data['cdInfo'] = $this->db->query("select * from patient_clinical_diagnosis pcd,patient_cd_line_items pcdl where pcd.patient_clinical_diagnosis_id=pcdl.patient_clinical_diagnosis_id and pcd.expert_opinion_id='".$expert_opinion_id."'")->result();
    $data['invInfo'] = $this->db->query("select * from patient_investigation pin,patient_investigation_line_items pil where pin.patient_investigation_id=pil.patient_investigation_id and pin.expert_opinion_id='".$expert_opinion_id."'")->result();
    $data['presInfo'] = $this->db->query("select * from patient_prescription pp,patient_prescription_drug ppd where pp.patient_prescription_id=ppd.patient_prescription_id and pp.expert_opinion_id='".$expert_opinion_id."'")->result();
