...
class Charts extends REST_Controller1
{
    ...
    //       exit();
     ...
     // End consultation
            ...
            $json['AnalyticsTable'][0]['data'][3]['name']= "Tele Consultation";
            $json['AnalyticsTable'][0]['data'][3]['no']= "4";
            $json['AnalyticsTable'][0]['data'][3]['amount']= (int)$telePieAmount;

            $json['AnalyticsTable'][0]['data'][4]['name']= "Discounts";
            $json['AnalyticsTable'][0]['data'][4]['no']= "5";
            $json['AnalyticsTable'][0]['data'][4]['amount']= (int)$totalDiscount;

            $json['AnalyticsTable'][0]['data'][5]['no']= "6";
            $json['AnalyticsTable'][0]['data'][5]['amount']=  (int)$pielrAmount;
            $json['AnalyticsTable'][0]['data'][5]['name']= "Lost Revenue";

            $json['AnalyticsTable'][0]['data'][6]['no']= "7";
            $json['AnalyticsTable'][0]['data'][6]['amount']= (int)$pharmacy_total_count;
            $json['AnalyticsTable'][0]['data'][6]['name']="Pharmacy Revenue";
            ...
            $clinic_id = $clinics_info[0]->clinic_id;
                  ...
                  $dCond = " (created_date_time BETWEEN '".$startDate."%' AND '".$endDate."%')";
                    ...
                    if(count($patients)>0)
                    {
                      ...
                      {
                        $json['AnalyticsPie']['label'][]=  $pt->location;
                        $json['AnalyticsPie']['data'][]= (int)$pt->count;

                        $json['AnalyticsTable'][0]['data'][$j]['name']= $pt->location;
        ...
        // Lab Avg Ticket Size
            for($a=$start_time;$a<=$end_time;$a++)
            {
               $lab_amount = $this->db->query("SELECT 
                ...
                if(count($lab_amount)>0)
                {
                    ...
                    $amount[] = array('no' => $int .$a,'amount' => $lab_amountt,'count' => $patient_countt,'avg' =>($lab_amountt == 0 || $patient_countt == 0)?0:(int)($lab_amountt/$patient_countt) );
            ...
            //    exit();
                ...
                if(count($pharmacy_amount)>0)
                {
                    ...
                    $amountt[] = array('no' => $int .$a,'amount' => $pharmacy_amountt,'count' => $patient_pharmacy_countt,'avg' =>($pharmacy_amountt == 0 || $patient_pharmacy_countt == 0)?0:(int)($pharmacy_amountt/$patient_pharmacy_countt) );
                    // $count[] = array_push($count, array('count' => $patient_countt));
