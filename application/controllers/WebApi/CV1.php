...
class CV1 extends REST_Controller
{
    ...
    public function __construct() {
        ...
        if ($this->post('requestpara') != NULL || $this->post('requestpara') != '') {
            ...
            if ($requestname == "patient_registration") {
              ...
              // echo $mobile;
            ...
            else if ($requestname == "patient_website_registration") {
                ...
                $data1= $this->Generic_model->insertDataReturnId('patients', $patient_update);
                ...
                if (count($last_umr) > 0) {
                    ...
                    if ($split_umr == date("my")) {
                        ...
                        $umr_no  = "P" . date("my") . $next_id;
                        // $udata['status'] = 0;
                        $patient_umr_no['umr_no'] = $umr_no;
                        $patient_umr_no['username'] = $umr_no;
                    } else {
                        $umr_no = "P" . date("my") . $data1;
                        $patient_umr_no['umr_no'] = $umr_no;
                        $patient_umr_no['username'] = $umr_no;
                    }
                } else {
                ...
                $this->Generic_model->updateData("patients", $patient_umr_no, array('patient_id' => $data1));
                $result = array('code' => '200', 'message' => 'Patient_created', 'result' => $data1, 'requestname' => 'patient_website_registration');
            }
            ...
            //replacing old image with new image(vikram)
            ...
            else if ($requestname == "patient_registration_someOneElse") {
                ...
                {
                    $patient_update['alternate_mobile'] = DataCrypt($fdata->alternate_mobile,'encrypt');                
                    $patient_update['mobile'] = '';
                }

                // if($fdata->alternate_mobile != '' || $fdata->alternate_mobile != null)
                //     $patient_update['alternate_mobile'] = DataCrypt($fdata->mobile,'encrypt');
                ...
                $patient_id = $this->Generic_model->insertDataReturnId('patients', $patient_update);

                $umr_no = "P".date("my").$patient_id;
                $data1['username'] = $umr_no;
                ...
                $this->Generic_model->updateData("patients", $data1, array('patient_id' => $patient_id));

                $result = array('code' => '200', 'message' => 'Patient_created', 'result' => $data1, 'requestname' => 'patient_website_registration');
            }
            else if($requestname == "patient_registration_new") {
            ...
            else if($requestname == "my_records_insert")
            {
                ...
                $config['allowed_types'] = 'jpg|JPG|png|PNG|csv|jpeg|pdf|PDF|doc|docx|DOC|DOCX';
                $file_count = count($_FILES['file_i']['name']);
                for ($i = 0; $i < $file_count; $i++) {
                    $_FILES['file_i[]']['name'] = $_FILES['file_i']['name'][$i];
                    $_FILES['file_i[]']['type'] = $_FILES['file_i']['type'][$i];
    ...
    public function patient_login_otp($parameters, $method, $user_id) {
        ...
        $otpsms = "Dear User,%nYour One Time Password (OTP) is $Rotp. Please enter the OTP to proceed.%nThank You,%nUMDAA Health Care.";
...
//Add Family Person to existing person
...
public function departments($parameters, $method, $user_id){
    ...
    if(count($departmentInfo)>0)
    {
        ...
        foreach($departmentInfo as $value)
        {
            ...
            $diseasesInfo = $this->db->select("*")->from("doctors")
            ->where("department_id ='" . $value->department_id . "'")
            ->get()->result();
            $a = 0;
            $diseases=array();
            foreach($diseasesInfo as $value)
            {
                if($value->diseases_dealt != '')
                {
                    $diseases[]= $value->diseases_dealt.",";
                }
                $a++;
            }
                // print_r($diseases);
            ...
            $para['departments'][$i]['department_diseases'] = implode(" ",$diseases);
            $para['departments'][$i]['department_icon'] = $src;
            // $para['departments'][$i]['department_icon'] = base_url('uploads/departments/'.$src);
            $i++;
        }
        $para['departments'][$i]['department_id'] = 0;
        $para['departments'][$i]['department_diseases'] = 'others';
...
public function getIndDoc($parameters, $method, $user_id) {
    ...
    //             $date = date('Y-m-d', strtotime('-1 day', strtotime($value)));
    //             $wday = date('N', strtotime($date));
    //             $para['doctor']['clinics'][0]['booked_slots'][$k]['date'] = $date;
    ...
    //                 }
    //                 $para['doctor']['clinics'][0]['booked_slots'][$k]['time_slot'] = $b_slots;
    //             } else {
    //                 $para['doctor']['clinics'][0]['booked_slots'][$k]['time_slot'] = NULL;
    //             }
    //             $k++;
    //             $blocking = $this->db->select("*")->from("calendar_blocking")->where("clinic_id='" . $clinic_id . "' and doctor_id='" . $doctor_id . "'")->get()->row();
    ...
    //             }
    //             $blocked_list = implode(',', $blocked_dates);
    //             $para['doctor']['clinics'][0]['blocked_dates'] = $blocked_list;
    //         }
    ...
    //                     $para['doctor']['clinics'][0]['booked_appointments'][$x] = $booked;
    //                     $x++;
    //                 }
    //             }else{
    //                 $para['doctor']['clinics'][0]['booked_appointments'] = $booked_list;
    //             }           
    //         }
    ...
    //                     $j = 0;
    //                     $para['doctor']['clinics'][0]['working_days'][$i]['dayNum'] = $value->weekday;
    //                     $para['doctor']['clinics'][0]['working_days'][$i]['day'] = $days[$value->weekday];
    //                     foreach($slotsInfo as $val){
    //                         $para['doctor']['clinics'][0]['working_days'][$i]['timings'][$j]['session'] = ucwords($val->session);
    //                         $para['doctor']['clinics'][0]['working_days'][$i]['timings'][$j]['schedule'] = date('h:i A', strtotime($val->from_time)).'-'.date('h:i A', strtotime($val->to_time));
                    ...
                    if (count($clinics_list) > 0) {
                        ...
                        foreach ($clinics_list as $clinic) {
                            ...
                            foreach ($weekOfdays as $key => $value) {
                                ...
                                $wday = date('N', strtotime($date));
                                $param['doctor']['clinics'][$c]['booked_slots'][$k]['date'] = $date;
                                $bs = $this->db->select("*")->from("appointments")->where("clinic_id='".$clinic->clinic_id."' and doctor_id='".$doctor_id."' and appointment_date='".$date."' and slot_type='video call'")->get()->result();
                                        //   echo $this->db->last_query();
                                    // exit();
                            ...
                            $param['doctor']['clinics'][$c]['clinic_id'] = $clinic->clinic_id;
                            $param['doctor']['clinics'][$c]['clinic_name'] = $clinic->clinic_name;
                            if($clinic->consulting_time=="")
                                $param['doctor']['clinics'][$c]['consultation_time'] = "10";
                            else
                                $param['doctor']['clinics'][$c]['consultation_time'] = $clinic->consulting_time;
                            $param['doctor']['clinics'][$c]['consulting_fee'] = $clinic->consulting_fee;
                            $param['doctor']['clinics'][$c]['registration_fee'] = $clinic->registration_fee;
                            ...
                            $week_day_list = $this->db->select("clinic_doctor_weekday_id,weekday")->from("clinic_doctor_weekdays")->where("clinic_doctor_id ='" . $clinics_list[0]->clinic_doctor_id . "' and slot='video call'")->group_by("weekday")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
                            ...
                            foreach ($week_day_list as $weekday) {
                                ...
                                if ($weekday->weekday == 1) {
                                    $week = "Monday";
                                    $check =  $this->db->query("select * from doctor_availability where doctor_id='".$doctorInfo->doctor_id ."' and feature='Tele' and status='1' and weekday='".$weekday->weekday."' and available='1'")->row();
                                    if(count($check)>0)
                                    {
                                        $sessions_list[] = date('H:i:s', strtotime($check->from_time)) . "-" . date('H:i:s', strtotime($check->to_time)). "-" .$check->session;
                                    }else{
                                        $week_day_list_slots = $this->db->select("*")->from("clinic_doctor_weekday_slots")->where("clinic_doctor_weekday_id ='" . $weekday->clinic_doctor_weekday_id . "'")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
                                        $cdws = 0;
                                        foreach ($week_day_list_slots as $wdls) {
                                ...
                                if ($weekday->weekday == 2) {
                                    $week = "Tuesday";
                                    $check =  $this->db->query("select * from doctor_availability where doctor_id='".$doctorInfo->doctor_id ."' and feature='Tele' and weekday='".$weekday->weekday."'  and status='1' and available='1'")->row();
                                    if(count($check)>0)
                                    {
                                        $sessions_list[] = date('H:i:s', strtotime($check->from_time)) . "-" . date('H:i:s', strtotime($check->to_time)). "-" .$check->session;
                                    }else{
                                ...
                                // $week_day_list_slots = $this->db->select("*")->from("clinic_doctor_weekday_slots")->where("clinic_doctor_weekday_id ='" . $weekday->clinic_doctor_weekday_id . "'")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
                                // // echo "<br>";echo $this->db->last_query();exit(); "<br>";
                                // $cdws = 0;
                                // foreach ($week_day_list_slots as $wdls) {
                                //     //  $sessions_list[] = date('h:i A', strtotime($wdls->from_time)) . "-" . date('h:i A', strtotime($wdls->to_time));
                                ...
                                $param['doctor']['clinics'][$c]['working_days'][$cdw]['day'] = $week;
                                $param['doctor']['clinics'][$c]['working_days'][$cdw]['dayNum'] = $weekday->weekday;
                ...
                if (count($clinics_list) > 0) {
                    ...
                    foreach ($clinics_list as $clinic) {
                        ...
                        foreach ($weekOfdays as $key => $value) {
                            ...
                            $wday = date('N', strtotime($date));
                            $param['doctor']['clinics'][$c]['booked_slots'][$k]['date'] = $date;
                            $bs = $this->db->select("*")->from("appointments")->where("clinic_id='".$clinic->clinic_id."' and doctor_id='".$doctor_id."' and appointment_date='".$date."' and slot_type='walkin'")->get()->result();
                                    //   echo $this->db->last_query();
                                // exit();
                        ...
                        $week_day_list = $this->db->select("clinic_doctor_weekday_id,weekday")->from("clinic_doctor_weekdays")->where("clinic_doctor_id ='" . $clinic->clinic_doctor_id . "' and slot='".$slot."'")->group_by("weekday")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
    ...
    if (count($doctor_appointment_list) > 0) {
        ...
        foreach ($doctor_appointment_list as $doc) {
            ...
            if (count($docInfo) > 0) {
                ...
                $para['docList'][$i]['profile_pic'] = $docDetails->profile_image != ''?base_url('uploads/doctors/' . $docDetails->profile_image):'';
...
public function health_education_articless($parameters, $method, $user_id) {
...
public function health_education_articles($parameters, $method, $user_id) {
    $article_info = $this->db->query("SELECT * from articles INNER JOIN article_department ON articles.article_id=article_department.article_id where articles.article_status='published' and (articles.citizens='1' or articles.citizens='2') order by articles.article_id DESC")->result();
    //  echo $this->db->last_query();
    //  exit();
    //  $articles = $this->db->query("select * from articles a,article_department ad where a.article_id=ad.article_id and a.posted_by='".$id."' group by a.article_id")->result();
    $j = 0;
    foreach ($article_info as $value) 
    {
        $para['articles'][$j]['article_author'] = $value->article_author;
        $data['articles'][$j]['read_article_link'] = $value->read_article_link;
        $para['articles'][$j]['posted_time'] = date('d-m-Y h:i A', strtotime($value->created_date_time));
        $para['articles'][$j]['article_status'] = $value->article_status;
        $para['articles'][$j]['department_name'] = $doctor_list->department_name;
        $para['articles'][$j]['fullname'] = "Dr. ".$doctor_list->first_name." ".$doctor_list->last_name; 
        ...
        $para['articles'][$j]['article_status'] = $value->article_status;
        $para['articles'][$j]['type'] = $value->article_type;
        $para['articles'][$j]['read_article_link'] = $value->read_article_link;
        ...
        elseif(strtolower($value->article_type) == "image")
        {
            ...
            foreach($images as $value)
            {
                ...
                $para['articles'][$j]['article_image'][] = base_url('uploads/article_images/'.$igm);
            }   
        ...
        $this->response(array('code'=>'200','message'=>'Doctors Data','result'=>$para,'requestname'=>$method));
}
...
// }
    ...
    public function healtRecordPDFF($parameters, $method, $user_id)
    {
    ...
    */ 
    ...
    public function doctor_videos($parameters,$method,$user_id)
    {
        ...
        if(count($videoInfo)>0)
        {
            ...
            foreach($videoInfo as $value)
            {
                ...
                $data['videos'][$i]['video_url'] = $value->video_url != ''?'https://www.youtube.com/embed/'.$value->video_url:'';
    ...
    public function save_billing($parameters,$method,$user_id){
      ...
      public function book_appointment($parameters, $method, $user_id)
      {
          ...
          */
          ...
          $check_appointment = $this->db->query("select * from appointments where doctor_id='" . $doctor_id . "' and patient_id='".$patient_id."'")->row();

          $dept_info = $this->db->query("select * from department where department_id='" . $doctor_info->department_id . "'")->row();
    ...
    public function family_health_records($parameters, $method, $user_id)
    {
        ...
        $insert_checklist['id'] = $this->Generic_model->insertDataReturnId('patient_family_health_records', $insert_checklist);
...
public function citizen_app_version($parameters,$method,$user_id)
{
    ...
    public function doctor_patient_documents($parameters,$method,$user_id)
    {
        ...
        $check = $this->db->query("select * from doctor_patient_documents where doctor_id='".$doctor_id."' and patient_id='".$patient_id."'")->row();
        if(count($check)>0)
        {
            $this->response(array('code'=>'201','message' => 'Already shared','requestname'=>$method));
        }else{
    ...
    public function checkPatientUmrNo($parameters, $method, $user_id)
    {
        ...
        else
        {
            ...
            // $data['patient_details']['registration_fee']=100;
            // $data['patient_details']['status']="0";
            $this->response(array('code'=>'201','message' =>'Success','result' => $data,'requestname'=> $method));
        }
    ...
    public function checkExists($parameters, $method, $user_id)
    {
        extract($parameters);
        ...
        if(count($check)>0)
        {
            ...
            foreach($check as $value)
            {
                ...
                $data['patients'][$i]['profile_photo'] = $value->photo != ''?base_url().'uploads/patients/'.$value->photo:'assets/images/profile.png';   
                $i++;             
            }
        }
        else
    ...
    public function getUpcomingAppointments($parameters, $method, $user_id){
        ...
        // echo $this->db->last_query();
        $check = $this->db->query("select *,p.title,p.title,p.first_name,p.last_name,p.umr_no,p.patient_id,
        c.clinic_name,c.location FROM appointments a,patients p,clinics c WHERE a.patient_id=p.patient_id and a.clinic_id=c.clinic_id and a.patient_id='".$patient_id."' and a.status != 'closed'")->result();
        if(count($check) > 0)
        {
            $i = 0;
        ...
        else
        {
            ...
            $this->response(array('code'=>'201','message'=>'No Data Found.','result'=>$para,'method'=>$method));
        }
    }
    ...
    public function filterData($parameters, $method, $user_id){
        extract($parameters);
        
        $this->response(array('code'=>'200','message'=>'Fasak','result'=>'Fasak','method'=>$method));
    }

   public function doctorsListNew($parameters, $method, $user_id)
{
    extract($parameters);
    $docInfo = $this->db->query("select * from doctors d,department de where d.department_id=de.department_id and d.department_id='".$department_id."'")->result();
    if(count($docInfo)>0)
    {
        ...
        foreach($docInfo as $value)
        {
            $doctor_degrees_list = $this->db->query("select * from doctor_degrees where doctor_id ='".$value->doctor_id."'")->result();
            $clinicsInfo = $this->db->query("select c.clinic_id,c.clinic_name,CONCAT(c.address,' ',c.location) as clinic_address,c.state_id,c.district_id,c.clinic_phone,c.email,c.pincode from clinics c,clinic_doctor cd where c.clinic_id=cd.clinic_id and cd.doctor_id='".$value->doctor_id."'")->result();
            $fasak = $this->db->query("select c.clinic_id,c.clinic_name,CONCAT(c.address,' ',c.location) as clinic_address,c.state_id,c.district_id,c.clinic_phone,c.email,c.pincode from clinics c,clinic_doctor cd where c.clinic_id=cd.clinic_id and cd.doctor_id='".$value->doctor_id."'")->row();
            // echo $this->db->last_query();
            // echo $clinics->clinic_id;
            // echo $clinicsInfo->clinic_id;
            // echo date('l');
            // exit();
            $clinics = array();
            $k = 0;
            foreach($clinicsInfo as $clinicValue){
                ...
                $stateInfo = $this->db->query("select * from states where state_id='".$clinicValue->state_id."'")->row();
                $districtInfo = $this->db->query("select * from districts where district_id='".$clinicValue->district_id."'")->row();
                // $clinic_doctor = $this->db->query("select * from clinic_doctor where clinic_id='3' and doctor_id='7'")->row();
                $clinics[$k]['clinic_id'] = $clinicValue->clinic_id;
                // $clinics[$k]['consultation_fees'] = $clinic_doctor->consulting_fees;
                // $clinics[$k]['online_consultation_fees'] = $clinic_doctor->online_consulting_fees;
                $clinics[$k]['clinic_name'] = $clinicValue->clinic_name;
                $clinics[$k]['clinic_address'] = $clinicValue->clinic_address.", ".$districtInfo->district_name.", ".$stateInfo->state_name." - ".$clinicValue->pincode;
                $clinics[$k]['clinic_phone'] = $clinicValue->clinic_phone;
                $clinics[$k]['email'] = $clinicValue->email;
        ...
        //    $clinics= $this->db->query("select c.clinic_id,c.clinic_name,CONCAT(c.address,' ',c.location) as clinic_address,c.state_id,c.district_id,c.clinic_phone,c.email,c.pincode from clinics c,clinic_doctor cd where c.clinic_id=cd.clinic_id and cd.doctor_id='".$value->doctor_id."'")->row();
        ...
        //         $clinic_doctor_id = $this->db->query("select * from clinic_doctor where clinic_id='".$clinic_id."' and doctor_id='".$doctor_id."'")->row();
                
        //     //     echo $this->db->last_query();
        //         $clinic_doctor_weekdays = $this->db->query("select * from clinic_doctor_weekdays where clinic_doctor_id='".$clinic_doctor_id->clinic_doctor_id."' and weekday='".$day."' and slot='walkin'")->row();
        //     //     echo $this->db->last_query();
        //     //     // echo $clinic_doctor_weekdays->clinic_doctor_weekday_id;
        //     //     exit();
        //     //     $availableDoctors = array();
        //         $a = 0;
        //         $availableDoctorsInfo = $this->db->query("SELECT * FROM clinic_doctor_weekday_slots WHERE '".$time."' BETWEEN from_time AND to_time and clinic_doctor_weekday_id='".$clinic_doctor_weekdays->clinic_doctor_weekday_id."'")->row();
        //         // echo $this->db->last_query();
        //         // exit();
                
        //         if(count($availableDoctorsInfo)>0)
        //         {
        //             $data['available'][$a]['doctor_id'] = $value->doctor_id;
        //             $data['available'][$a]['clinic_id'] = $clinic_id;
        //             $data['available'][$a]['clinic_doctor_weekday_slot_id'] = $availableDoctorsInfo->clinic_doctor_weekday_slot_id;
        //             $data['available'][$a]['clinic_doctor_weekday_id'] = $availableDoctorsInfo->clinic_doctor_weekday_id;
        //             $a++;
        //         }
        //         else{
        ...
        //         }
            ...
            $doctor_degree = implode(",", $degree_name);
            $data['doctors'][$j]['fasak'] = $fasak->clinic_id;
            $data['doctors'][$j]['doctor_id'] = $value->doctor_id;
            $data['doctors'][$j]['specialization'] =$value->speciality ;
            $data['doctors'][$j]['consultation_fees'] = $value->doctor_id;
            $data['doctors'][$j]['online_consultation_fees'] = $value->doctor_id;
            $data['doctors'][$j]['doctor_name'] = $value->first_name." ".$value->last_name;
            $data['doctors'][$j]['department_name'] = $value->department_name;
            $data['doctors'][$j]['experience'] = $value->experience." Years";
            $data['doctors'][$j]['about_doctor'] = $value->about ;
            ...
            $data['doctors'][$j]['qualification'] = $value->qualification;
            $data['doctors'][$j]['clinics'] = $clinics;
            $data['doctors'][$j]['avaliable_doctors'] = $availableDoctors;
        ...
        {
            $fasak = $this->db->query("select c.clinic_id,c.clinic_name,CONCAT(c.address,' ',c.location) as clinic_address,c.state_id,c.district_id,c.clinic_phone,c.email,c.pincode from clinics c,clinic_doctor cd where c.clinic_id=cd.clinic_id and cd.doctor_id='".$value->doctor_id."'")->row();
            $clinic_doctor_id = $this->db->query("select * from clinic_doctor where clinic_id='".$fasak->clinic_id."' and doctor_id='".$value->doctor_id."'")->row();
            $clinic_doctor_weekdays = $this->db->query("select * from clinic_doctor_weekdays where clinic_doctor_id='".$clinic_doctor_id->clinic_doctor_id."' and weekday='".$day."' and slot='walkin'")->row();
            $availableDoctorsInfo = $this->db->query("SELECT * FROM clinic_doctor_weekday_slots WHERE '".$time."' BETWEEN from_time AND to_time and clinic_doctor_weekday_id='".$clinic_doctor_weekdays->clinic_doctor_weekday_id."'")->row();

            if($availableDoctorsInfo->clinic_doctor_weekday_slot_id != null)
            {
                $data['available'][$k]['doctor_id'] = $value->doctor_id;
                $data['available'][$k]['time'] = $time;
                $data['available'][$k]['day'] = $day;
                $data['available'][$k]['clinic_doctor_weekdays'] = $clinic_doctor_weekdays->clinic_doctor_weekday_id;
                $data['available'][$k]['clinic_id'] = $fasak->clinic_id;
                $data['available'][$k]['clinic_doctor_id'] = $clinic_doctor_id->clinic_doctor_id;
                $data['available'][$k]['clinic_doctor_weekday_slot_id'] = $availableDoctorsInfo->clinic_doctor_weekday_slot_id;
                $k++;
            }
...
public function doctorsListNeww($parameters, $method, $user_id)
{
    extract($parameters);
    ...
    if(count($docInfo)>0)
    {
        ...
        foreach($docInfo as $value)
        {
            ...
            foreach($clinicsInfo as $clinicValue){
                    // Available Doctors
                    $clinic_id=$clinicValue->clinic_id; 
                    $doctor_id=$value->doctor_id;

                    $clinic_doctor = $this->db->query("select * from clinic_doctor where clinic_id='".$clinic_id."' and doctor_id='".$doctor_id."'")->row();
                    
                    
                    $clinic_doctor_weekdays = $this->db->query("select * from clinic_doctor_weekdays where clinic_doctor_id='".$clinic_doctor->clinic_doctor_id."' and weekday='".$day."' and slot='walkin'")->row();
                    // echo $this->db->last_query();
                    // echo $clinic_doctor_weekdays->clinic_doctor_weekday_id;
                    // exit();
                    $availableDoctors = array();
            ...
            //     $districtInfo = $this->db->query("select * from districts where district_id='".$clinicValue->district_id."'")->row();
            //     // $clinic_doctor = $this->db->query("select * from clinic_doctor where clinic_id='3' and doctor_id='7'")->row();
            //     $clinics[$a]['clinic_id'] = $clinicValue->clinic_id;
            //     // $clinics[$k]['consultation_fees'] = $clinic_doctor->consulting_fees;
            //     // $clinics[$k]['online_consultation_fees'] = $clinic_doctor->online_consulting_fees;
            //     $clinics[$a]['clinic_name'] = $clinicValue->clinic_name;
            //     $clinics[$a]['clinic_address'] = $clinicValue->clinic_address.", ".$districtInfo->district_name.", ".$stateInfo->state_name." - ".$clinicValue->pincode;
            //     $clinics[$a]['clinic_phone'] = $clinicValue->clinic_phone;
            //     $clinics[$a]['email'] = $clinicValue->email;
            //     $a++;
            // }
...
{
    extract($parameters);
    $reptInfo = $this->db->query("select images from citizen_records where citizen_record_id='" . $previous_document_id . "'")->row();
    $images = explode(",", $reptInfo->images);
    $i = 0;
    foreach ($images as $value) {
        //    echo "ss". substr($value,-3);   
        if (substr($value, -3) == "pdf") {
            $data[$i]['image'] = base_url('uploads/my_records/' . $value);
            $data[$i]['type'] = "pdf";
...
public function docList($parameters, $method, $user_id){
    extract($parameters);
    // $this->response(array('code'=>'200','message'=>$price,'result'=>$experience));
    $presentDay = date("N");
    $present_time = date("H:i:s");
    if($experience != ""){
        $exQ = $experience;
    ...
    if(count($doctors) > 0){
        ...
        foreach($doctors as $doc){
            ...
            $clinicsInfo = $this->db->query("select c.clinic_id,c.clinic_logo,c.clinic_name,CONCAT(c.address,' ',c.location) as clinic_address,c.state_id,c.district_id,c.clinic_phone,c.email,c.pincode from clinics c,clinic_doctor cd where c.clinic_id=cd.clinic_id and cd.doctor_id='".$doc->doctor_id."'")->result();
            $clinic_doctor = $this->db->query("select * from clinic_doctor where doctor_id='".$doc->doctor_id."'")->result();
            $clinic_doctor_id = $clinic_doctor[0]->clinic_doctor_id;
            $docInfo = $this->db->query("SELECT cd.doctor_id,cd.clinic_doctor_id,cd.consulting_fee,cd.online_consulting_fee,cd.clinic_id,cwd.clinic_doctor_weekday_id,cwd.slot from clinic_doctor cd,clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds where cd.clinic_doctor_id=cwd.clinic_doctor_id and cwd.slot='video call' and cwd.clinic_doctor_id='".$clinic_doctor_id."'and cwd.clinic_doctor_weekday_id=cwds.clinic_doctor_weekday_id and cwd.weekday='".$presentDay."' and cd.doctor_id='".$doc->doctor_id."' and '".$present_time."' > cwds.from_time and '".$present_time."' < cwds.to_time group by cd.doctor_id order by cwds.from_time ASC, cd.online_consulting_fee ".$priceQ)->row();
            // echo $this->db->last_query();
            // exit();
            $clinicsData = getDocClinicDetails($doc->doctor_id);
            $docDetails = doctorDetails($doc->doctor_id);
            if(count($docInfo) > 0){
                // $langs = explode(",",$doc->languages);
                // $lang = "";
                ...
                // $lang = substr($lang, 0, -1);
                    ...
                    $para['docList'][$i]['profile_pic'] = $docDetails->profile_image != '' ?base_url('uploads/doctors/'.$docDetails->profile_image):'';
                    $para['docList'][$i]['availability'] = "Available";
                    $para['docList'][$i]['available'] = "1";
                    $para['docList'][$i]['slot_type'] = $docInfo->slot;
                    $clinics = array();
                    $k = 0;
                    foreach($clinicsInfo as $clinicValue){
                        $stateInfo = $this->db->query("select * from states where state_id='".$clinicValue->state_id."'")->row();
                        ...
                        $clinics[$k]['clinic_id'] = $clinicValue->clinic_id;
                        $clinics[$k]['clinic_name'] = $clinicValue->clinic_name;
                        $clinics[$k]['clinic_logo'] =  $clinicValue->clinic_logo != '' ?base_url('uploads/clinic_logos/'.$clinicValue->clinic_logo):'';
                        $clinics[$k]['clinic_address'] = $clinicValue->clinic_address.", ".$districtInfo->district_name.", ".$stateInfo->state_name." - ".$clinicValue->pincode;
                        $clinics[$k]['clinic_phone'] = $clinicValue->clinic_phone;
                        $clinics[$k]['email'] = $clinicValue->email;
                        $k++;
                ...
                // $docInfo = $this->db->query("SELECT cd.doctor_id,cd.clinic_doctor_id,
                // cd.consulting_fee,cd.online_consulting_fee,cd.clinic_id,
                // cwd.clinic_doctor_weekday_id 
                // from clinic_doctor cd,clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds 
                // where cd.clinic_doctor_id=cwd.clinic_doctor_id 
                // and cwd.clinic_doctor_weekday_id=cwds.clinic_doctor_weekday_id 
                // and cwd.weekday='".$presentDay."' and cd.doctor_id='".$doc->doctor_id."' 
                // and '".$present_time."' > cwds.from_time 
                // and '".$present_time."' < cwds.to_time 
                // group by cd.doctor_id 
                // order by cwds.from_time ASC, cd.online_consulting_fee ".$priceQ)->row();

                $docQ = $this->db->query("SELECT doctor_id,consulting_fee,online_consulting_fee,clinic_id from clinic_doctor where doctor_id='".$doc->doctor_id."' group by doctor_id ")->row();
                // $docInfooo = $this->db->query("select * from `clinic_doctor_weekday_slots` as cd join `clinic_doctor_weekdays` as cdw where cd.clinic_doctor_weekday_id=cdw.clinic_doctor_weekday_id and cdw.clinic_doctor_id='7' and cdw.weekday='5' and '22:55:16' BETWEEN from_time and to_time")->row();
                // if(count($docInfooo)>0)
                // {
                //     $next_day = $this->db->query("SELECT * from clinic_doctor_weekdays cwd join clinic_doctor cwds where cwds.clinic_doctor_id=cwd.clinic_doctor_id and cwds.doctor_id='".$doc->doctor_id."' and cwd.slot='walkin'")->row();
                // }
                // else
                // {

                // }
                 //  select * from `clinic_doctor_weekday_slots` as cd join `clinic_doctor_weekdays` as cdw where cd.clinic_doctor_weekday_id=cdw.clinic_doctor_weekday_id and cdw.clinic_doctor_id='7' and cdw.weekday='5' and '22:55:16' BETWEEN from_time and to_time
                 if(count($docQ) > 0){
                    //  $nextAvailabilityInfo = getNextAvailabilityInfo($docQ->doctor_id, $presentDay, $present_time);
                    
                    // if($doc->languages == "telugu"){
                    //     $plang = "te";
                    // }
                    // $langs = explode(",",$doc->languages);
                    // $lang = "";
                ...
                //     $plang = "te";
                // }
                //     $lang = translate($doc->languages,"en","te");
                    // $lang = translate("telugu","en","te");
                    // $clinic_doctor = $this->db->query("select * from clinic_doctor where doctor_id='".$doc->doctor_id."'")->result();
                    // $clinic_doctor_id = $clinic_doctor[0]->clinic_id;
                    //for that day checking
                    $checkNextAvailbality = $this->db->query("SELECT cd.doctor_id,cd.clinic_doctor_id,
                    cd.consulting_fee,cd.online_consulting_fee,cd.clinic_id,cwd.slot,
                    cwd.clinic_doctor_weekday_id,cwd.weekday,cwds.from_time,cwds.to_time
                    from clinic_doctor cd,clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds
                    where 
                    cd.clinic_doctor_id=cwd.clinic_doctor_id
                    and cwd.clinic_doctor_weekday_id=cwds.clinic_doctor_weekday_id 
                    and cwd.clinic_doctor_id='".$clinic_doctor_id."'
                    and cwd.slot='video call' 
                    and cwd.weekday='".$presentDay."' 
                    and cd.doctor_id='".$doc->doctor_id."' 
                    and cwds.to_time > '".$present_time."'  
                    group by cd.doctor_id")->row();

                    // echo $this->db->last_query();
                    //     exit();
    $check_doctor_availablity = $this->db->query("select * from doctor_availability where doctor_id='".$doc->doctor_id."' and available='1' and status='1' and feature='Tele'")->row();
    // echo $this->db->last_query();
    // exit();
    if(count($check_doctor_availablity)>0){
        $nextAvailableTime = "Available";
        $slotType='video call';
        $para['docList'][$i]['available'] = "1";
        
    }else{
                    if(count($checkNextAvailbality)>0)
                    {
                        // $checkTime =  $this->db->query("SELECT * FROM `clinic_doctor_weekday_slots` where clinic_doctor_weekday_id='26' and '".$present_time."'  BETWEEN from_time and to_time")->row();
                        
                        $present_time = date("H:i:s");
                        if($present_time < $checkNextAvailbality->from_time)
                        {
                            $nextAvailableTime = "Available @".date('g:iA', strtotime($checkNextAvailbality->from_time)).','.date("d-m-Y");
                            $slotType='video call';
                            $para['docList'][$i]['available'] = "0";
                        }
                    }
                    else{
                        ...
                        if($presentDay < 7)
                        {
                         ...
                         //for that next day checking(video call)
        
                        //  $clinic_doctor = $this->db->query("select * from clinic_doctor where doctor_id='".$doc->doctor_id."'")->result();
                        //  echo $this->db->last_query();
                        //  exit();
                        //  $clinic_doctor_id = $clinic_doctor[0]->clinic_doctor_id;
                        $checkNextDayAvailbality = $this->db->query("SELECT cd.doctor_id,cd.clinic_doctor_id,
                        cd.consulting_fee,cd.online_consulting_fee,cd.clinic_id,cwd.slot,cwd.weekday,
                        cwd.clinic_doctor_weekday_id,cwds.from_time,cwds.to_time
                        from clinic_doctor cd,clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds
                        where 
                        cd.clinic_doctor_id='".$clinic_doctor_id."'
                        and cwd.clinic_doctor_weekday_id=cwds.clinic_doctor_weekday_id 
                        and cwd.clinic_doctor_id='".$clinic_doctor_id."'
                        and cwd.slot='video call' 
                        and cwd.weekday='".$day."' 
                        ...
                        //    $present_time = date("H:i:s");
                        //    if($present_time < $checkNextDayAvailbality->from_time)
                        //    {
                            if(count($checkNextDayAvailbality)>0)
                            {
                                $nextAvailableTime = "Available @".date('g:iA', strtotime($checkNextDayAvailbality->from_time)).','.date("d-m-Y", strtotime(date("d-m-Y") . "+" . 1 . " day"));
                                $slotType='video call';
                            }else{
                                //for that next day checking(if video call slot not available)
                                $checkNextDayAvailbality = $this->db->query("SELECT cd.doctor_id,cd.clinic_doctor_id,
                                cd.consulting_fee,cd.online_consulting_fee,cd.clinic_id,cwd.slot,cwd.weekday,
                                cwd.clinic_doctor_weekday_id,cwds.from_time,cwds.to_time
                                from clinic_doctor cd,clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds
                    ...
                    $para['docList'][$i]['department_name'] = $docDetails->department_name;
                    $para['docList'][$i]['profile_pic'] = $docDetails->profile_image != '' ?base_url('uploads/doctors/'.$docDetails->profile_image):'';
                    $para['docList'][$i]['availability'] = $nextAvailableTime;
                    $para['docList'][$i]['slot_type'] = $slotType;
                    $clinics = array();
                    $k = 0;
                    foreach($clinicsInfo as $clinicValue){
                        $stateInfo = $this->db->query("select * from states where state_id='".$clinicValue->state_id."'")->row();
    ...
    if($doctor_id != ""){
        ...
        foreach ($weekOfdays as $key => $value) {
            ...
            if (count($bs) > 0) {
                ...
                $para['booked_slots'][$k]['time_slot'] = $b_slots;
            } else {
                $para['booked_slots'][$k]['time_slot'] = NULL;
            }
            $k++;
            $blocking = $this->db->select("*")->from("calendar_blocking")->where("clinic_id='" . $clinic_id . "' and doctor_id='" . $doctor_id . "'")->get()->row();
            ...
            $blocked_list = implode(',', $blocked_dates);
            $para['blocked_dates'] = $blocked_list;
        }
        ...
        if(isset($parameters['patient_id'])){
            ...
            if(count($booked_list) > 0){
                $x = 0;    
                foreach($booked_list as $booked){
                    $para['booked_appointments'][$x] = $booked;
                    $x++;
                }
            }else{
                $para['booked_appointments'] = $booked_list;
            }           
        }
        ...
        if(count($check) > 0){
            ...
            foreach($check as $value){
                ...
                if(count($slotsInfo) > 0){
                    $j = 0;
                    $para['working_days'][$i]['dayNum'] = $value->weekday;
                    $para['working_days'][$i]['day'] = $days[$value->weekday];
                    foreach($slotsInfo as $val){
                        $para['working_days'][$i]['timings'][$j]['session'] = $val->session;
                        $para['working_days'][$i]['timings'][$j]['schedule'] = date('h:i A', strtotime($val->from_time)).'-'.date('h:i A', strtotime($val->to_time));
                        // $para['consultation'][$i]['timings'][$j]['to_time'] = $val->to_time;
                        $j++;
                    }
                }
...
public function getPatientsTreeList($parameters,$method,$user_id){
    ...
    if(count($check) > 0){
        ...
        foreach($check as $value){
            ...
            $para['patient_list'][$i]['profilePic'] =$value->photo != ''?base_url('uploads/patients/'.$value->photo):'';
            // $para['patient_list'][$i]['']
...
public function getDocProfile($parameters, $method, $user_id){
    ...
    if(count($doctor_list) > 0){
        ...
        $doctor_degree = implode(",", $degree_name);
        $deptInfo = $this->db->query("select * from department where department_id='".$doctor_list->department_id."'")->row();
        $data['doctors'][0]['doctor_id'] = $doctor_list->doctor_id;
        $data['doctors'][0]['specialization'] =$doctor_list->speciality ;
        $data['doctors'][0]['department_name'] =$deptInfo->department_name;
        $data['doctors'][0]['name'] = $doctor_list->first_name." ".$doctor_list->last_name;
        $data['doctors'][0]['experience'] = $doctor_list->experience." Years";
        $data['doctors'][0]['about_doctor'] = $doctor_list->about ;
        $data['doctors'][0]['contact'] = $doctor_list->mobile ;
        $data['doctors'][0]['google_review_link'] = $doctor_list->google_review_link ;
        $data['doctors'][0]['gender'] = strtoupper($doctor_list->gender) ;
        $data['doctors'][0]['visit_time'] = "" ;
        $data['doctors'][0]['known_languages'] = $doctor_list->languages ;
        $data['doctors'][0]['dealt_deseases'] = $doctor_list->diseases_dealt;
        $data['doctors'][0]['degrees'] = $doctor_degree;
        $data['doctors'][0]['qualification'] = $doctor_list->qualification;
        $data['doctors'][0]['clinics'] = $clinics;

        $achievements = $doctor_list->acheivements;
        $arra_ach = explode(",", $achievements);

        if($achievements == "" || $achievements == "null"){
            $data['doctors'][0]['achievements'] = array();
        }else{
            $data['doctors'][0]['achievements'] = $arra_ach;

        }

        $data['doctors'][0]['membership'] = $doctor_list->membership_in;
        $data['doctors'][0]['address'] = $doctor_list->address;
        $data['doctors'][0]['doctor_image'] = $doctor_list->profile_image!= ''?base_url("uploads/doctors/".$doctor_list->profile_image.""):'';
        $this->response(array('code'=>'200','message'=>'Doctors Data','result'=>$data,'requestname'=>$method));
    }
}

public function getHashToken($parameters, $method, $user_id)
{
    extract($parameters);
    $paytmParams = array();
    ...
    $paytmParams["body"] = array(
        ...
        "orderId"       => $number,
        // "callbackUrl"   => "https://securegw-stage.paytm.in/theia/paytmCallback?ORDER_ID=$number",
        // "callbackUrl"   => "https://devumdaa.in/citizens/#/payment/26",
    //    "callbackUrl"    => "http://devumdaa.in/dev/WebApi/Pay/checkArray",
       "callbackUrl"    => "https://umdaa.co/clinic/WebApi/Pay/checkArray",
        "txnAmount"     => array(
            "value"     => $amount,
            "currency"  => "INR",
        ),
...
public function doctorShareData($parameters, $method, $user_id)
{
    extract($parameters);
    if(isset($doctor_id)){
        $check = $this->db->query("select * from doctors where doctor_id='".$doctor_id."' ")->row();
        if(count($check) > 0){
            $udata['doctor_name'] = "Dr." . strtoupper($check->first_name . " " . $check->last_name);
            $udata['doctor_link_name'] = "dr".strtolower(str_replace(" ","",$check->first_name.$check->last_name));
            $udata['about'] = $check->about;
            $udata['doctor_image'] = ($check->profile_image != '' ? base_url() . 'uploads/doctors/'.$check->profile_image: null);
            $this->response(array('code'=>'200','message'=>'Success','result'=>$udata));
        }     
        $this->response(array('code'=>'201','message'=>'No Doctor Found'));   
    }
    else{
...
public function doc_info($parameters, $method, $user_id)
{
    extract($parameters);
    $doctors = $this->db->query("select * from doctors where doctor_id='".$doctor_id."'")->row();
    $clinic_logo =  $this->db->query("select clinic_logo from clinics c,clinic_doctor cd
    where c.clinic_id=cd.clinic_id and cd.doctor_id='".$doctor_id."'")->result();
    //  echo $this->db->last_query();
    //  exit();
    if(count($doctors) > 0){
        if(count($clinic_logo)>0)
        {
            $data['doctors'][0]['clinic_logo'] =  $clinic_logo[0]->clinic_logo != '' ?base_url('uploads/clinic_logos/'.$clinic_logo[0]->clinic_logo):'';
        }
        $dept_name = $this->db->query("select * from department where department_id='".$doctors->department_id."'")->row();
        $data['doctors'][0]['doctor_id'] = $doctors->doctor_id;
        $data['doctors'][0]['doctor_name'] = "Dr. ".$doctors->first_name." ".$doctors->last_name;
        $data['doctors'][0]['qualification'] = $doctors->qualification;
        $data['doctors'][0]['department_name'] = $dept_name->department_name;
        $data['doctors'][0]['doctor_image'] = $doctors->profile_image != ''?base_url().'uploads/doctors/'.$doctors->profile_image:'assets/images/profile.png';
     $this->response(array('code'=>'200','message'=>'Doctors Data','result'=>$data,'requestname'=>$method));
    }
    else
    {
        $data['doctors']=array();
        $this->response(array('code'=>'201','message'=>'No Doctors Data','result'=>$data,'requestname'=>$method));
    }
}
