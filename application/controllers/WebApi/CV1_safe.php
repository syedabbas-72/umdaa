...
public function departments($parameters, $method, $user_id){
    $departmentInfo = $this->db->select("department_id,department_name")->from("department")->get()->result();
...
public function citizen_app_version($parameters,$method,$user_id)
{
    ...
    public function getUpcomingAppointments($parameters, $method, $user_id){
        ...
        $check = $this->db->query("select * from appointments a,patients p where a.patient_id=p.patient_id and a.patient_id='".$patient_id."' and CONCAT(a.appointment_date, ' ', a.appointment_time_slot)  > '".$datetime."'")->result();
        // echo $this->db->last_query();
        if(count($check) > 0)
        {
            $i = 0;
    ...
    public function getPreviousAppointments($parameters, $method, $user_id){
        ...
        // $today = date('Y-m-d');
        $datetime = date("Y-m-d H:i:s");
        $check = $this->db->query("select a.*,p.title,p.title,p.first_name,p.last_name,p.umr_no,p.patient_id from appointments a,patients p where a.patient_id=p.patient_id and a.patient_id='".$patient_id."' and CONCAT(a.appointment_date, ' ', a.appointment_time_slot) < '".$datetime."'")->result();
        // echo $this->db->last_query();
        if(count($check) > 0)
        {
            $i = 0;
