    ...
    public function __construct() {
        ...
        if ($this->post('requestpara') != NULL || $this->post('requestpara') != '') {
            ...
            //replacing old image with new image(vikram)
                ...
                $data1['previous_document_id'] = $this->Generic_model->insertDataReturnId('patients', $patient_update);
    ...
    public function getUpcomingAppointments($parameters, $method, $user_id){
        ...
        if(count($check) > 0)
        {
            ...
            foreach($check as $cform)
            {
                ...
                $para['patient_details_info'][$i]['appointment_date'] = $cform->appointment_date;
                $i++;
                // $this->response(array('code'=>'200','message'=>'Success','result'=>$cform->clinic_id,'method'=>$cform[$i]->clinic_id));
            }
...
public function getHashToken($parameters, $method, $user_id)
{
    ...
    //    "callbackUrl"    => "http://devumdaa.in/dev/WebApi/Pay/checkArray",
       "callbackUrl"    => "https://clinic.devumdaa.in/WebApi/Pay/checkArray",
        "txnAmount"     => array(
            "value"     => $amount,
            "currency"  => "INR",
        ),
