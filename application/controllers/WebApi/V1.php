...
require APPPATH . '/controllers/WebApi/V1_1.php';

class V1 extends V1_1 {
    ...
    public function __construct() {
        ...
        if ($this->post('requestpara') != NULL || $this->post('requestpara') != '') {
            ...
            //     // Remove request name from the parameter
              ...
              extract($params);
            //   $result = array('code' => '200', 'message' => 'Patient already', 'result' =>($params), 'requestname' => $requestname);
              // echo $mobile;
                ...
                if($newPatient == 0){
                    ...
                    if(!isset($params['patient_id']))
                    {
                        ...
                        $patientRec = $this->db->select('patient_id,title,first_name,last_name,umr_no, date_of_birth, age, 
                        email_id as email, payment_status,clinic_id,referred_by_type,referred_by,referral_doctor_id,country,
                        occupation,mobile,alternate_mobile,age,gender,location,state_id,district_id,preferred_language,
                        address_line, pincode, photo, qrcode, created_date_time as registration_date, status')
                        ->from('patients')->where('patient_id ='.$params['patient_id'])->get()->row(); 
                        // echo $this->db->last_query();
                    ...
                    {
                        // Get Patient Complete Details
                        $patientRec = $this->db->select('patient_id,title,first_name,last_name,umr_no, date_of_birth, age,payment_status,
                        clinic_id,referred_by_type,referred_by, email_id as email, referral_doctor_id,country,occupation,mobile,
                        alternate_mobile,age,gender,location,state_id,district_id,preferred_language, address_line, pincode, photo,
                        qrcode, created_date_time, status')->from('patients')->where('patient_id ='.$params['patient_id'])->get()->row(); 
                ...
                //     $patientChkk = $this->db->select("*")
                //     ->from("patients")->where("mobile =",DataCrypt($mobile,'encrypt'))->get()->row();
                ...
                //     if ($params['district_id'] != NULL ||  $params['district_id'] != '') {
                //         $districtRec = $this->db->select('district_id, district_name')->from('districts')->where('district_id =',$params['district_id'])->get()->row();
                //         $geoTag = 1;
                //     } else {
                //         $geoTag = 0;
                //     }
                ...
                //     // get state name from state id
                //     if ($params['state_id'] != NULL || $params['state_id'] != '') {
                //         $stateRec = $this->db->select('state_id, state_name')->from('states')->where('state_id =',$params['state_id'])->get()->row();
                //         $geoTag = 1;
                //     } else {
                //         $geoTag = 0;
                //     }
                ...
                //     $patient_update['title'] =  $params['title'];
                //     $patient_update['first_name'] = $params['first_name'];
                //     $patient_update['middle_name'] = $params['middle_name'];
                //     $patient_update['last_name'] = $params['last_name'];
                //     $patient_update['occupation'] = $params['occupation'];
                //     $patient_update['country'] = $params['country'];
                //     $patient_update['last_name'] = $params['last_name'];
                //     $patient_update['gender'] = $params['gender'];
    
                //     if(empty($params['date_of_birth'])) { 
                //         $patient_update['date_of_birth'] = "";
                //     } else 
                //     {  
                //         $patient_update['date_of_birth'] = date('Y-m-d', strtotime($params['date_of_birth'])); 
                //     }
    
                //     $patient_update['age'] = $params['age'];
                //     $patient_update['age_unit'] = strtoupper(substr($params['age_unit'], 0, 1));
                //     $patient_update['mobile'] = DataCrypt($params['mobile'],"encrypt");
                //     $patient_update['alternate_mobile'] = DataCrypt($params['alternate_mobile'],"encrypt");
                //     $patient_update['email_id'] = $params['email_id'];
                //     $patient_update['address_line'] = $params['address_line'];
                //     $patient_update['location'] = $params['location'];
                //     $patient_update['district_id'] = $params['district_id'];
                //     $patient_update['state_id'] = $params['state_id'];
                //     $patient_update['pincode'] = $params['pincode'];
                //     $patient_update['referred_by_type'] =$params['referred_by_type']; 
                //     $patient_update['referred_by'] = $params['referred_by'];
                ...
                //         $paramss['referral_doctor'] = $params['referral_doctor'];
                //         $paramss['referral_doctor']->clinic_id = $params['clinic_id']; 
                //         $paramss['referral_doctor']->status = 1;
                //         $paramss['referral_doctor']->created_by = $requesterid;
                //         $paramss['referral_doctor']->modified_by = $requesterid;
                //         $paramss['referral_doctor']->created_datetime = date('Y-m-d H:i:s');
                //         $paramss['referral_doctor']->modified_datetime = date('Y-m-d H:i:s');
                ...
                //         $patient_update['referral_doctor_id'] = $params['referred_by'];
    
                //         // Get the name of the doctor
                //         $this->db->select('doctor_name');
                //         $this->db->from('referral_doctors');
                //         $this->db->where('rfd_id =',$params['referred_by']);
                //         $this->db->where('clinic_id =',$params['clinic_id']);
    
                //         $referralDocRec = $this->db->get()->row();
    
                //         $patient_update['referred_by'] = $referralDocRec->doctor_name;
                //     }
                //     $patient_update['created_by'] = $requesterid;
                //     $patient_update['modified_by'] = $requesterid;
                //     $patient_update['status'] = $params['status'];
                //     $patient_update['lat_long'] = $lat_long;
                //     $patient_update['preferred_language'] = $params['preferred_language'];
                ...
                //     $this->Generic_model->updateData('patients', $patient_update, array('patient_id' => $patientChkk->patient_id));

                //          // Get Patient Complete Details
                //          $patientRec = $this->db->select('patient_id,title,first_name,last_name,umr_no, date_of_birth, age, email_id as email, 
                //          payment_status,clinic_id,referred_by_type,referred_by,referral_doctor_id,country,occupation,
                //          mobile,alternate_mobile,age,gender,location,state_id,district_id,preferred_language, address_line, 
                //          pincode, photo, qrcode, created_date_time as registration_date, status')->from('patients')
                //          ->where('patient_id ='.$patientChkk->patient_id)->get()->row(); 
                ...
                //     $result = array('code' => '200', 'message' => 'Patient Details Updated', 'result' => $patientRec, 'requestname' => $requestname);
                ...
                $patient_update['referred_by'] = $fdata->referred_by;
                ...
                if(count($patientRec) > 0) {
                    ...
                    if($patientRec->date_of_birth == '0000-00-00'){
                        $patientRec->date_of_birth == '';
                    }
                    $patientRec->full_name =  ($patientRec->title ? ucfirst($patientRec->title)."." : "").strtoupper(trim($patientRec->first_name)." ".trim($patientRec->last_name));
                    $patientRec->priority = null;
                    $patientRec->patient_condition = null;
                ...
                $config['allowed_types'] = 'jpg|JPG|png|PNG|csv|jpeg|pdf|PDF|doc|docx,';
            ...
            //replacing old image with new image(vikram)
                ...
                $form_master_insert['created_by'] = $fdata->requesterid;
                ...
                // $form_master_insert['add_sticky_image'] = $file1;
                ...
                for($n=0;$n<count($fdata->form_image);$n++)
                {
                    if($fdata->form_image[$n] != '' || $fdata->form_image[$n] != NULL){
                        
                        $image_base64_form = base64_decode($fdata->form_image[$n]); 
                        $file = './uploads/section_images/' . uniqid() . '.jpg'; 
                        file_put_contents($file, $image_base64_form); 
                        
                        $scribblingData['patient_form_id'] = $patient_form_master_id;
                        $scribblingData['scribbling_image'] = $file;
                        $scribblingData['created_by'] = $requesterid;
                        $scribblingData['modified_by'] = $requesterid;
                        $scribblingData['created_date_time'] = $created_date_time;
                        $scribblingData['modified_date_time'] = $modified_datetime;
                        $this->Generic_model->insertData("patient_form_scribbling_images",$scribblingData);
            ...
            else if ($requestname == "save_custom_forms") {              
                //inserting data into patient form table
                // $form_master_insert['form_id'] = $fdata->form_id;
                // $form_master_insert['patient_id'] = $fdata->patient_id;
                // $form_master_insert['doctor_id'] = $fdata->requesterid;
                // $form_master_insert['form_type'] = $fdata->form_type;
                $form_master_insert['appointment_id'] = $fdata->appointment_id;

                $appointmentList = $this->db->select("*")->from("appointments")
                ->where("appointment_id= '".$form_master_insert['appointment_id'] ."'")
                ->get()
                ->row();
                ...
                $form_master_insert['form_id'] = $fdata->form_id;
                $form_master_insert['patient_id'] = $appointmentList->patient_id;
                $form_master_insert['doctor_id'] = $doctor_details->doctor_id;
                $form_master_insert['form_type'] = $fdata->form_type;
                ...
                for($n=0;$n<count($fdata->form_image);$n++)
                {
                    if($fdata->form_image[$n] != '' || $fdata->form_image[$n] != NULL){
                        ...
                        $scribblingData['patient_form_id'] = $patient_form_master_id;
                        $scribblingData['scribbling_image'] = $file;
                        $scribblingData['created_by'] = '11';
                        $scribblingData['modified_by'] = '11';
                        $scribblingData['created_date_time'] = $created_date_time;
                        $scribblingData['modified_date_time'] = $modified_datetime;
                        $this->Generic_model->insertData("patient_form_scribbling_images",$scribblingData);
                ...
                for ($v = 0; $v < count($form_data); $v++) {
                    ...
                    $form_items['option_value'] = $form_data[$v]->option_value;
                    if($form_data[$v]->option_value == "Yes"){
                        $patient_condition['patient_id'] = $appointmentList->patient_id;
                        $patient_condition['patient_condition'] = $form_data[$v]->label_value;
                        $this->Generic_model->insertData('patient_condition', $patient_condition);
                    }
                    $form_items['parent_field_id'] = $form_data[$v]->parent_label_id;
            ...
            else if ($requestname == "clincal_dianosis_image_web") {
                $file2 = "";
                for ($i = 0; $i < count($fdata->form_image); $i++) {
                    $image_base64_form = base64_decode($fdata->form_image[$i]); 
                    $path = './uploads/clinical_diagnosis/';
                    $file =  uniqid() . '.jpg'; 
                    file_put_contents($path.$file, $image_base64_form);
                    // $file1[$i] = $file;
                    $file2 = $file2.$file.",";
                }
                $fileData = substr($file2,0,-1);
                ...
                if(count($check_exists) > 0){
                    ...
                    $data['images'] = $fileData;
                    $this->Generic_model->updateData('patient_clinical_diagnosis', $data, array('patient_clinical_diagnosis_id' => $check_exists->patient_clinical_diagnosis_id));
                    $res['patient_clinical_diagnosis_id'] = $check_exists->patient_clinical_diagnosis_id;
                }else{
                        $scribblingData['appointment_id'] = $fdata->appointment_id;
                        $scribblingData['images'] = $fileData;
                        $scribblingData['patient_id'] = $fdata->patient_id;
                        $scribblingData['doctor_id'] = $fdata->requesterid;
                        $scribblingData['clinic_id'] = $fdata->clinic_id;
                        $scribblingData['created_by'] = $fdata->requesterid;
                        $scribblingData['modified_by'] = $fdata->requesterid;
                        $scribblingData['created_date_time'] = $created_date_time;
                        $scribblingData['modified_date_time'] = $modified_datetime;
                        $res['patient_clinical_diagnosis_id'] = $this->Generic_model->insertDataReturnId('patient_clinical_diagnosis', $scribblingData);
                }

                $result = array('code' => '200', 'message' => 'Clinical Diagnosis Image Saved Successfully', 'result' => $res, 'requestname' => 'clincal_dianosis_image');

            } 
            else if ($requestname == "save_custom_formss") {              
                $form_master_insert['appointment_id'] = $fdata->appointment_id;
                ...
                for($n=0;$n<count($fdata->form_image);$n++)
                {
                    if($fdata->form_image != '' || $fdata->form_image != NULL){
                        ...
                        $file = './uploads/testImages/' . uniqid() . '.jpg'; 
                        file_put_contents($file, $image_base64_form); 
                        
                        $scribblingData['patient_form_id'] = 1;
                        $scribblingData['scribbling_image'] = $file;
                        $scribblingData['created_by'] =2;
                        $scribblingData['modified_by'] = 2;
                        $scribblingData['created_date_time'] = 1;
                        $scribblingData['modified_date_time'] = 1;
                        $this->Generic_model->insertData("patient_form_scribbling_images",$scribblingData);
                ...
                $result = array('code' => '200', 'message' =>$file,'requestname' => 'save_custom_form');
            } 
                ...
                $doctor_update['google_review_link'] = $fdata->google_review_link;
    ...
    public function clinic_master($parameters, $method, $user_id){
        ...
        // if($accessToken==$parameters['accessHeaderToken'])
        // {
        ...
        // else
        // {
        //     $this->response(array('code' => '201', 'message' => 'Your Session Expired ', 'result' => NULL, 'requestname' => $method));
        // }
    	
    }

    // Suggestions for Presenting Symptoms
    public function getPSSuggestions($parameters, $method, $user_id){
        extract($parameters);
        $psInfo = $this->db->query("select * from (select ps.*, count(*) as count from patient_ps_line_items ps,patient_presenting_symptoms pps where pps.patient_presenting_symptoms_id=ps.patient_presenting_symptoms_id and pps.doctor_id='".$doctor_id."' and pps.clinic_id='".$clinic_id."' group by ps.form_id order by count DESC LIMIT 15) as psy order by symptom_data ASC")->result();
        // echo $this->db->last_query();
        if(count($psInfo)>0)
        {
             $i = 0;
             foreach ($psInfo as $value) {
                $data['symptoms'][$i]['symptom'] = $value->symptom_data;
                $data['symptoms'][$i]['form_id'] = $value->form_id;
                $i++;
             }
            //  asort($data['symptoms']);
    ...
    public function getDrugSuggestions($parameters, $method, $user_id){
        extract($parameters);
        $psInfo = $this->db->query("select * from (select ppd.medicine_name,ppd.drug_id,count(*) as count from patient_prescription_drug ppd,patient_prescription pp where ppd.patient_prescription_id=pp.patient_prescription_id and pp.doctor_id='".$doctor_id."' and pp.clinic_id='".$clinic_id."' group by ppd.medicine_name order by count DESC LIMIT 15) as pdr order by medicine_name ASC")->result();
        if(count($psInfo)>0)
        {
            $i = 0;
             foreach ($psInfo as $value) {
                $data['drugs'][$i]['drug_id'] = (int)$value->drug_id;
                $data['drugs'][$i]['drug_name'] = $value->medicine_name;
                $i++;
             }
            //  asort($data['drugs']);
             $this->response(array('code' => '200', 'message' => 'success ', 'result' => $data, 'requestname' => $method)); 
    ...
    // Master for Presenting Symptoms
    public function SymptomMaster($parameters, $method, $user_id){
        extract($parameters);
        $info = $this->db->query("select * from hopi_synonyms")->result();
        if(count($info)>0)
        {
            $i =0;
             foreach ($info as $value) {
                $data['symptoms'][$i]['hopi_synonym_id'] = $value->hopi_synonym_id;
                $data['symptoms'][$i]['form_id'] = $value->form_id;
                $data['symptoms'][$i]['synonym'] = $value->synonym;
                $i++;
             }
             $res = sort($data['symptoms']);
             $this->response(array('code' => '200', 'message' => 'success ', 'result' => $data, 'requestname' => $method)); 
        }
        else
        {
    ...
    //     $docRec = $this->db->select('first_name,last_name,registration_code,doctor_id, mobile, email')->from('doctors')->where('mobile =',$mobile)->or_where('registration_code =',$registration_code)->get()->row();
    ...
    //                 $inchargeData['password'] = md5($inchargePwd);
    ...
    //             $clinicData['clinic_id'] = $clinic_id;
    ...
    //         $userData['clinic_id'] = $clinicData['clinic_id'];
    ...
    //         // If specified then add to Department masters and then update the Department Id
    //         if($other_department != '' || $other_department != NULL)
    //         {
    //         // Add New Department to the Department Masters
    //             $department['department_name'] = $other_department;
    //             $department['status'] = 1;
    //             // $department['review'] = 0;
    //             $department['created_by'] = $department['modified_by'] = $user_id;
    //             $department['created_date_time'] = $department['modified_date_time'] = date('Y-m-d H:i:s');

    //             $doctorData['department_id'] = $this->Generic_model->insertDataReturnId('department',$department);

    //             // unset other department item from the doctorData array
    //             unset($doctorData['other_department']); 
    //         }
    //         else
    //         {
    //             $doctorData['department_id'] = $department_id;
    //             $depInfo = $this->db->select("department_name")->from("department")->where("department_id='".$department_id."'")->get()->row();
    //         }

    //         $rand = str_pad(dechex(rand(0x000000, 0xFFFFFF)), 6, 0, STR_PAD_LEFT);
    //         $colorcode = '#'.$rand;
    //         $doctorData['doctor_id'] = $doctor_id;
    //         $doctorData['salutation'] = 'Dr';
    //         $doctorData['first_name'] = $first_name;
    //         $doctorData['last_name'] = $last_name;
    //         $doctorData['qualification'] = $qualification;
    //         // $doctorData['google_review_link'] = $google_review_link;
    //         $doctorData['email'] = $email;
    //         $doctorData['mobile'] = $mobile;
    //         $doctorData['color_code'] = $colorcode;
    //         $doctorData['registration_code'] = $registration_code;
    //         $doctorData['created_date_time'] = date('Y-m-d H:i:s');
    //         $doctorData['modified_date_time'] = date('Y-m-d H:i:s');
    ...
    //         $mapData['clinic_id'] = $clinicData['clinic_id']; 
    //         $mapData['primary_clinic'] = $primary_clinic;
    ...
    //         $cd_id = $this->Generic_model->insertDataReturnId('clinic_doctor',$mapData);
    //         //Doctor Slots
    //         $created_date_time = date("Y-m-d h:i:s");
    //         $modified_date_time = date("Y-m-d h:i:s");
    //         // for($i=1;$i<=7;$i++)
    //         // {
    //         //     $cwd['clinic_doctor_id'] = $cd_id;
    //         //     $cwd['weekday'] = $i;
    //         //     $cwd['created_date_time'] = $created_date_time;
    //         //     $cwd['modified_date_time'] = $modified_date_time;
    //         //     $cwd_id = $this->Generic_model->insertDataReturnId('clinic_doctor_weekdays', $cwd);
    //         //     for($j=1;$j<=3;$j++)
    //         //     {
    //         //         $cwds['clinic_doctor_weekday_id'] = $cwd_id;
    //         //         if($j == 1)
    //         //         {
    //         //             $cwds['from_time'] = "08:00:00";
    //         //             $cwds['to_time'] = "12:00:00";
    //         //         }
    //         //         elseif($j == 2)
    //         //         {
    //         //             $cwds['from_time'] = "13:00:00";
    //         //             $cwds['to_time'] = "17:00:00";
    //         //         }
    //         //         elseif($j == 3)
    //         //         {
    //         //             $cwds['from_time'] = "18:00:00";
    //         //             $cwds['to_time'] = "21:30:00";
    //         //         }
    //         //         $cwds['created_date_time'] = $created_date_time;
    //         //         $cwds['modified_date_time'] = $modified_date_time;
    //         //         $cwds_id = $this->Generic_model->insertDataReturnId('clinic_doctor_weekday_slots', $cwds);
    //         //     }
    //         // }
    ...
    //         $doctor['docDetails']['full_name'] = trim("Dr.".ucwords($first_name.' '.$last_name));
    //         $doctor['docDetails']['clinic_id'] = $clinicData['clinic_id'];
    //         $doctor['docDetails']['doctor_id'] = $doctor_id;

    //         // Shoot an email
    //         $from = 'UMDAA Heath Care Pvt Ltd';
    //         $to = $email;
    //         $subject = ucwords($clinic_name)." - Admin Credentials";
    //         if($clinic_id == 0)
    //         {
    //             $message = '<html lang="en"><head> <link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"> <div class="container mt-4"> <div class="row"> <div class="col-12" style="text-align:center"><img src="'.base_url(" assets/img/umdaa_clinic_logo.png ").'"></div></div><div class="container"> <div class="row"> <div class="col-md-12"> <p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">'.ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")).'</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="'.base_url().'">'.base_url().'</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p><img src="'.base_url("assets/img/settings.jpg ").'" style="width:80%"> <div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"> <div class="col-md-5 col-12 card" style="width: 100%">'.$forced.'</div><div class="col-md-5 col-12 card" style="width: 100%">'.$phcred.'</div></div><div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"> <div class="col-md-5 col-12 card" style="width: 100%">'.$labcred.'</div><div class="col-md-5 col-12 card" style="width: 100%">'.$nucred.'</div></div></div></div></div></div></body></html>';
    //             // $message = '<html lang="en"><head><link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"><div class="container mt-4"><div class="row"><div class="col-12" style="text-align:center"><img src="'.base_url("assets/img/umdaa_clinic_logo.png").'"></div></div><div class="container"><div class="row"><div class="col-md-12"><p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">'.ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")).'</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="'.base_url().'">'.base_url().'</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p> <img src="'.base_url("assets/img/settings.jpg").'" style="width:80%"><div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"><div class="col-md-5 col-12 card" style="width: 100%">'.$forced.'</div><div class="col-md-5 col-12 card" style="width: 100%">'.$phcred.'</div></div><div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"><div class="col-md-5 col-12 card" style="width: 100%">'.$labcred.'</div><div class="col-md-5 col-12 card" style="width: 100%">'.$nucred.'</div></div><div class="row"><div class="col-md-12"><ul class=" text-danger pl-2" style="font-size: 16px;font-weight:bold"><li>No PrePayments</li><li>First 100 Invoices Are Free</li><li>Billing at the end of the month according to number of SMS & Invoices are used.</li></ul><div class="row"><div class="col-md-5 col-12"><table style="width:50%;font-weight: bold" border="1" cellpadding="10"><tr><th style="text-align: left;">Registration & Consultation</th><td style="text-align: right;">Rs. 2/-</td></tr><tr><th style="text-align: left;">Consultation</th><td style="text-align: right;">Rs. 2/-</td></tr><tr><th style="text-align: left;">Pharmacy</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Procedure</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Lab</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Revisit During Consultation Period</th><td style="text-align: right;">FREE</td></tr><tr><th style="text-align: left;">SMS (120 Characters Per Message)</th><td style="text-align: right;">Rs. 0.25/-</td></tr><tr><th style="text-align: left;">GST (As Applicable)</th><td style="text-align: right;">18%</td></tr></table></div></div><p style="font-weight:bold;">'.ucwords(strtolower("Has To be paid with in 5 working days for uninterrupted services.")).'</p><p style="font-weight:bold;text-align: center" class=" text-center">'.ucwords(strtolower("For Any Queries Please Contact : +91 - 9100 462 015. Thank You.")).'</p></div></div></div></div></div></div></body></html>';    
    //         }
    //         else
    //         {
    //             // $message = '<html lang="en"><head><link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"><div class="container mt-4"><div class="row"><div class="col-12" style="text-align:center"><img src="'.base_url("assets/img/umdaa_clinic_logo.png").'"></div></div><div class="container"><div class="row"><div class="col-md-12"><p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">'.ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")).'</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="'.base_url().'">'.base_url().'</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p> <img src="'.base_url("assets/img/settings.jpg").'" style="width:80%"><div class="row"><div class="col-md-12"><ul class=" text-danger pl-2" style="font-size: 16px;font-weight:bold"><li>No PrePayments</li><li>First 100 Invoices Are Free</li><li>Billing at the end of the month according to number of SMS & Invoices are used.</li></ul><div class="row"><div class="col-md-5 col-12"><table style="width:50%;font-weight: bold" border="1" cellpadding="10"><tr><th style="text-align: left;">Registration & Consultation</th><td style="text-align: right;">Rs. 2/-</td></tr><tr><th style="text-align: left;">Consultation</th><td style="text-align: right;">Rs. 2/-</td></tr><tr><th style="text-align: left;">Pharmacy</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Procedure</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Lab</th><td style="text-align: right;">Rs. 1/-</td></tr><tr><th style="text-align: left;">Revisit During Consultation Period</th><td style="text-align: right;">FREE</td></tr><tr><th style="text-align: left;">SMS (120 Characters Per Message)</th><td style="text-align: right;">Rs. 0.25/-</td></tr><tr><th style="text-align: left;">GST (As Applicable)</th><td style="text-align: right;">18%</td></tr></table></div></div><p style="font-weight:bold;">'.ucwords(strtolower("Has To be paid with in 5 working days for uninterrupted services.")).'</p><p style="font-weight:bold;text-align: center" class=" text-center">'.ucwords(strtolower("For Any Queries Please Contact : +91 - 9100 462 015. Thank You.")).'</p></div></div></div></div></div></div></body></html>';
    //             $message='<html lang="en"><head> <link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"> <div class="container mt-4"> <div class="row"> <div class="col-12" style="text-align:center"><img src="'.base_url(" assets/img/umdaa_clinic_logo.png ").'"></div></div><div class="container"> <div class="row"> <div class="col-md-12"> <p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">'.ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")).'</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="'.base_url().'">'.base_url().'</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p><img src="'.base_url(" assets/img/settings.jpg ").'" style="width:80%"> </div></div></div></div></body></html>';
    //         }
    ...
    //         // //Message To Doctor
    //         // $docSMS = "Dear ".$doctor['full_name']." Welcome to UMDAA Family. Your account has been successfully created. You can log in to our UMDAA App using your registered Mobile Number and Password. Please Check Your Email For Further Information. Please Visit this Link for Further Instructions. http://bit.ly/umdaa1";
    //         // sendsms($mobile, $docSMS);

    //         // //Message TO Administrator
    //         // $adminMobile = "6302758875";
    //         // $adminMessage = "New Doctor Has registered. Name : ".$doctor['full_name'].", Mobile : ".$mobile.", Speciality : ".$depInfo->department_name;
    //         // sendsms($adminMobile, $adminMessage);
    //         $Name = $doctor['docDetails']['full_name'];
    //         $Number = $mobile;
    //         //Message To Doctor
    //         // $docSMS = "Dear ".$doctor['docDetails']['full_name']." Welcome to UMDAA Family. Your account has been successfully created. You can log in to our UMDAA App using your registered Mobile Number and Password. Please Check Your Email For Further Information. Please Visit this Link for Further Instructions. http://bit.ly/umdaa1";
    ...
    //         textlocalSend($mobile, $welcome);

    //         //Message TO Administrator
    //         $adminInfo = $this->db->query("select registration_number from app_version where app_category='Doctors'")->row();
    //         $adminMobile = $adminInfo->registration_number;
    //         // $adminMessage = "New Doctor Has registered. Name : ".$doctor['docDetails']['full_name'].", Mobile : ".$mobile.", Speciality : ".$depInfo->department_name;
            
    //         $Specialty = $depInfo->department_name;
    //         $adminMessage = "New Doctor has Registered. Name: $Name; Mobile No.: $Number; Specialty: $Specialty."; //working
    //         textlocalSend($adminMobile, $adminMessage);
    //         textlocalSend("8790211777", $adminMessage);
    ...
    //         if($docRec->mobile == $mobile){
    //             $message = "Dr. ".$docRec->first_name." ".$docRec->last_name." already registered with this Mobile Number";
    //         }

    //         if($docRec->email == $email){
    //             $message = "Dr. ".$docRec->first_name." ".$docRec->last_name." already registered with this Email ID";
    //         }

    //         if($docRec->registration_code == $registration_code){
    //             $message = "Dr. ".$docRec->first_name." ".$docRec->last_name." already registered with this Registration Code";
    //         }

    //         $this->response(array('code' => '201', 'message' => $message, 'result' => $doctor, 'requestname' => $method));
    //     }       
    ...
    public function doctor_registration($parameters, $method, $user_id)
    {
        ...
        if (count($docRec) == 0) {
            ...
            if ($clinic_id == 0) {
                ...
                $adminPwd = $this->Generic_model->getrandomstring(8);

                /* creating admin*/
                // $adminData['username'] = "";
                $adminData['password'] = md5($adminPwd);
                $adminData['clinic_id'] = $clinicData['clinic_id'];
                ...
                $empcode = strtoupper(substr($clinic_name,0,3)).'-' . date('mY') . $admin_id;
                $adminUname = $empcode;

                $emp1['employee_id'] = $admin_id;
                $emp1['employee_code'] = $empcode;
                $emp1['first_name'] = $adminUname;
                $emp1['clinic_id'] = $clinicData['clinic_id'];
                $emp1['status'] = 1;
                ...
                $admincred = '<p class="font-weight-bold text-danger">Clinic Head Credentials</p><p class="font-weight-bold">Username : ' . $adminUname . '<br>Password : ' . $adminPwd . '</p>';
                //Giving Permissions to Admin
                $clinicRoles = $this->db->query("select * from clinic_roles where belongs_to_role='".$admroleInfo->role_id."'")->row();
                $entities = $this->db->query("select * FROM `package_features` pf,modules m,user_entities ue where m.module_id=pf.module_id and ue.user_entity_id=pf.entity_id and pf.feature_type='Module' and pf.package_id='".$package_id."' and m.role_id='".$admroleInfo->role_id."' and ue.parent_id='0' order by ue.position ASC")->result();
                if(count($entities) > 0){
                    foreach($entities as $ent){
                        $datas['clinic_id'] = $clinicData['clinic_id'];
                        $datas['role_id'] = $admroleInfo->role_id;
                        $datas['clinic_role_id'] = $clinicRoles->clinic_role_id;
                        $datas['entity_id'] = $ent->entity_id;
                        $datas['status'] = 1;
                        $datas['created_by'] = $admin_id;
                        $datas['created_date_time'] = date("Y-m-d H:i:s");
                        $datas['modified_by'] = $admin_id;
                        $datas['modified_date_time'] = date("Y-m-d H:i:s");
                        $this->Generic_model->insertData('clinic_role_permissions', $datas);
                    }
                }
            ...
            // Package Info
            $package['doctor_id'] = $doctor_id;
            $package['clinic_id'] = $clinicData['clinic_id'];
            $package['package_id'] = "5";
            $package['package_subscription_date'] = date("Y-m-d H:i:s");
            $package['package_validity'] = 1;
            $package['package_price'] = "0";
            $package['coupon'] = "Summer";
            $package['coupon_discount'] = "0";
            $package['status'] = 1;
            $package['created_by'] = $doctor_id;
            $package['created_date_time'] = date('Y-m-d H:i:s');
            $package['modified_by'] = $doctor_id;
            $package['modified_date_time'] = date('Y-m-d H:i:s');
            $this->Generic_model->insertData('doctor_packages', $package);
            ...
            $subject = ucwords($clinic_name) . " - Admin Credentials";
            if ($clinic_id == 0) {
                $message = '<html lang="en"><head> <link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"> <div class="container mt-4"> <div class="row"> <div class="col-12" style="text-align:center"><img src="' . base_url(" assets/img/umdaa_clinic_logo.png ") . '"></div></div><div class="container"> <div class="row"> <div class="col-md-12"> <p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">' . ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")) . '</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="' . base_url() . '">' . base_url() . '</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p><img src="' . base_url("assets/img/settings.jpg ") . '" style="width:80%"> <div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"><div class="col-md-5 col-12 card" style="width: 100%">' . $admincred . '</div> <div class="col-md-5 col-12 card" style="width: 100%">' . $forced . '</div><div class="col-md-5 col-12 card" style="width: 100%">' . $phcred . '</div></div><div class="row justify-content-around" style="display: flex;flex-wrap: wrap;font-weight: bold"> <div class="col-md-5 col-12 card" style="width: 100%">' . $labcred . '</div><div class="col-md-5 col-12 card" style="width: 100%">' . $nucred . '</div></div></div></div></div></div></body></html>';
            } else {
                $message = '<html lang="en"><head> <link href="https://fonts.googleapis.com/css?family=Poppins&display=swap" rel="stylesheet"></head><body style="background: #fff !important;font-family: Poppins !important"> <div class="container mt-4"> <div class="row"> <div class="col-12" style="text-align:center"><img src="' . base_url(" assets/img/umdaa_clinic_logo.png ") . '"></div></div><div class="container"> <div class="row"> <div class="col-md-12"> <p class="mt-4 font-weight-bold text-primary p-0" style="font-style: italic;font-weight: bold">' . ucwords(strtolower("Congratulations! Your Account has been successfully created. You can start using your account from your android mobile using your registered email and password. If you Want to use Our Modules Which has been selected by you. Please Click this below Link to Access our Portal.")) . '</p><p style="margin-top: 10px;font-weight: bold">URL : <a href="' . base_url() . '">' . base_url() . '</a></p><p style="margin-top: 10px;font-weight: bold">Once You Login, Please Go To Settings To Manage Your Profile.</p><img src="' . base_url(" assets/img/settings.jpg ") . '" style="width:80%"> </div></div></div></div></body></html>';
            }
            ...
            $ok = $this->mail_send->Content_send_all_mail($from, $to, $subject, '', '', $message);
            $DName = $doctor['docDetails']['full_name'];
            $Number = $mobile;
            //Message To Doctor
            // $docSMS = "Dear ".$doctor['docDetails']['full_name']." Welcome to UMDAA Family. Your account has been successfully created. You can log in to our UMDAA App using your registered Mobile Number and Password. Please Check Your Email For Further Information. Please Visit this Link for Further Instructions. http://bit.ly/umdaa1";
            // $welcome = "Dear $Name,%nWelcome to UMDAA Family. Your account has been created successfully. You can log into our App with your Reg. Mobile/Email. For further information check you Reg. email account. Please visit this link for further instructions https://tx.gl/r/26DIV/#AdvdTrack# ."; //working               
            ...
            $welcome = "Dear $DName,%nWelcome to UMDAA Family. Your account has been created successfully. You can log into our App with your Reg. Mobile/Email. For further information, check your Reg. email account. Please visit this link for further instructions https://tx.gl/r/26DIV/#AdvdTrack#";
            textlocalSend($mobile, $welcome);
            ...
            $Specialty = $depInfo->department_name;
            // $adminMessage = "New Doctor has Registered. Name: $DName; Mobile No.: $Number; Specialty: $Specialty."; //working
            $adminMessage = "New Doctor has Registered. Name: $DName; Mobile No.: $Number; Specialty: $Specialty.%n- UMDAA Health Care";
            textlocalSend("8790211777", $adminMessage);
    ...
    public function askReview($parameters, $method, $user_id){
        extract($parameters);
        $appInfo = $this->Generic_model->getSingleRecord('appointments', array('appointment_id'=>$appointment_id));
        // echo $this->db->last_query();
        if(count($appInfo) > 0){
            // $docInfo = doctorDetails($appInfo->doctor_id);
            // $data['google_review_link'] = $docInfo->google_review_link;
            // $data['doctor_id'] = $appInfo->doctor_id;
            // $data['user_id'] = $appInfo->patient_id;
            // $data['appointment_id'] = $appInfo->appointment_id;
            // $data['status'] = 0;
            // $data['created_by'] = $appInfo->doctor_id;
            // $data['created_date_time'] = date('Y-m-d H:i:s');
            // $this->Generic_model->insertData('google_reviews', $data);


            $userInfo = getPatientDetails($appInfo->patient_id);
            $docInfo = doctorDetails($appInfo->doctor_id);
            $clinicInfo = clinicDetails($appInfo->clinic_id);
            // echo $this->db->last_query();

            $GLink = $docInfo->google_review_link;
            $PName = trim(getPatientName($appInfo->patient_id));
            $DName = getDoctorName($appInfo->doctor_id);
            ...
            $gbusiness = "Thanks $PName for consulting with me at $CName.%nIt would be useful for my practice if you can take some time to post words of appreciation about me in Google Reviews $GLink%nThanks in advance%n$DName - UMDAA";
            ...
            $this->response(array('code' => '200', 'message' => 'Google review request sent', 'requestname' => $method));   
        }
        else{
            $this->response(array('code' => '201', 'message' => 'Error Occured', 'requestname' => $method));   
        }
    }
    ...
    // public function askReview($parameters, $method, $user_id){
    //     extract($parameters);
    //     $this->response(array('code' => '200', 'message' => 'Google review request will be sent', 'requestname' => $method));
    // }

    public function doctor_registration_web($parameters, $method, $user_id){
        ...
        if(count($docRec) == 0){
            ...
            //             $cwds['from_time'] = "08:00:00";
            //             $cwds['to_time'] = "12:00:00";
            //             $cwds['session']="morning";
            //         }
            //         elseif($j == 2)
            //         {
            //             $cwds['from_time'] = "13:00:00";
            //             $cwds['to_time'] = "17:00:00";
            //             $cwds['session']="afternoon";
            //         }
            //         elseif($j == 3)
            //         {
            //             $cwds['from_time'] = "18:00:00";
            //             $cwds['to_time'] = "21:00:00";
            //             $cwds['session']="evening";
            //         }
            //         $cwds['created_date_time'] = $created_date_time;
            //         $cwds['modified_date_time'] = $modified_date_time;
            //         $cwds_id = $this->Generic_model->insertDataReturnId('clinic_doctor_weekday_slots', $cwds);
    ...
    //profile Details
    public function  getProfileDetails($parameters, $method, $user_id){
        extract($parameters);
        $patientInfo = $this->db->query("select * from appointments a,
        patients p where p.patient_id=a.patient_id and a.appointment_id='".$appointment_id."'")->row();
        $data['patient_id'] = $patientInfo->patient_id;
        $data['title'] = $patientInfo->title;
        $data['first_name'] = $patientInfo->first_name;
        $data['last_name'] = $patientInfo->last_name;
        $data['umr_no'] = $patientInfo->umr_no;
        $data['dob'] = $patientInfo->date_of_birth;
        $data['photo'] = base_url("uploads/patients/").$patientInfo->photo;
        $data['age'] = explode(" ", $patientInfo->age)[0];
        $data['gender'] = $patientInfo->gender;
        $data['location'] = $patientInfo->location;
        $data['mobile'] = DataCrypt($patientInfo->mobile,'decrypt');
        $data['alternate_mobile'] = DataCrypt($patientInfo->alternate_mobile,'decrypt');
        $data['email_id'] = DataCrypt($patientInfo->email_id, 'decrypt')?DataCrypt($patientInfo->email_id, 'decrypt'):
        $patientInfo->email_id;
        $this->response(array('code' => '200', 'message' => 'Patient Details', 'result' => $data, 'requestname' => $method));
    }


    //Logout 
    public function logout($parameters, $method, $user_id){
        //$accessToken = getAccessToken($user_id);
      //  if($accessToken==headers['accessHeaderToken'])
        //{
            extract($parameters);
            // $data['secure_accessToken'] = '';
            $this->Generic_model->deleteRecord('users_device_info', array('user_id' => $user_id,'platform'=>'Angular','fcm_id'=>$fcm_id));
            $this->response(array('code' => '200', 'message' => 'Logged Out Successfully', 'result' => NULL, 'requestname' => $method));
        //}
        //else
        //{
          //  $this->response(array('code' => '201', 'message' => 'Your Session Expired', 'result' => NULL, 'requestname' => $method));
...
public function getDocProfileInfo($parameters,$method,$user_id){
    extract($parameters);
    $users_details = $this->db->query("select * from doctors d,users u where u.user_id=d.doctor_id and d.doctor_id='".$doctor_id."'")->row();
    if(count($users_details) > 0){
        $roles = $this->db->query("select * from roles where role_id='".$users_details->role_id."'")->row();
        $profiles = $this->db->query("select * from profiles where profile_id='".$users_details->profile_id."'")->row();
        $clinicInfo = $this->db->query("select * from clinic_doctor where doctor_id='".$users_details->doctor_id."' order by clinic_doctor_id ASC")->row();
        $departments = $this->db->query("select * from department where department_id='".$users_details->department_id."'")->row();
        $clinic = $this->db->query("select * from clinics where clinic_id='".$clinicInfo->clinic_id."'")->row();
        // $param['user_id'] = $users_details->user_id;
        // $param['password'] = $users_details->password;
        // $param['username'] = getDoctorName($users_details->doctor_id);
        // $param['clinic_id'] = $clinicInfo->clinic_id;
        // $param['department_id'] = $users_details->department_id;
        // $param['name'] = getDoctorName($users_details->doctor_id);
        // $param['gender'] = $users_details->gender;
        // $param['email'] = $users_details->email_id;
        // $param['user_type'] = $users_details->user_type;
        // $param['role_id'] = $users_details->role_id;
        // $param['role_name'] = $roles->role_name;
        // $param['profile_id'] = $users_details->profile_id;
        // $param['profile_name'] = $profiles->profile_name;
        // $param['status'] = $users_details->status;
        
        $param['clinic_id'] = $clinicInfo->clinic_id;
        $param['clinic_name'] = $clinic->clinic_name;
        $param['department_id'] = $users_details->department_id;
        $param['department_name'] = $departments->department_name;
        $param['doctor_id'] = $doctor_id;
        $param['image'] = base_url() . "uploads/doctors/".$users_details->profile_image;
        $param['first_name'] = $users_details->first_name;
        $param['email'] = $users_details->email_id;
        $param['last_name'] = $users_details->last_name;
        $param['mobile'] = $users_details->mobile;
        $param['qualification'] = $users_details->qualification;
        $param['registration_code'] = $users_details->registration_code;
        $param['email'] = $users_details->email_id;
        $param['gender'] = $users_details->gender;
        $this->response(array('code'=>'200','message'=>'Profile Info','result'=>$param,'method'=>$method));
    }
    else{
        $param = "No doctors Found";
        $this->response(array('code'=>'200','message'=>'Profile Info','result'=>$param,'method'=>$method));
    }
}
...
//         $fcmCheck = $this->db->query("select * from users_device_info where user_id='".$result->user_id."' and fcm_id='".$parameters['fcmId']."'")->row();
//         if(count($fcmCheck) > 0){
//             $fcmData['platform'] = "Angular";
//             $fcmData['last_login_time'] = date('Y-m-d H:i:s');
//             $this->Generic_model->updateData('users_device_info', $fcmData, array('users_device_info_id'=>$fcmCheck->users_device_info_id));
//         }
//         else{
//             if($parameters['fcmId'] != ""){
//                 $fcm_id['fcm_id'] = $parameters['fcmId'];
//                 // $fcm_id['device_id'] = $parameters['deviceid'];    
//                 $fcm_id['platform'] = "Angular";
//                 $fcm_id['user_id'] = $result->user_id;
//                 $fcm_id['last_login_time'] = date('Y-m-d H:i:s');
//                 $fcm_id['created_date_time'] = date('Y-m-d H:i:s');
//                 $this->Generic_model->insertData('users_device_info', $fcm_id);
//             }
            
//         }
//         $param['fcmQuery'] = $this->db->last_query();
//         // $fcm_id['secure_accessToken'] = $accessToken;
...
//             $param['clinic_id'] = $clinics_list[0]->clinic_id;
...
//                 // Get details
//                 $doctor_details =
//                  $this->db->select('Doc.doctor_id, Doc.first_name, Doc.last_name,
//                   C.clinic_id, C.clinic_name, C.location, 
//                   Doc.department_id, Doc.email, Doc.mobile, 
//                   Doc.qualification, Doc.registration_code,Doc.profile_image,Doc.google_review_link')
//                  ->from('doctors Doc')->join('clinic_doctor CD','Doc.doctor_id = CD.doctor_id')
//                  ->join('clinics C','CD.clinic_id = C.clinic_id')
//                  ->where('Doc.doctor_id =',$param['user_id'])
//                  ->get()
//                  ->row();
//                 $deptInfo = $this->db->query("select department_name from department where department_id='".$doctor_details->department_id."'")->row();
//                 $param['doctor_details'] = $doctor_details;
//                 $param['doctor_details']->department_name = $deptInfo->department_name;
//                 $param['doctor_details']->image =  base_url() . "uploads/doctors/".$doctor_details->profile_image;
//             }
...
//        $result = $this->db->select("*")->from("users")->where("(username = '" . $email . "' or email_id = '" . $email . "' or mobile ='" . $email . "' ) and password ='" . $password . "' and (role_id='4' or role_id='6')")->get()->row();
...
//                if($users_details->role_id == "6"){
//                    $packages_info = $this->db->query("select * from doctor_packages where clinic_id='" . $users_details->clinic_id . "'")->row(); 
//                }
//                else{
//                    $packages_info = $this->db->query("select * from doctor_packages where doctor_id='" . $users_details->user_id . "'")->row();
//                }

//                $funAccess = $this->db->query("select * from package_features where package_id='" . $packages_info->package_id . "' and feature_type='Functionality'")->result();
//                if(count($funAccess) > 0){
//                    $i = 0;
//                    foreach($funAccess as $value){
//                        $funInfo = FunctionalityInfo($value->functionality_id);
//                        $param['functionalities'][$i] = $funInfo->feature_name;
//                        $i++;
//                    }
//                }
//                $param['query'] = "select * from module_entities me,modules m where me.module_id=m.module_id and m.role_id='".$users_details->role_id."'";
//                // $access = $this->db->query("select * from package_features where package_id='" . $packages_info->package_id . "' and feature_type='Module' group by entity_id")->result();
//                if($users_details->role_id == "6"){
//                    $empInfo = $this->db->query("select * from employees where employee_id='".$users_details->user_id."'")->row();
//                    if($empInfo->assigned_roles == ""){
//                        $entities = $this->db->query("select * from module_entities me,modules m where me.module_id=m.module_id and m.role_id='".$users_details->role_id."'")->result();
//                    }
//                    else{
//                        $entities = $this->db->query("select * FROM `package_features` pf,modules m,user_entities ue where m.module_id=pf.module_id and ue.user_entity_id=pf.entity_id and pf.feature_type='Module' and pf.package_id='".$packages_info->package_id."' and m.role_id='".$users_details->role_id."' and ue.parent_id='0' order by ue.position ASC")->result();
//                    }
//                }
//                else{
//                    $entities = $this->db->query("select * from module_entities me,modules m where me.module_id=m.module_id and m.role_id='".$users_details->role_id."'")->result();
...
//                        $entityInfo = userEntityInfo($value->entity_id);
//                        if($entityInfo->parent_id == 0){
//                            if($entityInfo->category == "Ionic"){
//                                $param['Ionic'][$i]['title'] = $entityInfo->user_entity_name;
//                                $param['Ionic'][$i]['url'] = $entityInfo->entity_url;
//                                $param['Ionic'][$i]['icon'] = substr($entityInfo->user_entity_name, 0, 1);
//                                $param['Ionic'][$i]['access'] = checkAccess($value->entity_id, $value->module_id, $packages_info->package_id);
//                                $i++;
//                            }
//                            if($entityInfo->category == "Angular"){
//                                $param['Angular'][$j]['name'] = $entityInfo->user_entity_name;
//                                $param['Angular'][$j]['link'] = $entityInfo->entity_url;
//                                $param['Angular'][$j]['icon'] = $entityInfo->entity_icon;
//                                $param['Angular'][$j]['Activeicon'] = $entityInfo->active_icon;
//                                $param['Angular'][$k]['access'] = checkAccess($value->entity_id, $value->module_id, $packages_info->package_id);
//                                $j++;
//                            }
//                        }
//                        else{
//                            $parentInfo = userEntityInfo($entityInfo->parent_id);
//                            $param['subpages'][$k]['name'] = $entityInfo->user_entity_name;
//                            $param['subpages'][$k]['parent'] = $parentInfo->user_entity_name;
//                            $param['subpages'][$k]['access'] = checkAccess($value->entity_id, $value->module_id, $packages_info->package_id);
//                            $k++;
//                        }
...
//                        $param['clinics'][$c]['clinic_id'] = $clinic_val->clinic_id;
//                        $param['clinics'][$c]['clinic_name'] = $clinic_val->clinic_name;
//                        if ($clinic_val->clinic_logo == "" || $clinic_val->clinic_logo == null) {
//                            $clinic_logo = "null";
//                        } else {
//                            $clinic_logo =  base_url("/uploads/clinic_logos/" . $clinic_val->clinic_logo . "");
//                        }
//                        if ($clinic_val->clinic_emblem == "" || $clinic_val->clinic_emblem == null) {
//                            $clinic_emblem = "";
//                        } else {
//                            $clinic_emblem =  base_url("/uploads/clinic_logos/" . $clinic_val->clinic_emblem . "");
//                        }
                ...
                foreach ($entities as $value) {
                    ...
                    if ($entityInfo->parent_id == 0) {
                        if ($entityInfo->category == "Ionic") {
                            $param['Ionic'][$i]['title'] = $entityInfo->user_entity_name;
                            $param['Ionic'][$i]['url'] = $entityInfo->entity_url;
                            $param['Ionic'][$i]['icon'] = $entityInfo->entity_icon;
                            $param['Ionic'][$i]['access'] = checkAccess($value->entity_id, $value->module_id, $packages_info->package_id);
                            $i++;
                        }
                        if ($entityInfo->category == "Angular") {
...
public function forgot_password_otp($parameters, $method, $user_id) {
    // echo $check_user = "select * from users where email_id='".$parameters['email_id']."' or mobile='".$parameters['email_id']."'";
    // $check_user = $this->db->query("select * from users where email_id='".$parameters['email_id']."' or mobile='".$parameters['email_id']."'")->row();
    ...
    if (count($check_user) > 0) {
        ...
        $random_otp = $Rotp = mt_rand(100000, 999999);
        $mobile = $check_user->mobile;
        // $message = "OTP is " . $random_otp . " to reset your password.";
        $fogOtp = "Dear User,%nOTP to reset your password at UMDAA is $Rotp and is valid for 2 mins.";
        textlocalSend($mobile, $fogOtp);
        $result['otp'] = $random_otp;
        $result['mobile'] = $check_user->mobile;
...
public function checkDocDigiSign($parameters, $method, $user_id){
    extract($parameters);
    if(isset($doctor_id)){
        $check = $this->Generic_model->getSingleRecord('digital_signatures', array('user_id'=>$doctor_id));
        if(count($check) > 0){
            $data['docSign'][0]['signStatus'] = 1;
            $this->response(array('code'=>'201','message'=>'Signature Already Exists','result'=>$data));
        }
        else{
            $data['docSign'][0]['signStatus'] = 0;
            $this->response(array('code'=>'200','message'=>'Can Enter Signature','result'=>$data));
        }
    }
    else{
        $data['docSign'] =[];
        $this->response(array('code'=>'202','message'=>'Error Occured','result'=>$data));
    }
}
...
public function presenting_symptoms_list($parameters, $method, $user_id) {
    ...
    if (count($presenting_symptoms) > 0) {
        ...
        foreach ($presenting_symptoms as $psl) {
            ...
            $param['presenting_symptoms_list'][$ps]['form_id'] = $psl->form_id;
...
public function presenting_symptoms_update($parameters, $method, $user_id) {
    ...
    //updating present symptoms for angular
    public function presenting_symptoms_updatee($parameters, $method, $user_id) {
        $param['form_id'] = $parameters['form_id'];
        $param['span_type'] = $parameters['span_type'];
        $param['symptom_data'] = $parameters['symptom_data'];
        $param['time_span'] = $parameters['time_span'];
        $ok = $this->Generic_model->updateData('patient_ps_line_items', $param, array('pps_line_item_id' => $parameters['pps_line_item_id']));
...
public function state_master($parameters, $method, $user_id) {
    $clinic_id = $parameters['clinic_id'];
    $role_id = $parameters['role_id'];
    if ($role_id == 1) {
            //$doctors=$this->db->query("select * from doctors ");
    ...
    // $patient_val['path'] = base_url() . "uploads/masters_json/masters.zip";
    $this->response(array('code' => '200', 'message' => 'success ', 'result' => $states, 'requestname' => $method));
...
public function previous_documents($parameters, $method, $user_id) {
    ...
    // $pc_infoo = $this->db->select("*")
    // ->from("doctor_patient_documents p")
    // ->join("citizen_records c","p.document_id=c.citizen_record_id")
    // ->where("p.patient_id='287'")    
    // ->get()
    // ->result();
    ...
    //     foreach ($pc_infoo as $pav) {
    //         $param['previous_documents'][$pvv]['previous_document_id'] = $pav->citizen_record_id;
    //         // $param['previous_documents'][$pvv]['appointment_id'] = $pav->appointment_id;
    //         $param['previous_documents'][$pvv]['current_date'] = $pav->cur_date;
    //         $param['previous_documents'][$pvv]['patient_id'] = $pav->patient_id;
    //         $param['previous_documents'][$pvv]['report_date'] = $pav->report_date;
    //         $param['previous_documents'][$pvv]['document_type'] = $pav->document_type;

    //         $param['previous_documents'][$pvv]['description'] = $pav->description;
    //         $images = trim($pav->images, ",");
    //         $picture_explode = explode(",", $images);
    //         for ($k = 0; $k < count($picture_explode); $k++) {
    //             $param['previous_documents'][$pvv]['images'][$k]['image'] = base_url('uploads/my_records/' . trim($picture_explode[$k]));
    //         }
    //         $pvv++;
    //     }
    // }
    ...
    if (count($previous_documents) > 0) {
        ...
        foreach ($previous_documents as $pav) {
            ...
            if(substr(implode($picture_explode),-3) == "pdf")
            {
                  $param['previous_documents'][$pv]['type']="pdf";
                  $param['previous_documents'][$pv]['images'][$k]['image'] = base_url('uploads/previous_documents/' . trim($picture_explode[$k]));
            }
            else if(substr(implode($picture_explode),-3) == "doc")
            {
                $param['previous_documents'][$pv]['type']="doc";
                $param['previous_documents'][$pv]['images'][$k]['image'] = base_url('uploads/previous_documents/' . trim($picture_explode[$k]));
            }
            else{
                $param['previous_documents'][$pv]['type']="images";
                $param['previous_documents'][$pv]['images'][$k]['image'] = base_url('uploads/previous_documents/' . trim($picture_explode[$k]));
            }
            }
            $pv++;
...
public function followup_template_list($parameters, $method, $user_id) {
    ...
    if (count($check_template) > 0 && $check_template->followup_id != NULL) {
        ...
        foreach ($template_info as $key => $value) {
            ...
            $data["follow_up_rows"][$k]['row_data'][$j]['title'] = date("d/m/Y", strtotime($value->appointment_date))." ".date("H:i:s", strtotime($value->appointment_time_slot));
        ...
        // echo "<pre>";print_r($app_id);echo "</pre>";
        foreach ($template_info as $key => $value) {
            $test[] = $this->db->select("*")->from("patient_vital_sign")->where("vital_sign_recording_date_time = (select max(vital_sign_recording_date_time) From patient_vital_sign where appointment_id = '" . $value->appointment_id . "')")->order_by("vital_sign_recording_date_time","DESC")->get()->result();
            // echo $this->db->last_query();
        ...
        foreach ($vitals_details as $key => $value) {
            ...
            foreach ($app_dates as $key2 => $value2) {
                $data["follow_up_rows"][$c]['row_id'] = "$c";
                $data["follow_up_rows"][$c]['row_data'][$rc]['appointment_id'] = $app_id[$i];
                $data["follow_up_rows"][$c]['row_data'][$rc]['column_id'] = "$rc";
                $data["follow_up_rows"][$c]['row_data'][$rc]['title'] = getvitalsbydate($value2, $value->vital_sign, $app_id[$i]);
        ...
        foreach ($pc_info as $key => $pvalue) {
            ...
            foreach ($app_id as $key => $dvalue) {
             ...
             $template_info12345 = $this->db->select("*")
             ->from("patient_followup_line_items")
             ->where("parameter_id='" .  $pvalue->parameter_id . "' and appointment_id ='" . $dvalue . "' ")
             ->get()
             ->row();
                ...
                $data["follow_up_rows"][$pr]['row_data'][$pcr]['title'] = $template_info12345->parameter_value;
        ...
        foreach ($pl_info as $key => $plvalue) {
            ...
            foreach ($app_id as $key => $dvalue) {
                ...
                $template_info123456 = $this->db->select("*")
                ->from("patient_followup_line_items")
                ->where("parameter_id='" .  $plvalue->parameter_id . "' and appointment_id ='" . $dvalue . "' ")
                ->get()
                ->row();
                ...
                $data["follow_up_rows"][$pl]['row_data'][$pcl]['title'] = $template_info123456->parameter_value;
        ...
        foreach ($template_info as $key => $vinfo) {
            ...
            $pc_infoo = $this->db->select("*")
            ->from("parameters p")
            ->join("patient_followup_line_items pfli","p.parameter_id=pfli.parameter_id")
            ->where("pfli.appointment_id='".$appointment_id."' and parameter_type='Clinical' and p.parameter_id=pfli.parameter_id")    
            ->get()
            ->result();

            $pl_info = $this->db->select("*")->from("parameters")->where("parameter_type='Lab' and parameter_id IN (" . $pci->parameter_id . ")")->get()->result();
            $pl_infoo = $this->db->select("*")
            ->from("parameters p")
           ->join("patient_followup_line_items pfli","p.parameter_id=pfli.parameter_id")
            ->where("pfli.appointment_id='".$appointment_id."' and parameter_type='Lab' and p.parameter_id=pfli.parameter_id")    
            ->get()
            ->result();
            $pc = 0;
            foreach ($pc_infoo as $key => $value) {
                ...
                $data["visits_info"][$s]['Clinical_params'][$pc]['value'] = $value->parameter_value;
                $pc++;
            }
            $pl = 0;
            foreach ($pl_infoo as $key => $value2) {
                ...
                $data["visits_info"][$s]['Lab_parameters'][$pl]['value'] = $value2->parameter_value;
                $pl++;
            }
...
public function parameter_insert($parameters, $method, $user_id) {
    ...
    //followup according to ddepartments
    public function followup_dept($parameters,$method,$user_id){
        extract($parameters);
        $appInfo = $this->db->query("select d.department_id from appointments a,doctors d where d.doctor_id=a.doctor_id and a.appointment_id='".$appointment_id."'")->row();
        if(count($appInfo)>0)
        {
            $fwDeptInfo = $this->db->query("select * from followup_department fd,followup fu where fu.followup_id=fd.followup_id and fd.department_id='".$appInfo->department_id."'")->result();
            // $this->response($fwDeptInfo);
            if(count($fwDeptInfo)>0)
            {
                foreach($fwDeptInfo as $value)
                {
                    $data['followup_name'] = $value->followup_name;
                    $data['followup_id'] = $value->followup_id;
                }

                $this->response(array('code' => '200', 'message' => 'Follow Up', 'result' => $data, 'requestname' => $method));
            }
            else
            {
                $data = [];
                $this->response(array('code' => '200', 'message' => 'No Followup Found', 'result' => $data, 'requestname' => $method));
        ...
        // $this->response(array('code'=>$this->db->last_query()));
    }
        ...
        if(file_exists("./uploads/drugs_master/" . $parameters['clinic_id'] . "_masters.zip")){
            ...
            if (count($investigation_details) > 0) {
                foreach ($investigation_details as $investigation) {
                    $inv['investigation'][$in]['investigation_id'] = (int)$investigation->investigation_id;
                    ...
                    $inv['investigation'][$in]['category'] = $investigation->template_type;
            ...
            $clinicId = $parameters['clinic_id'];
            $clinic_inv_details = $this->db->select("*")->from("clinic_investigations ci")->join("investigations i","ci.investigation_id = i.investigation_id")->where('ci.clinic_id',$clinicId)->get()->result();
            $in = 0;
            if (count($clinic_inv_details) > 0) {
                foreach ($clinic_inv_details as $c_investigation) {
                    $clinic_inv['investigation'][$in]['clinic_investigation_id'] = (int)$c_investigation->clinic_investigation_id;
                    $clinic_inv['investigation'][$in]['investigation_id'] = (int)$c_investigation->investigation_id;
                    ...
                    $clinic_inv['investigation'][$in]['category'] = $c_investigation->template_type;
            ...
            $inward = $this->db->select("clinic_id, batch_no, drug_id, sum(quantity) as quantity_supplied, expiry_date, status")->from("clinic_pharmacy_inventory_inward")->where("clinic_id = '" . $parameters['clinic_id'] . "' AND status = 1 AND archieve=0")->group_by("drug_id")->get()->result();

            $i = 0;
            if (count($inward) > 0) {
                foreach ($inward as $pharmacy_inventory) {

                    $outward = $this->db->select("batch_no, drug_id, sum(quantity) as quantity_sold")->from("clinic_pharmacy_inventory_outward")->where("batch_no = '" . $pharmacy_inventory->batch_no . "' AND drug_id = '" . $pharmacy_inventory->drug_id . "' AND clinic_id = '" . $parameters['clinic_id'] . "'")->group_by("drug_id")->get()->row();
    ...
    //Get the Available Quantity of drugs in pharmacy
    public function getPharmacyStock($parameters, $method, $user_id)
    {
        $inward = $this->db->select("clinic_id, batch_no, drug_id, sum(quantity) as quantity_supplied, expiry_date, status")->from("clinic_pharmacy_inventory_inward")->where("clinic_id = '" . $parameters['clinic_id'] . "' AND status = 1 AND archieve=0")->group_by("drug_id")->get()->result();
        // echo $this->db->last_query();
        $i = 0;
        if (count($inward) > 0) {
            foreach ($inward as $pharmacy_inventory) {
                ...
                $para['p_inventory'][$i] = $pharmacy_inventory;
                $i++;
            }
        } else {
            $para['p_inventory'] = array();
        }
...
public function patient_registrations($parameters, $method, $user_id) {
    ...
    $user_reg['email_id'] = DataCrypt($parameters['email_id'], 'encrypt');
    $user_reg['mobile'] = DataCrypt($parameters['mobile'], 'encrypt');
    ...
    $subject = "New Patient Credentials";
    $header = '<html>
    <body>
...
public function getNewDrugs($parameters, $method, $user_id){
    $clinic_id = $this->session->userdata("clinic_id");
    extract($parameters);
    $res = $this->db->query("select * from drug where created_date_time > '".$lastSyncDate."'")->result();
    if(sizeof($res)>0)
    {
        $i = 0;
        foreach ($res as $value) {
            ...
            unset($value->modified_date_time);
            $data['drug'][$i] = $value;
            $i++;
        }
        $this->response(array('code' => '200', 'message' => 'success ', 'result' => $data, 'requestname' => $method));
    ...
    {
        $this->response(array('code' => '200', 'message' => 'No New Drugs Found ', 'result' => null, 'requestname' => $method));
    }
...
public function patientsList($parameters, $method, $user_id) {
   ...
   //list of patients w.r.t to doctor
   public function patients_doctor_List($parameters, $method, $user_id) {

    extract($parameters);
    $doctor_id = $parameters['doctor_id'];
    // $this->response(array('code' => '200', 'message' => 'NO data Found', 'result' => $parameters), 200);
    if (isset($parameters['mobile'])) {
        ...
        // SELECT * FROM patients t1 INNER JOIN appointments t2 ON t1.patient_id = t2.patient_id where t2.doctor_id="11" and t1.clinic_id="3"
        $patients = $this->db->select("*")
        ->from("patients p")
        ->join("appointments ap","ap.patient_id = p.patient_id")
        ->join("states s","p.state_id=s.state_id","left")
        ->join("districts d","p.district_id=d.district_id","left")
       
         ->where( "p.clinic_id='" . $clinic_id . "' and ap.doctor_id='" . $doctor_id . "'")
        // ->where("ap.doctor_id='11'")
        ->order_by("p.patient_id","desc")
        ->get()
        ->result();
...
public function immunization_status($department = NULL, $patient_id = NULL){
    ...
    public function appointments($parameters, $method, $user_id)
    {
        ...
        if (count($patients) > 0) {
            ...
            foreach ($patients as $patient) {
                ...
                $patient_val['appointment_list'][$i]['clinic_id'] = $patient->clinicID;
                ...
                if ($patient->appointment_payment_status == "0") {
                    $patient_val['appointment_list'][$i]['apptype'] = "Provisional";
                } elseif ($patient->appointment_payment_status == "1") {
                    $patient_val['appointment_list'][$i]['apptype'] = "Definite";
                }

                // if($patient->mobile != '' || $patient->mobile != NULL)
                $patient_val['appointment_list'][$i]['mobile'] = DataCrypt($patient->mobile, 'decrypt');
                // else
                // $patient_val['appointment_list'][$i]['mobile'] = null;

                // if($patient->alternate_mobile != '' || $patient->alternate_mobile != NULL)
                $patient_val['appointment_list'][$i]['alternate_mobile'] = DataCrypt($patient->alternate_mobile, 'decrypt');
                // else
                //     $patient_val['appointment_list'][$i]['alternate_mobile'] = null;
                $review_days = $this->db->select("*")
                    ->from("clinic_doctor cd")
                    ->where("cd.doctor_id ='" . $patient->doctor_id . "' and cd.clinic_id='" . $patient->clinic_id . "'")
                    ->get()->row();
                // $date=date_create($patient->appointment_date);
                // $previous_review_date = date_sub($date,date_interval_create_from_date_string($review_days->review_days.' days'));
                $appointment_type = "";
                // echo 

                if ($patient->appointment_type == 'Follow-up' || $patient->appointment_type == NULL) {
                    $appointments = $this->db->select("*")
                        ->from("appointments ap")
                        ->where("ap.doctor_id ='" . $patient->doctor_id . "' and ap.clinic_id='" . $patient->clinic_id . "'
                        and ap.patient_id='" . $patient->patient_id . "'")
                        ->order_by("ap.appointment_id", "desc")
                        ->get()->result();
                        // echo $this->db->last_query();
                        // $patient_val['appointment_list'][$i]['query'] = $this->db->last_query();
                    if(count($appointments) > 1){
                        $review_days = $review_days->review_days > 0 ? $review_days->review_days : '7';
                        $date1 = date_create($patient->appointment_date);
                        $date2 = date_create($appointments[1]->appointment_date);
                        $diff = date_diff($date1, $date2);
                        $diff_days = $diff->format("%a");
                        if ($diff_days >  $review_days) {
                            $appointment_type = 'R';
                        } else {
                            $appointment_type = 'F';
                        }
                ...
                elseif($patient->appointment_type == "New"){
                    $appointment_type = "N";
                }
                // else {
                //     $appInfo = $this->db->query("select count(*) as count from appointments where clinic_id='" . $patient->clinic_id . "' and doctor_id='" . $patient->doctor_id . "' and patient_id='" . $patient->patient_id . "'")->row();
                //     $patient_val['appointment_list'][$i]['query'] = $this->db->last_query();
                //     if ($appInfo->count > 1) {
                //         $appointment_type = "F";
                //     } else {
                ...
                $patient_val['appointment_list'][$i]['slot_type'] = $patient->slot_type;
                $patient_val['appointment_list'][$i]['web_appointment_date'] = date("d-m-Y", strtotime($patient->appointment_date));
                ...
                // $patient_val['appointment_list'][$i]['appointment_type'] = $patient->appointment_type == 'New' ? strtoupper(substr($patient->appointment_type, 0, 1)) : $appointment_type;
                $patient_val['appointment_list'][$i]['appointment_type'] = $appointment_type;
                ...
                $patient_val['appointment_list'][$i]['web_registartion_date'] = date("d-m-Y", strtotime($patient->registration_date));
                ...
                $patient_val['appointment_list'][$i]['rmp_id'] = $patient->rmp_id;
                $patient_val['appointment_list'][$i]['rmp_id_status'] = $condition_rmp;
            ...
            if (count($patient_val) > 0) {
                $patient_val = $patient_val;
            } else {
                $patient_val['appointment_list'] = [];
            }
            $this->response(array('code' => '200', 'message' => $latest_app_date, 'result' => $patient_val, 'requestname' => $method));
        } else {
            $patient_val['appointment_list'] = [];
            $this->response(array('code' => '200', 'message' => 'NO data Found', 'result' => $patient_val), 200);
        }
    }
    ...
    */ 
        public function search_patient($parameters, $method, $user_id) {
            ...
            ->from('patients')
            ->group_start()
            ->where('mobile =',DataCrypt($srchParam,'encrypt'))
            ->or_where('alternate_mobile =',DataCrypt($srchParam,'encrypt'))
            ->or_where('umr_no =',$srchParam)
            ->or_like('mobile',DataCrypt($srchParam,'encrypt'), 'both')
            ->or_like('alternate_mobile',DataCrypt($srchParam,'encrypt'), 'both')
            ->or_like('first_name',$srchParam, 'both')
            ->or_like('last_name',$srchParam, 'both')
            ->group_end()
            ...
            if(count($patientsRec) > 0){
                ...
                foreach($patientsRec as $patient) {
                    ...
                    $patient['appointment_list'][$x] = $patient;
                    $patient['mobile'] = DataCrypt($patient['mobile'],'decrypt');
                    $patient['alternate_mobile'] = DataCrypt($patient['alternate_mobile'],'decrypt');    
                    $patient['email_id'] = DataCrypt($patient['email_id'],'decrypt');    
                    
                    $x++;

                    $patientArray['appointment_list'][] = $patient; 
                }
                $code = "200";
                $message = "success";
            }else{
            ...
            $this->response(array('code' => $code, 'message' => $message, 'result' => $patientArray, 'requestname' => $method));
    ...
    */ 
    public function search_patient_appointment($parameters, $method, $user_id)
    {
        ...
        // get patient and related patients with mobile no.
        // get patient details with UMR no.
        $patientsRec = $this->db->select('patient_id, title, 
            first_name, last_name, umr_no, gender, clinic_id,
             referred_by, referred_by_type, date_of_birth, age,
              occupation, country, mobile, alternate_mobile,
               email_id as email, photo, qrcode, allergy, 
               address_line as address, location, state_id, 
               district_id, pincode, preferred_language, guardian_id, 
               created_date_time as registartion_date')
            ->from('patients')
            ->group_start()
            ->where('mobile =', DataCrypt($srchParam, 'encrypt'))
            ->or_where('alternate_mobile =', DataCrypt($srchParam, 'encrypt'))
            ->or_where('umr_no =', $srchParam)
            ->or_like('mobile', DataCrypt($srchParam, 'encrypt'))
            ->or_like('alternate_mobile', DataCrypt($srchParam, 'encrypt'))
            ->or_like('first_name', $srchParam, 'both')
            ->or_like('last_name', $srchParam, 'both')
        ...
        if (count($patientsRec) > 0) {
            ...
            $x = 0;
            foreach ($patientsRec as $patient) {
                $docPatientCheck = $this->Generic_model->getAllRecords('doctor_patient', array('doctor_id' => $user_id, 'patient_id' => $patient['patient_id']));
                // echo $this->db->last_query();
                if (count($docPatientCheck) <= 0) {
                    continue;
                }
                ...
                $this->db->select('A.appointment_id,SUBSTR(A.appointment_type,1,1) as appointment_type,A.doctor_id, CONCAT("Dr.",Doc.first_name, " ", Doc.last_name) as doctor_name, A.appointment_date,  A.appointment_time_slot as appointment_time, A.check_in_time, A.check_out_time, A.priority, A.consultation_start_time, A.consultation_end_time, A.course_in_hospital as doctor_comments, IF(A.rmp_id > 0,1,0) as rmp_id_status,A.rmp_id, A.status, Doc.department_id, Doc.color_code, Dep.department_name as department');
                $this->db->from('appointments A');
                ...
                // echo $this->db->last_query();
                // $patient['web_appointment_date'] = date("d-m-Y", strtotime($appointment_date));
                $patient['qb_user_id'] = '';
                ...
                // echo "s".$patient->appointment_type = strtoupper(substr($appointment[0]->appointment_type, 0, 1));
                // if(count($patient) > 0){
    ...
    */
    ...
    //Get Individual Doctor Slot Timings
    public function getIndDoc($parameters, $method, $user_id) {
        ...
        if($slot == 'video call')
        {
            ...
            // get all doctors belongs to the clinic '$clinic_id'
            $doctorInfo = $this->db->select("cd.*,d.*,de.*,cd.clinic_id")->from("clinic_doctor cd")->join("doctors d","cd.doctor_id=d.doctor_id")->join("department de","d.department_id=de.department_id")->where("cd.clinic_id ='" . $clinic_id . "' and d.doctor_id='".$doctor_id."'")->group_by("cd.doctor_id")->order_by("d.doctor_id","asc")->get()->row();
            ...
            if (count($doctorInfo) > 0) {
                    ...
                    $clinics_list = $this->db->select("cd.*,c.*,cd.clinic_id")->from("clinic_doctor cd")->join("clinics c","cd.clinic_id=c.clinic_id")->where("cd.doctor_id ='" . $doctorInfo->doctor_id . "' and c.clinic_id='".$clinic_id."'")->group_by("cd.clinic_id")->order_by("c.clinic_id","asc")->get()->row();
    
                    if (count($clinics_list) > 0) {
                            $cdw = 0;
                            $param['doctor']['clinics']['clinic_id'] = $clinics_list->clinic_id;
                            $param['doctor']['clinics']['clinic_name'] = $clinics_list->clinic_name;
                            if($clinics_list->online_consulting_time=="")
                                $param['doctor']['clinics']['consultation_time'] = "10";
                            else
                                $param['doctor']['clinics']['consultation_time'] = $clinics_list->online_consulting_time;
                            $param['doctor']['clinics']['consulting_fee'] = $clinics_list->online_consulting_fee;
                            $param['doctor']['clinics']['registration_fee'] = $clinics_list->registration_fee;
                            $param['doctor']['clinics']['pharmacy_discount'] = $clinics_list->pharmacy_discount;
                            $param['doctor']['clinics']['lab_discount'] = $clinics_list->lab_discount;
    
                            $week_day_list = $this->db->select("clinic_doctor_weekday_id,weekday")->from("clinic_doctor_weekdays")->where("clinic_doctor_id ='" . $clinics_list->clinic_doctor_id . "' and slot='".$slot."'")->group_by("weekday")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
                            ...
                            foreach ($week_day_list as $weekday) {
                                ...
                                $week_day_list_slots = $this->db->select("*")->from("clinic_doctor_weekday_slots")->where("clinic_doctor_weekday_id ='" . $weekday->clinic_doctor_weekday_id . "'")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
                                // echo "<br>";echo $this->db->last_query();echo "<br>";
                                $cdws = 0;
                                foreach ($week_day_list_slots as $wdls) {
                                    $sessions_list[] = date('H:i:s', strtotime($wdls->from_time)) . "-" . date('H:i:s', strtotime($wdls->to_time)). "-" .$wdls->session;
                                    //  $sessions_list[] = date('h:i A', strtotime($wdls->from_time)) . "-" . date('h:i A', strtotime($wdls->to_time));
                                    //  $sessions_list[] = $wdls->from_time."-".$wdls->to_time;
                                    $cdws++;
                                }
    
                                $param['doctor']['clinics']['working_days'][$cdw]['day'] = $week;
                                $param['doctor']['clinics']['working_days'][$cdw]['dayNum'] = $weekday->weekday;
                                // $param['sessions_list'] = $sessions_list;
                                // echo $week."<br>";print_r($sessions_list);echo "<br>";
                                for ($i = 0; $i < count($sessions_list); $i++) {
                                    $sl = explode("-", $sessions_list[$i]);
                                    // $time = date("H", strtotime($sl[0]));
                                    if ($sl[2] == 'morning') {
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['session'] = 'Morning';
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['schedule'] = $sl[0] . '-' . $sl[1];
                                    } else if ($sl[2] == 'afternoon') {
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['session'] = 'Afternoon';
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['schedule'] = $sl[0] . '-' . $sl[1];
                                    } else if ($sl[2] == 'evening') {
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['session'] ='Evening';
                                        $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['schedule'] = $sl[0] . '-' . $sl[1];
                                    }
                                    // echo $sessions_list[$i]."<br>";
                                    // $sl = explode("-", $sessions_list[$i]);
                                    // echo $sl[0]."ir";
                                    //  $time = date("H", strtotime($sl[0]));
                                    //  $time = explode(":",$sl[0])[0];
        ...
        else
        {
            ...
            if (count($doctorInfo) > 0) {
                    ...
                    if (count($clinics_list) > 0) {
                            ...
                            $param['doctor']['clinics']['clinic_id'] = $clinics_list->clinic_id;
                            $param['doctor']['clinics']['clinic_name'] = $clinics_list->clinic_name;
                            if($clinics_list->consulting_time=="")
                                $param['doctor']['clinics']['consultation_time'] = "10";
                            else
                                $param['doctor']['clinics']['consultation_time'] = $clinics_list->consulting_time;
                            $param['doctor']['clinics']['consulting_fee'] = $clinics_list->consulting_fee;
                            $param['doctor']['clinics']['registration_fee'] = $clinics_list->registration_fee;
                            $param['doctor']['clinics']['pharmacy_discount'] = $clinics_list->pharmacy_discount;
                            $param['doctor']['clinics']['lab_discount'] = $clinics_list->lab_discount;
                            ...
                            foreach ($week_day_list as $weekday) {
                                ...
                                for ($i = 0; $i < count($sessions_list); $i++) {
                                    ...
                                    // if ($time < 12) {
                                    //     $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['session'] = "Morning";
                                    //     $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['schedule'] =  $sl[1];
                                    // } 
                                    // else if ($time > 12 && $time < 17) {
                                    //     $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['session'] = "Afternoon";
                                    //     $param['doctor']['clinics']['working_days'][$cdw]['timings'][$i]['schedule'] = $sl[0] . '-' . $sl[1];
    ...
    public function procedure_list($parameters, $method, $user_id) {
        ...
        // Get department id of the doctor
        $departmentInfo = $this->db->select('department_id')->from('doctors')->where('doctor_id =',$doctor_id)->get()->row();

        $department_id = $departmentInfo->department_id;

        $list = $this->db->select("*")->from("medical_procedures mp")->join("procedure_department pd","mp.medical_procedure_id = pd.medical_procedure_id")->where("department_id='".$department_id."'")->get()->result();
    ...
    public function book_appointment($parameters, $method, $user_id) {
        ...
        // Get Nurse Role ID
        $roleInfo = $this->db->select("role_id")->from("roles")->where("role_name='Nurse'")->get()->row();

        //Get User Role ID        
        $userroleInfo = $this->db->select("role_id")->from("users")->where("user_id='".$user_id."'")->get()->row();
        ...
        // Get patient info
        $patient_info = $this->db->select('patient_id, title, first_name, last_name, mobile, email_id, payment_status,alternate_mobile,status')->from('patients')->where('patient_id =',$patient_id)->get()->row();
        ...
        */
        ...
        if(count($checkAppointment) > 0) {
            ...
            $data['modified_date_time'] = date('Y-m-d H:i:s');
            // if Nurse
            if($roleInfo->role_id==$userroleInfo->role_id)
            {
                $data['status'] = "vital_signs";
            }
            else
            {
            ...
            // $DName = "Dr.".ucwords($doctor_info->first_name . " " . $doctor_info->last_name);
            // $PName = getPatientName($patient_info->patient_id);
            ...
            // $DName = getDoctorName($doctor_info->doctor_id);
            ...
            // // }
            // // echo $patientMobile."<br>";
            // // echo $patientSMSContent."<br>";
            // // echo  sendsms($patientMobile, $patientSMSContent);
            ...
            // //     if($sms == 1)
            // //     {
            // //         sendsms($patientMobile, $sms_content);
            // //         smsCounter($doctor_id);
            // //         sendsms($patientMobile, $patientSMSContent);
            // //         smsCounter($doctor_id);
            // //     }
            ...
            // //     if($sms == 1)
            // //     {
            // //         sendsms($alternateMobile, $patientSMSContent);
            // //         smsCounter($doctor_id);
            // //         sendsms($patientMobile, $sms_content);
            // //         smsCounter($doctor_id);
            // //     }
            ...
            //     if($sms == 1)
            //     {
            //         textlocalSend($patientMobile, $patientSMSContent);
            //         smsCounter($doctor_id);
            //         // sendsms($patientMobile, $sms_content);
            //     }
            // }
            ...
            //     if($sms == 1)
            //     {
            //         textlocalSend($alternateMobile, $patientSMSContent);
            //         smsCounter($doctor_id);
            //         textlocalSend($patientMobile, $sms_content);
            //         smsCounter($doctor_id);
            //     }
            // }
            $date_split = explode("-", date("Y-m-d", strtotime($appointment_date)));
            ...
            // Send SMS asking to download application & register with UMDAA Health Care Application
            // $sms_content = "Thanks for registering with Dr.".ucwords($doctor_info->first_name." ".$doctor_info->last_name).". Download application using the link https://play.google.com/store/apps/details?id=com.patient.umdaa&hl=en";
    ...
    public function book_appointment($parameters, $method, $user_id) {
        ...
        for($i=0; $i<count($billing_line_items); $i++){
            ...
            $unit = $billing_line_items[$i]['discount_unit'y('code' => '200', 'message' => 'Appointment Booked', 'result' => $param, 'requestname' => $method));
    }
    */
...
// print_r($parameters);
        ...
        $data['doctorInfo']=$doctor_details = $this->Generic_model->getSingleRecord('doctors',array('doctor_id'=>$doctor_id),$order='');
        ...
        $billing = $this->Generic_model->getAllRecords('billing_line_items',array('billing_id'=>$billing_id),$order='');
        $data['patientInfo']=$patient_details = $this->Generic_model->getSingleRecord('patients',array('patient_id'=>$patient_id),$order='');     
        ...
        $html = $this->load->view('billing/generate_billing', $data, true);
        $pdfFilePath = "billing_" . $patient_id . $billing_id.time() . ".pdf";
        $data['file_name'] = $pdfFilePath;
    ...
    // get clinical diagnosis suggestions
    public function getCDSuggestions($parameters,$method,$user_id){
        extract($parameters);
        ...
        else
        {
            ...
            $cdInfo = $this->db->query("select * from (select pcdl.disease_name,pcdl.clinical_diagnosis_id, count(*) as count from patient_clinical_diagnosis pcd,patient_cd_line_items pcdl where pcd.patient_clinical_diagnosis_id=pcdl.patient_clinical_diagnosis_id and pcd.doctor_id='".$doctor_id."' and pcd.clinic_id='".$clinic_id."' group by pcdl.disease_name order by count DESC LIMIT 15) as pcd order by disease_name ASC")->result();
            ...
            if(count($cdInfo)>0)
            {
                ...
                {
                    $data['cdSuggestions'][$i]['clinical_diagnosis_id'] = $value->clinical_diagnosis_id;
                    $data['cdSuggestions'][$i]['disease_name'] = $value->disease_name;
                    $getCode = $this->db->select("*")->from("clinical_diagnosis")
                    ->where("clinical_diagnosis_id='" . $value->clinical_diagnosis_id . "'")->get()->row();
                    $data['cdSuggestions'][$i]['code'] = $getCode->code;
                    $i++;
                }
    ...
    // get investigations suggestions
    public function getInvSuggestions($parameters,$method,$user_id){
        extract($parameters);
        if(empty($doctor_id) || empty($clinic_id))
        {
            $this->response(array('code'=>'201','message'=>'Error Occured','result'=>'Send The Parameters','requestname'=>$method));
        ...
        else
        {
            ...
            $invInfo = $this->db->query("select * from (select pinl.investigation_id,pinl.investigation_name, count(*) as count from patient_investigation pin,patient_investigation_line_items pinl where pin.patient_investigation_id=pinl.patient_investigation_id and pin.doctor_id='".$doctor_id."' and pin.clinic_id='".$clinic_id."' group by pinl.investigation_name order by count DESC LIMIT 15) as pin order by investigation_name ASC")->result();
            // $data['query'] = $this->db->last_query();
            if(count($invInfo)>0)
            {
                $i = 0;
                ...
                {
                    $data['invSuggestions'][$i]['investigation_id'] = $value->investigation_id;
                    $data['invSuggestions'][$i]['investigation_name'] = $value->investigation_name;
                    $data1[$i] = $value->investigation_name;
                    $data2[$i] = $value->investigation_id;
                    $i++;
                }
                // echo "<pre>";print_r($data2);echo "</pre>";
                // echo "<pre>";print_r($data);echo "</pre>";
                // asort($data['invSuggestions']);
    ...
    public function appointment_status($parameters, $method, $user_id) {
        ...
        if ($status_type == 'checked_in') {
            ...
            $this->Generic_model->angularNotifications($ap_details->patient_id, $appointment_id, $ap_details->doctor_id, $ap_details->clinic_id, 'check_in', 'dashboard');
    ...
    public function patient_clinical_diagnosis($parameters, $method, $user_id) {
        ...
        if($type == 'del'){
            ...
            if($parameters['clinicaldiagnosis'][0]['clinical_diagnosis_id'] == 0){
                $check_data=$this->db->select("*")->from("patient_cd_line_items")
                ->where("patient_clinical_diagnosis_id='".$patient_clinical_diagnosis_id."' and disease_name='".$parameters['clinicaldiagnosis'][0]['disease_name']."'")
                ->get()->row();
            }
            else{
                $check_data=$this->db->select("*")->from("patient_cd_line_items")
                ->where("patient_clinical_diagnosis_id='".$patient_clinical_diagnosis_id."' and clinical_diagnosis_id='".$parameters['clinicaldiagnosis'][0]['clinical_diagnosis_id']."'")
                ->get()->row();
            }
            ...
            if(count($check_data)>0)
            {
                $this->response(array('code' => '201', 'message' => 'Already Exists', 'result' =>  'Already Exists', 'requestname' => $method));
            }
            else{
    ...
    public function investigations_submit($parameters, $method, $user_id) {
        ...
        if (count($investigation) > 0) {
            ...
            if($type == "del"){
                ...
                $this->response(array('code' => '200', 'message' => 'Record Deleted Successfully', 'result' => [], 'requestname' => $method));
            }

            if(count($check_inv_exist) > 0) {

                if($investigation[0]['investigation_id'] == 0){
                    $checkInfo = $this->Generic_model->getSingleRecord('patient_investigation_line_items', array('patient_investigation_id'=>$check_inv_exist->patient_investigation_id,'investigation_name'=>$investigation[0]['investigation_name']));        
                }
                else{
                    $checkInfo = $this->Generic_model->getSingleRecord('patient_investigation_line_items', array('patient_investigation_id'=>$check_inv_exist->patient_investigation_id,'investigation_id'=>$investigation[0]['investigation_id']));
                }
            ...
            if(count($checkInfo)>0){
                ...
                $patient_details = $this->db->select("*")->from("patients")
            ...
            $this->Generic_model->angularNotifications($ap_details->patient_id, $appointment_id, $ap_details->doctor_id, $ap_details->clinic_id, 'investigation', 'dashboard');
...
public function prescription_submit($parameters, $method, $user_id) {
    // start updating pdf_status for summary(sync b/n andriod and angular)
    $pdf_status = 1;
    ...
    //end updating pdf_status for summary(sync b/n andriod and angular)
    // check for existing prescription 
//  echo  $parameters['drug_allergy'];
//  exit();
    $drug['allergy'] = $parameters['drug_allergy'];
    $this->Generic_model->updateData("patients",$drug, array('patient_id'=>$parameters['patient_id']));
    ...
    if($type == "del"){
        ...
        // // Check number of line items with master record id
        // $count = $this->Generic_model->getNumberOfRecords('patient_prescription_drug',array('patient_prescription_id'=>$prescription_drug[0]['patient_prescription_id']));

        // $countRes = $this->db->query("select count(*) as count from patient_prescription_drug where patient_prescription_id=".$prescription_drug[0]['patient_prescription_id'])->result();

        // $count = $countRes[0]->count;
       
        // if($count == 0){
        //     // Delete master record as well
        //     $res = $this->Generic_model->deleteRecord('patient_prescription',array('patient_prescription_id'=>$prescription_drug[0]['patient_prescription_id']));
        // }
        ...
        if ($check_exist > 0) {
            ...
            if (count($prescription_drug) > 0) {
                for ($i = 0; $i < count($prescription_drug); $i++) {
                    ...
                    // if($prescription_drug_insert[$i]['preffered_intake']=="")
                    //     $prescription_drug_insert[$i]['preffered_intake'] = "AF";
                    // else
                    ...
                    $parameters['prescription'][$i]['patient_prescription_id'] = $check_exist->patient_prescription_id;
                }   
            } 
            ...
            $check_data = $this->db->select("*")->from("patient_prescription")->where("appointment_id ='" . $parameters['appointment_id'] . "'")->get()->row();
            if (count($prescription_drug) > 0) {
                for ($i = 0; $i < count($prescription_drug); $i++) {
                    $prescription_drug_insert[$i]['patient_prescription_id'] = $check_data->patient_prescription_id;
                    $prescription_drug_insert[$i]['drug_id'] = $prescription_drug[$i]['drug_id'];
                    $prescription_drug_insert[$i]['medicine_name'] = $prescription_drug[$i]['medicine_name'];
                    $prescription_drug_insert[$i]['day_schedule'] = $prescription_drug[$i]['day_schedule'];
                    $prescription_drug_insert[$i]['day_dosage'] = $prescription_drug[$i]['day_dosage'];
        ...
        $parameters['prescription'][$i]['appointment_id'] = $parameters['appointment_id'];
        $parameters['prescription'][$i]['dosage_frequency'] = $prescription_drug[$i]['dosage_frequency'];

        $this->response(array('code' => '200', 'message' => 'Prescription Created Successfully', 'result' => $parameters, 'requestname' => $method));

        $ap_details = $this->Generic_model->getSingleRecord('appointments', array('appointment_id' => $parameters['appointment_id']), '');
        $this->Generic_model->angularNotifications($ap_details->patient_id, $appointment_id, $ap_details->doctor_id, $ap_details->clinic_id, 'pharmacy', 'dashboard');
        ...
        $i = 0;
        $patient_prescription_edit['patient_prescription_id'] = $prescription_drug[$i]['patient_prescription_id'];
        ...
        // if($patient_prescription_edit[$i]['preffered_intake']=="")
        //     $patient_prescription_edit[$i]['preffered_intake'] = "AF";
        // else
        ...
        $updateRes = $this->Generic_model->updateData('patient_prescription_drug', $patient_prescription_edit, array('patient_prescription_drug_id'=>$prescription_drug[$i]['patient_prescription_drug_id']));

        $this->response(array('code' => '200', 'message' => 'Prescription Saved Successfully', 'result' => $parameters['drug_allergy'], 'requestname' => $method));
...
public function patient_vitals_insert($parameters, $method, $user_id) {
    ...
    {
        $vital_sign_recording_date_time = date("Y-m-d H:i:s");
        for ($v = 0; $v < count($vitalsign); $v++) {
            ...
            $patient_vital['vital_sign_recording_date_time'] = $vital_sign_recording_date_time;
...
//updating in patient consent form
    ...
    $data1['umr_no'] = $parameters['umr_no'];
...
public function get_patient_consent_check_list($parameters, $method) {

    $query = "";
    if($parameters['patient_consent_form_max']!="0")
    {
        $query = $this->db->select("*")->from("patient_checklist cf")->where("cf.patient_consent_form_id='" . $parameters['patient_consent_form_id'] . "' and "
        ."cf.category='" . $parameters['patient_consent_form_category'] ."'")
        ->limit($parameters['patient_consent_form_max'],$parameters['patient_consent_form_min'])
        ->get()->result();
    }
    else
    {
        $query = $this->db->select("*")->from("patient_checklist cf")->where("cf.patient_consent_form_id='" . $parameters['patient_consent_form_id'] . "' and "
        ."cf.category='" . $parameters['patient_consent_form_category'] ."'")->get()->result();
    }
    ...
    $this->response(array('code' => '200', 'message' => 'Consent Checklist','count'=>count($query), 'result' => $data, 'requestname' => $method));
}
...
//inventory outwards insert
    ...
    public function downloadConsentform($parameters, $method, $user_id) {
        ...
        $pdfFilePath = $clinic_id."_".$patient_id."_".$appointment_id."_".time() . ".pdf";
        $this->load->library('M_pdf');
        $stylesheet  = '';
        //$stylesheet .= file_get_contents(base_url()."assets/css/bootstrap.min.css"); // external css
        $stylesheet .= file_get_contents(base_url()."assets/css/pdf.css");
    ...
    public function patient_summary_list($parameters, $method, $user_id) {
        ...
        $appointments = $this->db->select("a.appointment_id,a.course_in_hospital, a.appointment_date,
         d.doctor_id, d.salutation, d.first_name, d.last_name, de.department_id, de.department_name,a.pdf_status")
        ->from("appointments a")
        ->join("doctors d","a.doctor_id=d.doctor_id","left")
        ->join("department de","d.department_id=de.department_id","left")
        ->where("a.patient_id='" . $patient_id . "' and a.doctor_id='".$doctor_id."' and a.appointment_date <= '".date('Y-m-d')."'")
...
public function dischargeSummary($parameters, $method, $user_id) {
    ...
    $this->response(array('code' => '200', 'message' => 'PDF File', 'result' => $data, 'requestname' => $method));
}
...
//     $data['patientInfo'] = getPatientDetails($data['appointments']->patient_id);
...
// echo "\nIF=".$this->db->last_query();
    // echo "\n-----------------------------------------------------------------------\n";
                    ...
                    foreach ($row_elements as $key => $rresult) {
                        ...
                        $data['form']['sections'][$a]['sub_sections'][$i]['rows'][$j]['labels'][$k]['row_index'] = $rresult->row_index;
                        $data['form']['sections'][$a]['sub_sections'][$i]['rows'][$j]['labels'][$k]['column_index'] = $rresult->column_index;
                        $options = $this->db->select("*")->from("field_option")->where("field_id='" . $rresult->field_id."'")->order_by("position","ASC")->get()->result();
// echo "\nIF=".$this->db->last_query();
    // echo "\n-----------------------------------------------------------------------\n";
                        $l = 0;
                        foreach ($options as $okey => $oresult) {
    ...
    // echo "\n-----------------------------------------------------------------------\n";
                ...
                foreach ($label_result as $key2 => $value2) {
                    ...
                    $data['form']['sections'][$a]['sub_sections'][$i]['labels'][$j]['widgetType'] = $value2->field_type;
                    $data['form']['sections'][$a]['sub_sections'][$i]['labels'][$j]['labelText'] = $value2->field_name;
                    $field_option_result = $this->db->query('select * from field_option  where field_id=' . $value2->field_id .' order by position ASC')->result();
                // echo "\nELSE=".$this->db->last_query();
    // echo "\n-----------------------------------------------------------------------\n";
                    $k = 0;
                    foreach ($field_option_result as $key3 => $value3) {
...
public function prescription_saveas_template($parameters, $method, $user_id) {
    ...
    // Check if the template exist with the same name w.r.to the doctor id & template name
    $templateCount = $this->db->select('prescription_template_id, prescription_template_name')->from('prescription_template')->where('prescription_template_name =',$parameters['prescription_template_name'])->where('doctor_id =',$parameters['doctor_id'])->get()->num_rows();

    if($templateCount > 0){
        $msg = "Template already exist with the same name";
        $code = '201';
        $param = null;
    }else{
        ...
        for ($i = 0; $i < count($drugs_info); $i++) {
            unset($insert_line_items['prescription_template_line_item_id']);
            ...
            $insert_line_items['mode'] = $drugs_info[$i]['mode'];   
            ...
            $insert_line_items['dosage_unit'] = $drugs_info[$i]['dosage_unit'];
            ...
            $insert_line_items['prescription_template_line_item_id'] = $this->Generic_model->insertDataReturnId('prescription_template_line_items', $insert_line_items);
            $param[$i] = $insert_line_items;
        }

        $msg = "Prescription Template Saved Successfully";
        $code = "200";
    }

    $this->response(array('code' => $code, 'message' => $msg, 'result' => $param, 'requestname' => $method));
}
...
//echo "hi";exit();
    ...
    for ($i = 0; $i < count($investgating_info); $i++) {
        unset($insert_line_items['investigation_template_line_item_id']);
        ...
        $insert_line_items['investigation_template_line_item_id'] = $this->Generic_model->insertDataReturnId('doctor_investigation_template_line_items', $insert_line_items);
    ...
    if (count($getlist) > 0) {
        ...
        foreach ($getlist as $key => $value) {
            ...
            foreach ($getchild as $key => $value2) {
                ...
                $data['template_list'][$i]['prescriptions'][$j]['dosage_unit'] = $value2->dosage_unit;
...
public function patient_procedure_edit($parameters, $method, $user_id) {
    ...
    if (count($checking_patient_procedure) > 0) {
        ...
        //     $data['procedure_description'] = "<html><body style='padding:0px; margin:0px;'>" . $check_doctor_procedure->medical_procedure . "/body></html>";
        // } else {
        ...
        $check_doctor_procedure = $this->db->select("*")->from("doctor_medical_procedures")->where("doctor_id ='" . $doctor_id . "' and medical_procedure_id ='" . $medical_procedure_id . "' and clinic_id ='" . $clinic_id . "'")->order_by('doctor_medical_procedure_id','DESC')->get()->row();
        // echo $this->db->last_query();
        if (count($check_doctor_procedure) > 0) {
            $data['procedure_description'] = "<html><body style='padding:0px; margin:0px;'>" . $check_doctor_procedure->medical_procedure . "</body></html>";
        } else {
        ...
        // $data['procedure_description'] = $checking
    }
...
$this->response(array('code' => '200', 'message' => 'Vitals Print Successful', 'result' => $result, 'requestname' => $method));
    ...
    ****/
    ...
    //     // $paraa = $clist->patient_id;
    //     // $paraa = $clist->doctor_id;

    //     $clist1 = $this->db->select("appointment_id")->from("appointments ap")
    ...
    //             $i = 0;
    //             $clist3 = $this->db->select("appointment_id,patient_clinical_diagnosis_id")->from("patient_clinical_diagnosis pi")
    //             ->where("pi.appointment_id ='" . $cform->appointment_id . "'  ")
    //             ->get()
    //             ->row();
    //             $para['investigation'][$i]['appointment_id'] = $clist3->appointment_id;
    //             $para['investigation'][$i]['patient_clinical_diagnosis_id']=$clist3->patient_clinical_diagnosis_id;

                
    
    //             $inv_lineitems = $this->db->select("*")->from("patient_cd_line_items pil")
    //             ->where("pil.patient_clinical_diagnosis_id='".$clist3->patient_clinical_diagnosis_id."'")
    //             ->get()
    //             ->result();
    ...
    //             $i = 0;
    //             $clist3 = $this->db->select("appointment_id,patient_investigation_id")->from("patient_investigation pi")
    //             ->where("pi.appointment_id ='" . $cform->appointment_id . "'  ")
    //             ->get()
    //             ->row();
    //             $para['investigation'][$i]['appointment_id'] = $clist3->appointment_id;
    //             $para['investigation'][$i]['patient_investigation_id']=$clist3->patient_investigation_id;
    
    //             $inv_lineitems = $this->db->select("*")->from("patient_investigation_line_items pil")
    //             ->where("pil.patient_investigation_id='".$clist3->patient_investigation_id."'")
    //             ->get()
    //             ->result();
                
    //             $j=0;
    //             foreach($inv_lineitems as $inv_lineitem){
    //                     // $paraa['investigation'][$j]['investigation_name'] = $inv_lineitem->investigation_name;
    //                     // $paraa['investigation'][$j]['investigation_id'] = $inv_lineitem->investigation_id;

    //             $paraa['investigations_list'][$j]['patient_investigation_line_item_id'] = $inv_lineitem->patient_investigation_line_item_id;
    //             $paraa['investigations_list'][$j]['patient_investigation_id'] = $inv_lineitem->patient_investigation_id;
    //             // $paraa['investigation'][$j]['appointment_id'] = $apt_id;
    //             $paraa['investigations_list'][$j]['category'] = $inv_lineitem->category;
    //             $paraa['investigations_list'][$j]['checked'] = $inv_lineitem->checked;
    //             $paraa['investigations_list'][$j]['investigation_id'] = $inv_lineitem->investigation_id;
    //             $paraa['investigations_list'][$j]['investigation_name'] = $inv_lineitem->investigation_name;
    //             $paraa['investigations_list'][$j]['mrp'] = $inv_lineitem->mrp;
    ...
    public function getLatestInv($parameters, $method, $user_id)
    {
        $appointment_id = $parameters['appointment_id'];
        $clinic_id = $parameters['clinic_id'];
            ...
            if(count($chec1) == 0)
            {
                ...
                if(count($abc)>0)
                {
                    foreach($abc as $clist12)
                    {
                        ...
                        $paraa['investigations_list'][$j]['patient_investigation_line_item_id'] = $pid;
                        $paraa['investigations_list'][$j]['patient_investigation_id'] = $iddddd;
                        // $paraa['investigation'][$j]['appointment_id'] = $apt_id;
                        // $paraa['investigation'][$j]['category'] = $inv_lineitem->category;
                        $paraa['investigations_list'][$j]['checked'] = $clist12->checked;
                        $paraa['investigations_list'][$j]['investigation_id'] = $clist12->investigation_id;
                        $paraa['investigations_list'][$j]['investigation_name'] = $clist12->investigation_name;
                 // $paraa['investigation'][$j]['mrp'] = $inv_lineitem->mrp;
                $patient_lab = $this->db->query("select i.report_status,pl.patient_lab_report_id from investigation_status i,patient_lab_reports pl where pl.patient_investigation_line_item_id=i.patient_investigation_line_item_id and pl.patient_investigation_line_item_id='".$pid."'")->row();
                $investigation_clinic_status = $this->db->query("select * from clinic_investigations where investigation_id='".$clist12->investigation_id."'")->row();
                // echo $this->db->last_query();
                if(count($investigation_clinic_status)>0)
                    $invStatus = "1";
                else
                    $invStatus = "0";

                $status = $patient_lab->report_status;

                $paraa['investigations_list'][$j]['investigation_status'] = $status;
                $paraa['investigations_list'][$j]['patient_lab_report_id'] = $patient_lab->patient_lab_report_id;
                $paraa['investigations_list'][$j]['clinic_status'] = "0";
               ...
               if(count($inv_lineitems)>0)
               {
                foreach($inv_lineitems as $inv_lineitem){
                    $status = "";
                     ...
                     // $paraa['investigation'][$j]['mrp'] = $inv_lineitem->mrp;
                    // echo $this->db->last_query();
                    // print_r($bl_lineItem_Info);
                    $patient_lab = $this->db->query("select i.report_status,pl.patient_lab_report_id from investigation_status i,patient_lab_reports pl where pl.patient_investigation_line_item_id=i.patient_investigation_line_item_id and pl.patient_investigation_line_item_id='".$inv_lineitem->patient_investigation_line_item_id."'")->row();
                    // $paraa['query'] = $this->db->last_query();
                    $investigation_clinic_status = $this->db->query("select * from clinic_investigations where investigation_id='".$inv_lineitem->investigation_id."'")->row();
                    // echo $this->db->last_query();
                    if(count($investigation_clinic_status)>0)
                        $invStatus = "1";
                    else
    ...
    public function patientHealthRecords($parameters, $method, $user_id) {
        ...
        $getAppointments = $this->db->select("a.appointment_id, a.appointment_date, a.appointment_time_slot, b.first_name, b.last_name, c.department_name")->from("appointments a")->join("doctors b","a.doctor_id=b.doctor_id")->join("department c","b.department_id=c.department_id")->where("a.patient_id='".$parameters['patient_id']."'")->order_by("a.appointment_id","DESC")->get()->result();
  ...
  elseif($userInfo->role_id == "6")
  {
    $gform_data = $this->Generic_model->getAllRecords('patient_form',array('form_type'=>$form_type,'patient_id'=>$patient_id,'doctor_id'=>$docId),array('field'=>'created_date_time','type'=>'desc'));
...
public function getSavedRecursiveForm_pdf($parameters, $method, $user_id)
{
    ...
    $data['form_type']=$getFormType=$this->db->query("select * from 
    patient_form where  patient_form_id='".$patient_form_id."'")->row();

    // if($getFormType->form_type == 'GPE')
    // {
        $data['form_typee']=$getFormType->form_type;
    // }
    // else if($getFormType->form_type == 'SE')
    // {
    //     $data['form_typee']='Systemic examination Data';
    // }

    if($getFormType->form_type == 'Systemic Examination')
    {
        $data['sugg'] = $this->db->query("select * from suggestions_list where
        patient_id='".$patient_id."' and doctor_id='".$doctor_id."' 
        and form_type='SE'")->result();	
    }
    else if($getFormType->form_type == 'Other Systems')
    {
        $data['sugg']= $this->db->query("select * from suggestions_list where
        patient_id='".$patient_id."' and doctor_id='".$doctor_id."' 
        and form_type='OS'")->result();	
    }
    else
    {
        $data['sugg'] = $this->db->query("select * from suggestions_list where
        patient_id='".$patient_id."' and doctor_id='".$doctor_id."' 
        and form_type='".$getFormType->form_type."'")->result();	
    }
    ...
    $data_1['patient_form_id'] = $patient_form_id;
...
public function app_version($parameters,$method,$user_id)
{
    ...
    if(count($result)>0)
    {
        ...
        if(isset($parameters['doctor_id'])){
            ...
            if($promotionalPic->doctor_id == "11")
            {
...
public function getVideoDoctorTimings($parameters, $method, $user_id) {
        ...
        foreach ($doctors_list as $doctor) {
            ...
            if (count($clinics_list) > 0) {
                ...
                foreach ($clinics_list as $clinic) {
                    ...
                    $param['doctor'][$d]['clinics'][$c]['clinic_name'] = $clinic->clinic_name;
                    $param['doctor'][$d]['clinics'][$c]['consultation_time'] = $clinic->consulting_time;
                    $param['doctor'][$d]['clinics'][$c]['consulting_fee'] = $clinic->online_consulting_fee;
                    $param['doctor'][$d]['clinics'][$c]['registration_fee'] = $clinic->registration_fee;
                    $param['doctor'][$d]['clinics'][$c]['pharmacy_discount'] = $clinic->pharmacy_discount;
                    $param['doctor'][$d]['clinics'][$c]['lab_discount'] = $clinic->lab_discount;
