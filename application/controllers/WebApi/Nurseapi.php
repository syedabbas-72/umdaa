    ...
    if(!empty($boolean)){
        ...
        if(!empty(isset($_POST))){
            ...
            if($nurse_id !="" and $patient_id !="")
                        ...
                        $this->db2->query("INSERT INTO nurse_patient_vitals (patient_id,nurse_id,clinic_id,vital_priority,level_of_consciousness,heart_rate,blood_pressure,respiration,temperature,saturation,oxigen,copd,vital_creation_date,vitals_date)VALUES ('$patient_id','$nurse_id','$clinic_id','$vital_priority','$level_of_consciousness','$heart_rate','$blood_pressure','$respiration','$temperature','$saturation','$oxigen','$copd',CURRENT_TIMESTAMP(),'$vitals_date')");
...
//    print_r($_POST);
    ...
    if(!empty($boolean)){ 
         ...
         $this->db2->query("INSERT into nurse_patient (nurse_id,patient_name,patient_age,patient_mobile_number,patient_location,patient_gender,priority,clinic_id) 
         values ('$nurse_id','$patient_name','$patient_age','$patient_mobile_number','$patient_location','$patient_gender','3','$clinic_id')");
...
public function getClinicPatient_post(){
    ...
    if(!empty($boolean)){
        ...
        $tab=$this->db2->select('count(*) as val')->from('nurse_patient')->where('clinic_id= "'.$clinic_id.'"')->get()->row();
        ...
        // $total=$this->db2->query("select * from nurse_patient where clinic_id='".$clinic_id."'")->result_object();
        ...
        // // $data['code']="200";
        // // $data['patients']=$total;
        // // // $this->response(array('code'=>'200','message'=>'sucessfull',$data));
        // // $this->response($data);
        // }else{
        //     $this->response(array('code' => '404', 'message' => 'there are no patients'), 404);
...
public function getNursePatientVItals_post(){  
    ...
    if(1==1){
        ...
        $this->response(array('code' => '404', 'message' => 'there are no patients'), 200);
    }
    }
    else
    {
...
public function latestvitals_post(){

    extract($_POST);

   $latiestvitals= $this->db2->query("SELECT * FROM nurse_patient_vitals WHERE patient_id = '".$patient_id."' ORDER BY nurse_patient_vitals_id DESC LIMIT 0,1")->row();

   $vital_id=$latiestvitals->nurse_patient_vitals_id; 
   $patient_id =$latiestvitals->patient_id;
   $nurse_id=$latiestvitals->nurse_id;
   $clinic_id=$latiestvitals->clinic_id;
   $vital_priority=$latiestvitals->vital_priority;
   $level_of_consciousness=$latiestvitals->level_of_consciousness;
   $heart_rate=$latiestvitals->heart_rate;
   $blood_pressure=$latiestvitals->blood_pressure;
   $respiration=$latiestvitals->respiration;
   $temperature=$latiestvitals->temperature;
   $saturation=$latiestvitals->saturation;
   $oxigen=$latiestvitals->oxigen;
   $copd=$latiestvitals->copd;
   $date=$latiestvitals->vital_creation_date;
   $vitals_date=$latiestvitals->vitals_date;

   $this->response(array('code' => '200', 'message' => 'sucess','latientvitals'=>array(
       'vital_id'=>$vital_id,
       'patient_id'=>$patient_id,
       'nurse_id'=>$nurse_id,
       'clinic_id'=>$clinic_id,
       'vital_prority'=>$vital_priority,
