...
public function get_batch_details()
{
	...
	foreach($batches as $result)
	{
		...
		if($aqty == 0)
			continue;
		echo "<tr><td><input type='checkbox' class='batch_cb' value='".trim($result['batch_no'])."' name='batchno[]' onclick='enable_text_box(".'"'.stripslashes($result['batch_no']).'"'.")' id='batchno_".trim($result['batch_no'])."' />".trim(stripslashes($result['batch_no']))."</td><td>".$aqty."</td><td><input type='number' class='digits' id='bqty_".trim($result['batch_no'])."' readonly onkeyup='checkvalue(".$aqty.",\"".trim(stripslashes($result['batch_no']))."\");' /><input type='hidden' name='unitp' id='unitp_".stripslashes($result['batch_no'])."' value='".$unitp."' /></td></tr>";
	}
	echo "<table>";
}
