...
public function patient_consent_form_add($parameters,$method,$user_id)
{
    ...
    $this->response(array('code' => '200', 'message' => 'add', 'result' => $cf, 'requestname' => $method));
}
...
public function patient_consent_form_list($parameters,$method,$user_id)
{
    ...
    $clist = $this->db->select("*")->from("patient_consent_forms cf")
        ->where("cf.appointment_id='" . $appointment_id . "' order by consent_form_id")
        ->get()
        ->result();
    ...
    public function doctor_registration($parameters, $method, $user_id){
        ...
        if(count($docRec) == 0){
            ...
            if($clinic_id == 0){
                ...
                $primary_clinic = 1;
                $inchargeUname = "FO_".str_replace(' ', '', $clinic_name)."_".rand(1,99);
            ...
            $message = "<h4>Congratulations!<br><br>Your account has been successfully created.<h4><br><br>You can log in to our UMDAA App using your registered Mobile Number and Password. For accessing Admin portal You have to check this below link.<br><br>".base_url().". <br><br>Below are your credentials.<br><br><b>Username:</b>".$inchargeUname."<br><br><b>Password:<b> ".$inchargePwd;

            $ok = $this->mail_send->Content_send_all_mail($from, $to, $subject, '', '', $message);   

            //Message To Doctor
            $docSMS = "Dear ".$doctor['full_name']." Welcome to UMDAA Family. Your account has been successfully created. You can log in to our UMDAA App using your registered Mobile Number and Password.";
...
public function prescription_submit($parameters, $method, $user_id) {
    ...
    if($type == "del"){
        ...
        if($patient_prescription_edit[$i]['preffered_intake']=="")
            ...
            $patient_prescription_edit[$i]['preffered_intake'] = $prescription_drug[$i]['preffered_intake'];
...
public function shortSummary($parameters, $method, $user_id) {
    ...
    foreach ($vital_sign as $key => $value) {
        ...
        $data['patient_investigations'] = $this->db->select("*")->from("patient_investigation_line_items pil")->join("patient_investigation pi","pil.patient_investigation_id=pi.patient_investigation_id")->where(" pi.appointment_id='".$appointment_id."'")->get()->result();
...
public function dischargeSummary($parameters, $method, $user_id) {
    ...
    $appointment_id = $parameters['appointment_id'];
        //$data['visit']=$visit;
    $data['appointments'] = $this->db->select("a.*, c.clinic_name,c.clinic_logo,c.address,c.clinic_phone,p.title,p.first_name as pname,p.last_name as plname,p.gender,p.age,p.address_line,p.allergy,p.umr_no,p.preferred_language,p.mobile,p.qrcode,d.salutation,d.first_name as dfname,d.last_name as dlname, d.qualification, d.registration_code, dep.department_name")
    ->from("appointments a")->join("clinics c","a.clinic_id = c.clinic_id","left")->join("patients p","a.patient_id = p.patient_id","left")->join("doctors d","a.doctor_id = d.doctor_id","left")->join('department dep','d.department_id = dep.department_id')->where("a.appointment_id='" . $appointment_id . "'")->get()->row();
...
public function fullSummary($parameters, $method, $user_id) {
    ...
    foreach ($vital_sign as $key => $value) {
        ...
        $data['get_se_info'] = $this->db->query("select * from patient_form where appointment_id='".$appointment_id."' and form_type='Systemic Examination' order by patient_form_id DESC")->row();        
...
$this->response(array('code' => '200', 'message' => 'Vitals Print Successful', 'result' => $result, 'requestname' => $method));
    ...
    ****/
    ...
    public function prescription_template_delete($parameters, $method, $user_id) 
    {
        ...
        if($type == "del")
        {
            $res = $this->Generic_model->deleteRecord('prescription_template',array('prescription_template_id'=>$prescription_template_delete[0]['prescription_template_id']));
            $res = $this->Generic_model->deleteRecord('prescription_template_line_items',array('prescription_template_id'=>$prescription_template_delete[0]['prescription_template_id']));
            $this->response(array('code' => '200', 'message' => 'Record Deleted Successfully', 'result' => NULL, 'requestname' => $method));  
        }
        else if($type == 'edit'){
            $i = 0;
