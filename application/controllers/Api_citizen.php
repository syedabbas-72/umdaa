...
public function patientdoctor_profile($parameters, $method, $user_id){
    $patient_id = $parameters['patient_id'];
    $doctor_appoinment_list = $this->db->query("select doctor_id from clinic_doctor_patient where patient_id ='".$patient_id."'  group by doctor_id")->result();
    $j=0;
...
public function doctor_feedback_list($parameters, $method, $user_id){
    ...
    if(count($feedbackList) > 0){
        $this->response(array('code' => '200', 'message' => 'Feedback list', 'result' => $feedbackList, 'requestname' => $method));
    }else{
