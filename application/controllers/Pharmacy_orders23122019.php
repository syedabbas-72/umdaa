...
public function generateinventorypdf()
{
    ...
    $pdfFilePath = "TI-".$clinic_id.date("dmY").".pdf";
  ...
  //   }
    ...
    public function getFinances() {
        ...
        if ($start == $end) {
            ...
            $billingMaster = $this->db->query("select * from billing where status!='2' and billing_type='Pharmacy' and clinic_id='".$clinic_id."' ".$billCond." and created_date_time LIKE '".$start."%'")->result();
        } else {
            ...
            $billingMaster = $this->db->query("select * from billing where status!='2' and billing_type='Pharmacy' and clinic_id='".$clinic_id."' ".$billCond." and created_date_time BETWEEN '".$start."%' AND '".$end."%'")->result();
        }
        // $revenue['query'] = $this->db->last_query();
        $expectedRevenue = 0;
        $convertedRevenue = 0;
        $outPeopleRevenue = 0;
        $billingCount = 0;
        $OutdiscountRevenue = 0;
        $discountRevenue = 0;

        foreach($billingMaster as $value)
        {

            $billingItemsMaster = $this->db->query("select amount,total_amount from billing_line_items where billing_id='".$value->billing_id."'")->result();

            foreach($billingItemsMaster as $bm_value) 
            {
                if($bm_value->total_amount!=0)
                {
                    if($value->patient_id==NULL || $value->patient_id=="")
                        $OutdiscountRevenue = $OutdiscountRevenue +($bm_value->total_amount-$bm_value->amount);
                }
            }
        }

        $revenue['outdiscounts'] = number_format($OutdiscountRevenue, 2);
        if (count($patientPrescriptions) > 0) {
            ...
            foreach ($patientPrescriptions as $prescription) {
                ...
                // Check whether this prescription converted as a bill or no
                // If converted get the amount of the billing
                $convertedPrecription = $this->db->select('billing_id')->from('billing')->where('patient_prescription_id ="'.$prescription->patient_prescription_id.'" and status!="2"')->get()->result_array();
                ...
                if (count($convertedPrecription) > 0) {
                    ...
                    foreach ($convertedPrecription as $billing) {
                        ...
                        // Get the line items of the billing and sum of the amounts
                        $billingInfo = $this->db->select('amount,total_amount')->from('billing_line_items')->where('billing_id =', $billing['billing_id'])->get()->result();

                        foreach ($billingInfo as $value) {
                            $convertedRevenue = $convertedRevenue + $value->amount;
                            if($value->total_amount!=0)
                                $discountRevenue = $discountRevenue + ($value->total_amount-$value->amount);
                        }
                    }
                }
            }
        }
        $revenue['expected_prescriptions_count'] = count($patientPrescriptions);
        $revenue['converted_prescriptions_count'] = count($convertedPrecription);
        $revenue['billing_count'] = $billingCount;
        $revenue['expected_revenue'] = number_format($expectedRevenue,2);
        $revenue['converted_revenue'] = number_format($convertedRevenue, 2);
        $revenue['indiscounts'] = number_format($discountRevenue, 2);
        $lost_revenue = number_format($expectedRevenue,2) - number_format($convertedRevenue, 2)-$discountRevenue;
        if($lost_revenue>0)
            $revenue['lost_revenue'] = number_format($lost_revenue,2);
        else
            $revenue['lost_revenue'] = "0.00";

        // Get the list of customers purchased drugs from the pharmacy who are outsiders w.r.to the date
        if ($start == $end) {
            $outPeople = $this->db->query("select billing_id, guest_name, guest_mobile from billing where patient_prescription_id = 0 and clinic_id = '" . $clinic_id . "' and billing_type = 'Pharmacy' and  status!='2' and billing_date_time like '" . $start . "%'")->result();
        } else {
            $outPeople = $this->db->query("select billing_id, guest_name, guest_mobile from billing where patient_prescription_id = 0 and clinic_id = '" . $clinic_id . "' and billing_type = 'Pharmacy' and  status!='2' and billing_date_time between '" . $start . "%' and '" . $end . "%'")->result();
        }

        if (count($outPeople) > 0) {

            $revenue['out_people_count'] = count($outPeople);
            ...
            $revenue['out_people_count'] = 0;
        }

        $revenue['out_people_revenue'] = number_format($outPeopleRevenue, 2);
...
//	Creating inventory json w.r.t to clinic
    ...
    function raise_shortage_indent() {
        ...
        for ($i = 0; $i < $drug_ids; $i++) {
            ...
            $lineinfo['vendor_id'] = $_POST['vendor'][$i];
            $lineinfo['status'] = 1;
