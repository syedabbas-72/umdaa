...
class V1_1 extends REST_Controller
{
    ...
    public function getReviews($parameters, $method, $user_id)
    {
        extract($parameters);
        $reviewsInfo = $this->db->query("select * from patient_doctor_feedback where doctor_id='".$doctor_id."'")->result();
        if(count($reviewsInfo) > 0)
        {
            $i = 0;
            foreach($reviewsInfo as $value);
            {
                $patientInfo = $this->db->query("select * from patients where patient_id='".$value->patient_id."'")->row();
                ...
                $param['reviews'][$i]['description'] = $value->description;
                $param['reviews'][$i]['patient_name'] = $title.$patientInfo->first_name." ".$patientInfo->last_name;
                $param['reviews'][$i]['description'] = $value->description;
                $i++;
            }
        ...
        {
            $param['reviews'] = [];
            $this->response(array('code'=>'201','message'=>'Reviews Not Found','result'=>$param,'method'=>$method));
        }
    }
    ...
    //             $patient_prescription = $this->Generic_model->getAllRecords('patient_prescription', array('patient_prescription_id'=>$iddd));
                
    //             $date1 = date_create($appInfo->appointment_date);
    //             $date2 = date_create($patient_prescription->follow_up_date);
    //             $diffDate = date_diff($date1,$date2);

    //             $paraa['instructions'] = $patient_prescription->general_instructions;
    //             $paraa['plan'] = $patient_prescription->plan;
    //             $paraa['next_followup_date'] = $diffDate->format("%R%a");
    ...
    //     $date1 = date_create($appInfo->appointment_date);
    //     $date2 = date_create($inv_lineitems[0]->follow_up_date);
    //     $diffDate = date_diff($date1,$date2);
    //    $paraa['instructions'] = $inv_lineitems[0]->general_instructions;
    //    $paraa['plan'] = $inv_lineitems[0]->plan;
    //    $paraa['next_followup_date'] = $diffDate->format('%a');
    ...
    public function get_latestInvestigations($parameters, $method, $user_id){
        ...
        $check = $this->db->query("select * from patient_investigation where appointment_id='".$appointment_id."'")->row();
        // echo $this->db->last_query();
        // for this appointment if investigations not exists
        if(count($check) <= 0){
            // then find for previous appointment having investigations
            $app_id = $this->db->query("select appointment_id from appointments where patient_id='".$appInfo->patient_id."' and doctor_id='".$appInfo->doctor_id."' and clinic_id='".$appInfo->clinic_id."' order by appointment_id DESC LIMIT 1,1")->row();
            // echo $this->db->last_query();
            // if appointment found
            if(count($app_id) > 0){
                $invInfo = $this->db->query("select * from patient_investigation pin,patient_investigation_line_items pinvl where pin.patient_investigation_id=pinvl.patient_investigation_id and pin.appointment_id='".$app_id->appointment_id."' and pinvl.created_by='".$app_id->doctor_id."'")->result();
                if(count($invInfo) > 0){
                    // if investigations exits add to present appointment 
                    $data['patient_id'] = $appInfo->patient_id;
                    $data['clinic_id'] = $appInfo->clinic_id;
                    $data['doctor_id'] = $appInfo->doctor_id;
                    $data['appointment_id'] = $appInfo->appointment_id;
                    ...
                    foreach($invInfo as $value){
                        // Add investigations Line Items to present appointment
                        
                        $data2['patient_investigation_id'] = $patient_investigation_id;
                        $data2['investigation_id'] = $value->investigation_id;
                        $data2['investigation_name'] = $value->investigation_name;
                        $data2['checked'] = $value->checked;
                        $data2['status'] = 1;
                        $data2['created_by'] = $user_id;
                        $data2['created_date_time'] = date('Y-m-d H:i:s');
                        $data2['modified_by'] = $user_id;
                        $data2['modified_date_time'] = date('Y-m-d H:i:s');
                        $patient_investigation_line_item_id = $this->Generic_model->insertDataReturnId('patient_investigation_line_items', $data2);

                        $paraa['investigations_list'][$j]['patient_investigation_line_item_id'] = $patient_investigation_line_item_id;
                        $paraa['investigations_list'][$j]['patient_investigation_id'] = $patient_investigation_id;
                        $paraa['investigations_list'][$j]['checked'] = $value->checked;
                        $paraa['investigations_list'][$j]['investigation_id'] = $value->investigation_id;
                        $paraa['investigations_list'][$j]['investigation_name'] = $value->investigation_name;
                        
                        $patient_lab = $this->db->query("select i.report_status,pl.patient_lab_report_id from investigation_status i,patient_lab_reports pl where pl.patient_investigation_line_item_id=i.patient_investigation_line_item_id and pl.patient_investigation_line_item_id='".$value->patient_investigation_line_item_id."'")->row();
                        $paraa['query'] = $this->db->last_query();
                        $investigation_clinic_status = $this->db->query("select * from clinic_investigations where investigation_id='".$value->investigation_id."'")->row();
        ...
        // if investigations exists for present appointment
        else{
            $invInfo = $this->db->query("select * from patient_investigation pin,patient_investigation_line_items pinvl where pin.patient_investigation_id=pinvl.patient_investigation_id and pin.appointment_id='".$appointment_id."' and pinvl.created_by='".$appInfo->doctor_id."'")->result();
            if(count($invInfo) > 0){
                $j = 0;
                foreach($invInfo as $value){
                    $paraa['investigations_list'][$j]['patient_investigation_line_item_id'] = $value->patient_investigation_line_item_id;
                    $paraa['investigations_list'][$j]['patient_investigation_id'] = $value->patient_investigation_id;
                    $paraa['investigations_list'][$j]['checked'] = $value->checked;
                    $paraa['investigations_list'][$j]['investigation_id'] = $value->investigation_id;
                    $paraa['investigations_list'][$j]['investigation_name'] = $value->investigation_name;
    ...
    public function get_latestPrescription($parameters, $method, $user_id){
        ...
        $check = $this->db->query("select * from patient_prescription where appointment_id='".$appointment_id."'")->row();
        // echo $this->db->last_query();
        // for this appointment if drugs not exists
        
        $patient_allergy = $this->db->select("*")->from("patients ps")
        ->where("ps.patient_id='" . $appInfo->patient_id . "' ")
        ->get()
        ->row();
        $paraa['drug_allergy'] = $patient_allergy->allergy;
        ...
        if(count($check) <= 0){
            ...
            // if appointment found
            if(count($app_id) > 0){
                $drugInfo = $this->db->query("select * from patient_prescription pp,patient_prescription_drug ppd where pp.patient_prescription_id=ppd.patient_prescription_id and pp.appointment_id='".$app_id->appointment_id."' and ppd.created_by='".$app_id->doctor_id."'")->result();
                if(count($drugInfo) > 0){
                    // if drugs exits add to present appointment 
                    $data['patient_id'] = $appInfo->patient_id;
                    $data['clinic_id'] = $appInfo->clinic_id;
                    $data['doctor_id'] = $appInfo->doctor_id;
                    $data['general_instructions'] = $drugInfo[0]->general_instructions;
                    $data['plan'] = $drugInfo[0]->plan;
                    // $data['umr_no'] = $appInfo->umr_no;
                    $data['status'] = 1;
                    $data['created_by'] = $user_id;
                    $data['created_date_time'] = date('Y-m-d H:i:s');
                    ...
                    $patient_prescription_id = $this->Generic_model->insertDataReturnId('patient_prescription', $data);
                    // exit;

                    $pp = $this->Generic_model->getSingleRecord('patient_prescription', array('patient_prescription_id'=>$patient_prescription_id));

                    $j = 0;
                    
                    if($pp->followup_date == "0000-00-00"){
                        $followupdate = "";
                    }
                    else{
                        $followupdate = $pp->followup_date;
                    }
                    $date1 = date_create($appInfo->appointment_date);
                    $date2 = date_create($followupdate);
                    $diffDate = date_diff($date1,$date2);

                    $paraa['instructions'] = $pp->general_instructions;
                    $paraa['plan'] = $pp->plan;
                    $paraa['next_followup_date'] = $diffDate->format("%a");
                    foreach($drugInfo as $value){
                        // Add drugs Line Items to present appointment
                        
                        $data2['patient_prescription_id'] = $patient_prescription_id;
                        $data2['drug_id'] = $value->drug_id;
                        $data2['medicine_name'] = $value->medicine_name;
                        $data2['day_schedule'] = $value->day_schedule;
                        $data2['day_dosage'] = $value->day_dosage;
                        $data2['dosage_frequency'] = $value->dosage_frequency;
                        $data2['preffered_intake'] = $value->preffered_intake;
                        $data2['drug_dose'] = $value->drug_dose;
                        $data2['dosage_unit'] = $value->dosage_unit;
                        $data2['preffered_time_gap'] = $value->preffered_time_gap;
                        $data2['dose_course'] = $value->dose_course;
                        $data2['quantity'] = $value->quantity;
                        $data2['mode'] = $value->mode;
                        $data2['checked'] = $value->checked;
                        $data2['status'] = $value->status;
                        $data2['remarks'] = $value->remarks;
                        $data2['created_date_time'] = date('Y-m-d H:i:s');
                        $data2['modified_by'] = $user_id;
                        $data2['modified_date_time'] = date('Y-m-d H:i:s');
                        $patient_prescription_drug_id = $this->Generic_model->insertDataReturnId('patient_prescription_drug', $data2);
                             
                        $paraa['prescription'][$j]['patient_prescription_drug_id'] = $patient_prescription_drug_id;
                        $paraa['prescription'][$j]['patient_prescription_id'] = $patient_prescription_id;
                        $paraa['prescription'][$j]['day_schedule'] = $value->day_schedule;
                        $paraa['prescription'][$j]['day_dosage'] = $value->day_dosage;
                        $paraa['prescription'][$j]['dose_course'] = $value->dose_course;
                        $paraa['prescription'][$j]['mode'] = $value->mode;
                        $paraa['prescription'][$j]['drug_dose'] = $value->drug_dose;
                        $paraa['prescription'][$j]['dosage_frequency'] = $value->dosage_frequency;
                        $paraa['prescription'][$j]['dosage_unit'] = $value->dosage_unit;
                        $paraa['prescription'][$j]['drug_id'] = $value->drug_id;
                        $paraa['prescription'][$j]['medicine_name'] = $value->medicine_name;
                        $paraa['prescription'][$j]['preffered_intake'] = $value->preffered_intake;
                        $paraa['prescription'][$j]['quantity'] = $value->quantity;
                        $paraa['prescription'][$j]['remarks'] = $value->remarks;
        ...
        // if drugs exists for present appointment
        else{
            $drugInfo = $this->db->query("select * from patient_prescription pp,patient_prescription_drug ppd where pp.patient_prescription_id=ppd.patient_prescription_id and pp.appointment_id='".$appointment_id."' and ppd.created_by='".$appInfo->doctor_id."'")->result();
            if(count($drugInfo) > 0){
                $j = 0;
                
                if($drugInfo[0]->follow_up_date == "0000-00-00"){
                    $followupdate = "";
                }
                else{
                    $followupdate = $drugInfo[0]->follow_up_date;
                }
                $date1 = date_create($appInfo->appointment_date);
                $date2 = date_create($followupdate);
                $diffDate = date_diff($date1,$date2);

                $paraa['instructions'] = $drugInfo[0]->general_instructions;
                $paraa['plan'] = $drugInfo[0]->plan;
                $paraa['next_followup_date'] = $diffDate->format("%a");

                foreach($drugInfo as $value){
                             
                    $paraa['prescription'][$j]['patient_prescription_drug_id'] = $value->patient_prescription_drug_id;
                    $paraa['prescription'][$j]['patient_prescription_id'] = $value->patient_prescription_id;
                    $paraa['prescription'][$j]['day_schedule'] = $value->day_schedule;
                    $paraa['prescription'][$j]['day_dosage'] = $value->day_dosage;
                    $paraa['prescription'][$j]['dose_course'] = $value->dose_course;
                    $paraa['prescription'][$j]['mode'] = $value->mode;
    ...
    //                 $lineItems = $this->Generic_model->getAllRecords('patient_cd_line_');
    ...
    //                 $paraa['clinicaldiagnosis'][$j]['disease_name'] = $clist12->disease_name." ".$code;
    //                 $e++;
    //               $j++;
    ...
    //     // echo "<pre>";print_r($inv_lineitems);echo "</pre>";
   
    //        $j=0;
    //        foreach($inv_lineitems as $inv_lineitem){
    //         //    echo $inv_lineitem->code;
    //                 $code = "";
    //                 if($inv_lineitem->code == ""){
    //                     $code = "";
    //                 }
    //                 else{
    //                     $code = "[".$inv_lineitem->code."]";
    //                 }
    ...
    //                 // $paraa['clinicaldiagnosis'][$j]['appointment_id'] = $apt_id;
    //                 $paraa['clinicaldiagnosis'][$j]['disease_name'] = $inv_lineitem->disease_name." ".$code;
    ...
    public function CompleteSe($parameters, $method, $user_id)
    {
        ...
        else if( $form_type == "Other Systems" || $form_type == "GPE" || $form_type == "Past History"
...
public function getInfo($parameters, $method, $user_id)
{
    ...
    $data['parameters'][0]['appointment_date'] = $appointmentList->appointment_date;
 ...
 public function sessionUpdate($parameters, $method, $user_id)
 {
    ...
    if(count($slots) > 0)
    {
     ...
     foreach($slots as $inv_lineitem){
             ...
             $updateRes = $this->Generic_model->
             updateData('clinic_doctor_weekday_slots',
...
public function patient_shared_documents($parameters, $method, $user_id) {
    ...
    if (count($documents) > 0) {
        ...
        foreach ($documents as $pav) {
            ...
            // $param['patient_documents'][$pv]['file_type'] = $pav->file_type;
