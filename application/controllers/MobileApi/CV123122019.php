...
public function getTokenNumber($parameters, $method, $user_id){
    ...
    {
        $patientInfo = $this->db->select("*")->from("appointments")->where("status!='closed' and patient_id='".$patient_id."'")->get()->result();
        $i = 0;
...
public function health_education_articles($parameters, $method, $user_id) {
    ...
    foreach($doctor_appointment_list as $doctor_val){        
         ...
         $article_info = $this->db->query("SELECT * from articles INNER JOIN article_department ON articles.article_id=article_department.article_id where department_id='".$doctor_list->department_id."'")->result();
         if(count($article_info)>0){


            $j=0;
            foreach($article_info as $article){ 
                $data['health_education'][$j]['department_name'] = $doctor_list->department_name;
                $data['health_education'][$j]['title'] = $article->article_title;
                $data['health_education'][$j]['first_name'] = $doctor_list->first_name; 
                $data['health_education'][$j]['last_name'] = $doctor_list->last_name; 
                $data['health_education'][$j]['salutation'] = $doctor_list->salutation; 
                $data['health_education'][$j]['profile_image'] = $doctor_list->profile_image;
                $data['health_education'][$j]['work_place_location'] =$doctor_list->work_place_location; 
                ...
                $data['health_education'][$j]['image_url'] = $article->video_image;
                $data['health_education'][$j]['video'] = $article->video_url;
...
public function doctor_feedback_list($parameters, $method, $user_id){
    ...
    $feedbackList = $this->db->select('doctor_id, clinic_id, description, created_date_time')->from('patient_doctor_feedback')->where('doctor_id =',$doctor_id)->get()->result_array();
...
public function patientTransactions($parameters, $method, $user_id)
{
    ...
    */ 
    ...
    public function doctor_videos($parameters,$method,$user_id)
    {
        ...
        $videoInfo = $this->db->query("select * from articles where created_by = '".$doctor_id."' and status='1' order by created_date_time DESC")->result();
        ...
        foreach($videoInfo as $value)
        {
            ...
            $data['videos'][$i]['article_image'] = $value->article_image;
