...
class VideoSharing extends REST_Controller1
{
    ...
    public function index_post(){
        ...
        $config['allowed_types']='mp4|MP4|jpg|JPG|png|PNG|jpeg|JPEG|pdf|PDF'; 
       	...
       	if($fileCount>0)
       	{
            ...
            $dept = explode(",",substr(trim($departments,'"'),1,-1));
            ...
            $this->Generic_model->pushNotifications('', '', trim($user_id,'"'), '', 'H.E Uploaded', '');
    ...
    public function ArticlesSearch_post(){
        if(isset($_POST))
        {
            ...
            foreach ($articles as $value) 
            {
                ...
                $data['articles'][$i]['doctor_pic'] = base_url("uploads/doctors/".$value->profile_image);
                $data['articles'][$i]['posted_date_time'] = date("d-m-Y h:i A", strtotime($value->created_date_time));
                $data['articles'][$i]['description'] = $value->short_description;
                $data['articles'][$i]['type'] = $value->article_type;
                ...
                elseif(strtolower($value->article_type) == "image")
                {
                    ...
                    {
                        $data['articles'][$i]['article_image'][] = base_url('uploads/article_images/'.$value);
                    }                    
                }
    ...
    public function Articles_get($id){
        ...
        else
        {
            ...
            $articles = $this->db->query("select * from articles a,article_department ad where a.article_id=ad.article_id and a.article_status='published' and ad.doctor_visibility='1' and ad.department_id IN ".$dept_ids." order by a.article_id DESC")->result();
            foreach ($articles as $value) 
            {
                $doctor_list = $this->db->query("select * from doctors d,department de where d.department_id=de.department_id and d.doctor_id='".$value->posted_by."'")->row();
                if(count($doctor_list)<=0)
    ...
    public function Approve_post(){
        if(isset($_POST))
        {
            ...
            if(!empty($article_id))
            {
                ...
                $this->Generic_model->pushNotifications('', '', $user_id, '', 'H.E Approval', '');
    ...
    public function Reject_post(){
        if(isset($_POST))
        {
            ...
            if(!empty($article_id))
            {
                ...
                $this->Generic_model->pushNotifications('', '', $user_id, '', 'H.E Rejection', '');
