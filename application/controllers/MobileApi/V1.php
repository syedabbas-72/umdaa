...
require APPPATH . '/controllers/MobileApi/V1_1.php';
...
class V1 extends V1_1 {
        ...
        if ($this->post('requestpara') != NULL || $this->post('requestpara') != '') {
            ...
            //   $result = array('code' => '200', 'message' => 'Patient already', 'result' =>($params), 'requestname' => $requestname);
              // echo $mobile;
                ...
                // Get Patient Complete Details
                $patientRec = $this->db->select('patient_id,title,first_name,last_name,umr_no, date_of_birth, age,age_unit, email_id as email,payment_status,clinic_id,referred_by_type,referred_by,country,occupation,mobile,alternate_mobile,age,gender,location,state_id,district_id,preferred_language, address_line as address, pincode, photo, qrcode, created_date_time as registration_date')->from('patients')->where('patient_id ='.$patient_id)->get()->row(); 
                ...
                $config['allowed_types'] = 'jpg|JPG|png|PNG|csv|jpeg|PDF|pdf';
            ...
            //replacing old image with new image(vikram)
                ...
                $doctor_update['speciality_type'] = $expData['speciality'] = $fdata->speciality_type;
                $this->Generic_model->updateData("doctor_wallet_prices", $expData, array('doctor_id'=>$doctor_id));
                // echo $this->db->last_query();
    ...
    public function getPSSuggestions($parameters, $method, $user_id){        
        ...
        // $tokenResult = checkToken($user_id, $accessToken);
        // if($tokenResult != 1)
        // {
        //     $this->response(array('code'=>'403','message'=>'Unauthorized Access'));
        // }
        // else
        // {
            extract($parameters);
            $psInfo = $this->db->query("select ps.symptom_data,ps.form_id,count(*) as count from patient_presenting_symptoms pps,patient_ps_line_items ps where pps.patient_presenting_symptoms_id=ps.patient_presenting_symptoms_id and pps.clinic_id='".$clinic_id."' and pps.doctor_id='".$doctor_id."' group by ps.symptom_data order by count DESC LIMIT 15")->result();
            // echo $this->db->last_query();
            if(count($psInfo)>0)
            {
                $i = 0;
                foreach ($psInfo as $value) {
                    ...
                    $data1['symptoms'][$i]['form_id'] = $value->form_id;
                    $i++;
                }
                asort($data['symptoms']);
                $j = 0;
                foreach($data['symptoms'] as $key => $value){
                    // print_r($data1['cdSuggestions']);
                    $param['symptoms'][$j]['form_id'] = $data1['symptoms'][$key]['form_id'];
                    $param['symptoms'][$j]['symptom'] = $value['symptom'];
                    $j++;
                }
    ...
    public function getDrugSuggestions($parameters, $method, $user_id){
        ...
        // if($tokenResult != 1)
        // {
        //     $this->response(array('code'=>'403','message'=>'Unauthorized Access','result'=>$accessToken));
        // }
        // else
        // {
            extract($parameters);
            $psInfo = $this->db->query("select ppd.medicine_name,ppd.drug_id,count(*) as count from patient_prescription_drug ppd,patient_prescription pp where ppd.patient_prescription_id=pp.patient_prescription_id and pp.doctor_id='".$doctor_id."' and pp.clinic_id='".$clinic_id."' group by ppd.medicine_name order by count DESC LIMIT 15")->result();
            // echo $this->db->last_query();
            if(count($psInfo)>0)
            {
                $i = 0;
                foreach ($psInfo as $value) {
                    $data1['drugs'][$i]['drug_id'] = (int)$value->drug_id;
                ...
                foreach($data['drugs'] as $key => $value){
                    // print_r($data1['cdSuggestions']);
                    $param['drugs'][$j]['drug_id'] = $data1['drugs'][$key]['drug_id'];
                    $param['drugs'][$j]['drug_name'] = $value['drug_name'];
                    $j++;
                }
                $this->response(array('code' => '200', 'message' => 'success ', 'result' => $param, 'requestname' => $method)); 
            }
    ...
    public function doctor_registration($parameters, $method, $user_id){
        ...
        $docRec = $this->db->select('first_name,last_name,registration_code,doctor_id, mobile, email')->from('doctors')->where('mobile =',$mobile)->or_where('registration_code =',$registration_code)->or_where('email =',$email)->get()->row();
        ...
        if(count($docRec) == 0){
            ...
            if($speciality_type != ""){
                $expData['speciality'] = $speciality_type;
                $expData['doctor_id'] = $doctor_id;
                $expData['amount'] = 0;
                $expData['created_by'] = $doctor_id;
                $expData['created_date_time'] = date("Y-m-d H:i:s");
                $this->Generic_model->insertData('doctor_wallet_prices', $expData);
            }
            // if($mapCheckRes == 0){
    ...
    public function logout($parameters, $method, $user_id){
        ...
        if(!empty($user_id) && !empty($fcm_id) && !empty($device_id)){
            $check = $this->db->query("select * from users_device_info where user_id='".$user_id."' and fcm_id='".$fcm_id."' and device_id='".$device_id."'")->row();
            if(count($check) > 0){
                $this->Generic_model->deleteRecord('users_device_info', array('users_device_info_id'=>$check->users_device_info_id));
            ...
            $this->response(array('code'=>'201','message'=>'Error Occured','result'=>NULL,'requestname'=>$method));
        }
    }
...
public function login_details($parameters, $method, $user_id) {
    ...
    $result = $this->db->select("*")->from("users")->where("(email_id = '" . $email . "' or mobile ='" . $email . "' or username='".$email."' ) and password ='" . $password . "'")->get()->row();
    // echo $this->db->last_query();
    ...
    if ($result == TRUE) {
        ...
        $token = DataCrypt(generateAccessToken(), 'encrypt');    
        /* FCM ID Update */
        $fcmCheck = $this->db->query("select * from users_device_info where user_id='".$result->user_id."' and fcm_id='".$parameters['fcmId']."' and device_id='".$parameters['deviceid']."'")->row();
        if(count($fcmCheck) > 0){
            $fcmData['last_login_time'] = date('Y-m-d H:i:s');
            $fcmData['platform'] = "Android";
            $fcmData['secureAccessToken'] = $token;
            $this->Generic_model->updateData('users_device_info', $fcmData, array('users_device_info_id'=>$fcmCheck->users_device_info_id));
        }
        else{
            if($parameters['fcmId'] != "" && $parameters['deviceid'] != ""){
                $fcm_id['fcm_id'] = $parameters['fcmId'];
                $fcm_id['platform'] = "Android";
                $fcm_id['secureAccessToken'] = $token;
        ...
        if (count($users_details) > 0) {
            ...
            $param['accessToken'] = DataCrypt($token, 'decrypt');
            $docDetails = doctorDetails($users_details->user_id);
            ...
            $param['department_name'] = $docDetails->department_name;
            $param['name'] = $name;
            $param['gender'] = $gender;
            ...
            foreach ($case_sheet_list as $case_values) {
                ...
                ->order_by("a.position")->get()->result();
                $k = 0;
                // echo $this->db->last_query();
            ...
            if($user_type == 'doctor'){
                ...
                $param['doctor_details']->google_review_link = $doctor_details->google_review_link == '0'?'':$doctor_details->google_review_link;
                ...
                $dwp = $this->db->query("select * from doctor_wallet_prices where doctor_id='".$param['user_id']."'")->row();
                $param['doctor_details']->speciality_type = $dwp->speciality;
...
public function previous_documents($parameters, $method, $user_id) {
    ...
    if (count($previous_documents) > 0) {
        ...
        foreach ($previous_documents as $pav) {
            ...
            $param['previous_documents'][$pv]['file_type'] = $pav->file_type;
...
public function followup_template_list($parameters, $method, $user_id) {
    ...
    if (count($check_template) > 0 && $check_template->followup_id != NULL) {
        ...
        foreach ($template_info as $key => $value) {
            ...
            $data["follow_up_rows"][$k]['row_data'][$j]['title'] = date("Y-m-d", strtotime($value->appointment_date))." ".date("h:i A", strtotime($value->appointment_time_slot));;
     ...
     * ** */
      ...
      public function addFollowup_template($parameters, $method, $user_id) {
        ...
        foreach ($template_info as $key => $value) {
            $data["follow_up_rows"][$k]['row_data'][$j]['column_id'] = "$j";
            $data["follow_up_rows"][$k]['row_data'][$j]['appointment_id'] = $value->appointment_id;
            $data["follow_up_rows"][$k]['row_data'][$j]['title'] = date('d-M-Y',strtotime($value->appointment_date))." ".date('h:i A',strtotime($value->appointment_time_slot));
            $data["follow_up_rows"][$k]['row_data'][$j]['edit_permission'] = 0;
            $j++;
        }
...
public function appointments($parameters, $method, $user_id) {
    ...
    if (isset($parameters['mobile'])) {
        ...
        if (count($patients) > 0) {
            ...
            foreach ($patients as $patient) {
                ...
                $patient_val['appointment_list'][$i]['age'] = $patient->age;
                $patient_val['appointment_list'][$i]['age_unit'] = $patient->age_unit;
                ...
                $doctor_details = doctorDetails($patient->doctor_id);
                if($doctor_details->google_review_link == ""){
                    // echo $doctor_details->google_review_link;
                    $patient_val['appointment_list'][$i]['askReview'] = 0;    
                }
                else{
                    // echo "Naveen";
                    $patient_val['appointment_list'][$i]['askReview'] = 1;
                }
    ...
    */ 
        ...
        // get patient details with UMR no.
            ...
            //         $docPatientCheck = $this->Generic_model->getAllRecords('clinic_doctor_patient', array('clinic_id'=>$clinic_id,'doctor_id'=>$user_id,'patient_id'=>$patient['patient_id']));
            //         // echo $this->db->last_query();
            ...
            if(count($patientsRec) > 0){
                ...
                // Get the latest appointment id of this patient 
                    $this->db->select('A.appointment_id,A.appointment_type,A.doctor_id, CONCAT("Dr.",Doc.first_name, " ", Doc.last_name) as doctor_name, A.appointment_date,  A.appointment_time_slot as appointment_time, A.check_in_time, A.check_out_time, A.priority, A.consultation_start_time, A.consultation_end_time, A.course_in_hospital as doctor_comments, A.status, Doc.department_id, Doc.color_code, Dep.department_name as department');
                    $this->db->from('appointments A');
        ...
        if (count($doctorInfo) > 0) {
                ...
                if (count($clinics_list) > 0) {
                        ...
                        if($clinics_list->consulting_time=="")
                            $param['doctor']['clinics']['consultation_time'] = "15";
                        else
                        ...
                        $week_day_list = $this->db->select("clinic_doctor_weekday_id,weekday")->from("clinic_doctor_weekdays")->where("clinic_doctor_id ='" . $clinics_list->clinic_doctor_id . "'")->group_by("weekday")->order_by("clinic_doctor_weekday_id","asc")->get()->result();
    ...
    public function book_appointment($parameters, $method, $user_id) {
        ...
        */
        ...
        if(count($checkAppointment) > 0) {
            ...
            $clinicDoc_Info = $this->db->query("select * from clinic_doctor where clinic_id='".$clinic_id."' and doctor_id='".$doctor_id."'")->row();

            $clinicDocPatientInfo = $this->db->query("select * from clinic_doctor_patient where clinic_id='".$clinic_id."' and patient_id='".$patient_id."' and doctor_id='".$doctor_id."'")->row();
            ...
            $param['appointment']['slot_type'] = $slot_type;
            $param['appointment']['consultation_fee'] = $clinicDoc_Info->consulting_fee;
            $param['appointment']['tele_consultation_fee'] = $clinicDoc_Info->online_consulting_fee;

            $param['appointment']['reg_payment_status'] = (int)$reg_pay;
...
// print_r($parameters);
        ...
        if($bill_type == "Provisional"){
            ...
            $today = date("Y-m-d");
            if($info->appointment_date == $today){
                $data['status'] = "in_consultation";
            }
            else{
                $data['status'] = "waiting";
            ...
            for($i=0; $i<count($billing_line_items); $i++){
                ...
                $billing_line_items[$i]['item_information'] = $billing_line_items[$i]['item_information'];
                ...
                if($billing_line_items[$i]['item_information'] == "Registration"){
                    unset($billing_line_items[$i]['discount']);
                    unset($billing_line_items[$i]['discount_unit']);
                }

                if($billing_line_items[$i]['item_information'] == "Consultation"){
                    if($discount == 0){
    ...
    public function getCDSuggestions($parameters,$method,$user_id){
        ...
        else
        {
            ...
            $cdInfo = $this->db->query("select pcdl.disease_name,pcdl.clinical_diagnosis_id, count(*) as count from patient_clinical_diagnosis pcd,patient_cd_line_items pcdl where pcd.patient_clinical_diagnosis_id=pcdl.patient_clinical_diagnosis_id and pcd.doctor_id='".$doctor_id."' and pcd.clinic_id='".$clinic_id."' group by pcdl.disease_name order by count DESC LIMIT 15")->result();
            // echo $this->db->last_query();
            if(count($cdInfo)>0)
            {
                $i = 0;
                foreach($cdInfo as $value)
                {
                    ...
                    $data1['cdSuggestions'][$i]['clinical_diagnosis_id'] = $value->clinical_diagnosis_id;
                    $data['cdSuggestions'][$i]['disease_name'] = $value->disease_name;
                    $i++;
                }
                // array_multisort($data['cdSuggestions'], SORT_ASC);
                asort($data['cdSuggestions']);
                $j = 0;
                foreach($data['cdSuggestions'] as $key => $value){
                    // print_r($data1['cdSuggestions']);
                    $param['cdSuggestions'][$j]['clinical_diagnosis_id'] = $data1['cdSuggestions'][$key]['clinical_diagnosis_id'];
                    $param['cdSuggestions'][$j]['disease_name'] = $value['disease_name'];
                    $j++;
                }
            }
            else
    ...
    public function getInvSuggestions($parameters,$method,$user_id){
        ...
        else
        {
            ...
            if(count($invInfo)>0)
            {
                ...
                {
                    $data1['invSuggestions'][$i]['investigation_id'] = $value->investigation_id;
                ...
                $j = 0;
                foreach($data['invSuggestions'] as $key => $value){
                    $param['invSuggestions'][$j]['investigation_id'] = $data1['invSuggestions'][$key]['investigation_id'];
                    $param['invSuggestions'][$j]['investigation_name'] = $value['investigation_name'];
                    $j++;
                }
...
public function prescription_submit($parameters, $method, $user_id) {
    ...
    if($type == "del"){
        ...
        $this->response(array('code' => '200', 'message' => 'Prescription Saved Successfully', 'result' => $parameters, 'requestname' => $method));
    ...
    if($parameters['days'] != "" || $parameters['days'] != NULL){
        ...
        if($parameters['followup_appointment_id'] != '' || $parameters['followup_appointment_id'] != NULL){
            ...
            $this->Generic_model->updateData('appointments',$followup_appointment,array('appointment_id'=>$parameters['followup_appointment_id']));
                ...
                $parentCheck = $this->Generic_model->getSingleRecord('appointments', array('parent_appointment_id'=>$parameters['appointment_id']));
                $appInfo = $this->Generic_model->getSingleRecord('appointments', array('appointment_id'=>$parameters['appointment_id']));
                if(count($parentCheck) < 1){
                    $appointment['parent_appointment_id'] = $parameters['appointment_id'];
                    $appointment['clinic_id'] = $parameters['clinic_id'];
                    $appointment['patient_id'] = $parameters['patient_id'];
                    $appointment['umr_no'] = $appInfo->umr_no;
                    $appointment['doctor_id'] = $parameters['doctor_id'];
                    $appointment['appointment_type'] = "Follow-up";
                    ...
                    $parameters['followup_appointment_id'] = $parentCheck->appointment_id;
                }
        }          
...
public function patient_vitals_info($parameters, $method, $user_id) {
    ...
    if (count($patient_info) > 0) {
        ...
        // $patient_vitals = $this->db->query("select * from appointments a, patient_vital_sign pv where a.patient_id=pv.patient_id and a.doctor_id='".$user_id."' and pv.patient_id='".$patient_id."' group by pv.vital_sign_recording_date_time")->result();
        // echo $this->db->last_query();

        if (count($patient_vitals) > 0) {
            for ($i = 0; $i < count($patient_vitals); $i++) {
                $data['vitals'][$i]['appointment_id'] = $patient_vitals[$i]->appointment_id;
                $appointment_info = $this->db->select('*')->from('appointments')->where('appointment_id ="' . $patient_vitals[$i]->appointment_id.'" and doctor_id="'.$user_id.'"')->get()->row();
                // echo $this->db->last_query();
...
//inventory outwards insert
    ...
    public function patient_summary_list($parameters, $method, $user_id) {
        ...
        ->where("a.patient_id='" . $patient_id . "' and a.doctor_id='".$doctor_id."' and a.clinic_id='".$clinic_id."' and a.appointment_date <= '".date('Y-m-d')."'")
    ...
    public function payChecksum($parameters, $method, $user_id){
        extract($parameters);

        // $paytmParams['list'] = array();

        $paytmParams['list']["MID"] = "sOynRz56677151483389";
        $paytmParams['list']["ORDER_ID"] = $ORDER_ID;
        $paytmParams['list']["CHANNEL_ID"] = $CHANNEL_ID;
        $paytmParams['list']["CUST_ID"] = $CUST_ID;
        $paytmParams['list']["INDUSTRY_TYPE_ID"] = $INDUSTRY_TYPE_ID;
        $paytmParams['list']["WEBSITE"] = $WEBSITE;
        $paytmParams['list']["TXN_AMOUNT"] = $TXN_AMOUNT;
        $paytmParams['list']["CALLBACK_URL"] = $CALLBACK_URL;

        $paytmChecksum = getChecksumFromArray($paytmParams['list'], "6tQiV1MAx4LeizE&");
        // $paytmChecksum = PaytmChecksum::generateSignature($paytmParams, '6tQiV1MAx4LeizE&');
        
        $para['checksum'] = $paytmChecksum;
...
public function patient_procedure($parameters, $method, $user_id) {
    ...
    $checking_patient_procedure = $this->db->select("*")->from("patient_procedure")->where("patient_procedure_id='".$parameters['patient_procedure_id']."'")->get()->row();
    // echo $this->db->last_query();
    if (count($checking_patient_procedure) > 0) {
        unset($data['medical_procedure']);

        $ok = $this->Generic_model->updateData("patient_procedure", $data, array('patient_procedure_id' => $parameters['patient_procedure_id']));
        // echo $this->db->last_query();
...
public function patient_procedure_edit($parameters, $method, $user_id) {
    ...
    // Here medical_procedure_id means not medical_procedure_id it is patient_procedure_id
    $check = $this->db->query("select * from patient_procedure where patient_procedure_id='".$medical_procedure_id."'")->row(); 

    if(count($check) > 0){
        if($check->medical_procedure == ""){
            $check_doctor_procedure = $this->db->select("*")->from("doctor_medical_procedures")->where("doctor_id ='" . $doctor_id . "' and medical_procedure_id ='" . $check->medical_procedure_id . "' and clinic_id ='" . $clinic_id . "'")->get()->row();
            ...
            $data['procedure_description'] = $check->medical_procedure;
        }
        $url_parameters = $patient_id . "/" . $doctor_id . "/" . $appointment_id . "/" . $check->medical_procedure_id . "/" . $clinic_id;
        $data["procedure_url"] = base_url("Procedure_update/patient_producer/" . $url_parameters);

        $data['surgeon'] = $check->surgeon;
        $data['anesthetist'] = $check->anesthetist;
        $data['assisting_surgeon'] = $check->assisting_surgeon;
        $data['type_of_anesthesia'] = $check->type_of_anesthesia;
        $data['assisting_nurse'] = $check->assisting_nurse;
        $data['preoperative_diagnosis'] = $check->preoperative_diagnosis;
        $data['postoperative_diagnosis'] = $check->postoperative_diagnosis;
        $data['indication'] = $check->indication;
        $data['position'] = $check->position;
        $data['patient_procedure_id'] = $check->patient_procedure_id;
        $data['clinic_id'] = $check->clinic_id;
        $data['patient_id'] = $check->patient_id;
        $data['appointment_id'] = $check->appointment_id;
        $data['doctor_id'] = $check->doctor_id;
        $data['medical_procedure_id'] = $check->medical_procedure_id;
...
$this->response(array('code' => '200', 'message' => 'Vitals Print Successful', 'result' => $result, 'requestname' => $method));
    ...
    ****/
    ...
    public function getSavedRecursiveForm($parameters, $method, $user_id)
    {
        ...
        $get_form = $this->db->select('GROUP_CONCAT(form_id) as form_ids')->from('patient_form')->where('form_type="'.$form_type.'" and appointment_id="'.$parameters['app_id'].'" and patient_id="'.$patient_id.'" and doctor_id="'.$doctor_id.'"')->get()->row();
...
if($form_type=='Systemic Examination')
{
    ...
    if(count($form_names)>0){
        ...
        $systemic_examination = $this->db->select("form_id,form_name,form_type,department_id")->from("form")->where("form_type='" . $form_type . "' and department_id='".$departcondition."'")->get()->result();
        if(count($systemic_examination)>0){
            $i=0;
            foreach($systemic_examination as $value){
                $data['para'][$i]['form_id'] = $value->form_id;
...
public function immunization_list($parameters, $method, $user_id){
    ...
    // Latest CD
    // public get_latestCD($parameters, $method, $user_id){
    //     extract($parameters);
    //     // $check = $this->db->query("select * from patient_clinical")->row();
    // }
...
public function checkToken($parameters, $method, $user_id){
    extract($parameters);
    $result = checkToken($user_id, $accessToken);
}
