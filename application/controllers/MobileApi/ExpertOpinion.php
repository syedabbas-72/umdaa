...
class ExpertOpinion extends REST_Controller1
{
    ...
    public function SavePayment_post(){
        ...
        if(count($check)>0)
        {
            ...
            // $this->Generic_model->pushNotifications('', '', $doctor_id, '', 'EO_Wallet_Money_Added', '');
        ...
        else
        {
            ...
            // Transaction History
            $data1['transaction_id'] = "UMDEXP".$doctor_id.$walletHistory->max+1;
            $data1['transaction_amount'] = $amount;
            $data1['transaction_type'] = "Credit";
            $data1['doctor_id'] = $doctor_id;
            $data1['created_by'] = $doctor_id;
    ...
    public function walletReq_post()
    {
        ...
        else
        {
            ...
            $this->Generic_model->pushNotifications('',  '', $doctor_id, '', 'Wallet Request', '');
        }
    }
    ...
    public function AcceptRequest_get($expID)
    {
        if(isset($_GET))
        {
            ...
            // $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Accept', '');
            if($expCheck>0)
            {
                if($expInfo->status == 1)
                {
                ...
                else
                {
                    ...
                    // echo $this->db->last_query();
                    // $transaction_amount = $doctorWalletInfo->amount;
        ...
        if(isset($_GET))
        {
            ...
            if($expCheck>0)
            {
                ...
                else
                {
                    ...
                    $this->Generic_model->updateData("expert_opinion",$data,array('expert_opinion_id'=>$expID));
                    $param['result'] = "Expert Opinion Closed";
                    $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Close', '');
                    $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
                }    
            }
            else
            ...
            if($expCheck > 0){
                ...
                if(count($parentCheck) < 1){
                    ...
                    if($diffDays >= 7){
                        ...
                        $param = "Request Sent Successfully";    
                        $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->referred_doctor_id, '', 'EO_Sent_Notification', '');
                        $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Received_Notification', '');
                        $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
                    }
        ...
        if(isset($_GET))
        {
            ...
            if($expCheck>0)
            {
                ...
                else
                {
                    ...
                    $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_FI', '');
                    $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
                }    
            }
            else
        ...
        if(isset($_GET))
        {
            ...
            if($expCheck>0)
            {
                ...
                else
                {
                    ...
                    // // $this->Generic_model->pushNotifications();
                    ...
                    $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Cancel', '');
                    $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
                }    
            }
            else
        ...
        if(isset($_GET))
        {
            ...
            if($expCheck>0)
            {
                ...
                else
                {
                    ...
                    $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
                    $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Reject', '');
                }    
            }
            else
            {
...
public function prescriptions_post() 
{
    ...
    public function walletHistory_get($docId)
    {
        if(isset($_GET))
        {
            $walletHistory = $this->db->query("select *,ew.payment_status as pstatus from eo_wallet_history ew,expert_opinion eo where eo.expert_opinion_id=ew.expert_opinion_id and ew.doctor_id='".$docId."'")->result();
            $i = 0;
            ...
            $walletInfo = $this->db->query("select * from eo_wallet_history where doctor_id='".$docId."' and expert_opinion_id=0")->result();
    ...
    public function sendMessage_post()                                                                   
    {
        if(isset($_POST))
        {
            ...
            $this->response(array('code'=>'200','message'=>'Success','result'=>$param));
            $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->parent_doctor_id, '', 'EO_Comment', '');
            $this->Generic_model->pushNotifications('', $expInfo->appointment_id, $expInfo->referred_doctor_id, '', 'EO_Message_Sent', '');
        }
        else
        {
            $param = "UnAuthorized Access";
