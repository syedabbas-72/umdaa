...
class Registration extends REST_Controller1
{
    ...
    public function getDoctorSlots_get($doctor_id,$clinic_id)
    {
        ...
        public function doctorEditSlots_post(){
            if(isset($_POST))
            {
                ...
                $con['consulting_fee'] = trim($consultation_fee, '"');
                $this->Generic_model->updateData('clinic_doctor',$con,array('clinic_doctor_id'=>$clinincDocInfo->clinic_doctor_id));
                $daysar = ['1','2','3','4','5','6','7'];
                for($x = 0; $x <=count($arr); $x++) {
                    $checked[] = $arr[$x]['day']; 
                    // if(!in_array($arr[$x]['day'], $daysar)){
                    //     ;
                    // }
                    if($arr[$x]['day'] == "1")
                    {
                  ...
                  $clinicDoctorInfo = $this->Generic_model->getSingleRecord('clinic_doctor', array('clinic_id'=>$clinic_id,'doctor_id'=>$doctor_id));
                  for($i = 1;$i <= 7;$i++){
                      if(in_array($i, $checked)){
                          continue;
                      }
                      else{
                          $cdw_info = $this->Generic_model->getSingleRecord('clinic_doctor_weekdays', array('clinic_doctor_id'=>$clinicDoctorInfo->clinic_doctor_id,'weekday'=>$i,'slot'=>'walkin'));
                          $cdws = $this->Generic_model->getAllRecords('clinic_doctor_weekday_slots', array('clinic_doctor_weekday_id'=>$cdw_info->clinic_doctor_weekday_id));
                          foreach($cdws as $value){
                              $this->Generic_model->deleteRecord('clinic_doctor_weekday_slots', array('clinic_doctor_weekday_slot_id'=>$value->clinic_doctor_weekday_slot_id));
                          }
                          $this->Generic_model->deleteRecord('clinic_doctor_weekdays', array('clinic_doctor_weekday_id'=>$value->clinic_doctor_weekday_id));
    ...
    public function videocallDoctorSlots_post(){
       ...
       public function doctorEditVideoSlots_post(){
           if(isset($_POST)){
               extract($_POST);
               $clinicDocInfo = $this->Generic_model->getSingleRecord('clinic_doctor', array('clinic_id'=>$clinic_id,'doctor_id'=>$doctor_id));
               if(count($clinicDocInfo) > 0){
                $con['online_consulting_fee'] = trim($consultation_fee, '"');
                $this->Generic_model->updateData('clinic_doctor', $con, array('clinic_doctor_id'=>$clinicDocInfo->clinic_doctor_id));

                $weekdaysInfo = $this->db->query("SELECT * from clinic_doctor_weekdays cwd, clinic_doctor_weekday_slots cwds where cwd.clinic_doctor_weekday_id=cwds.clinic_doctor_weekday_id and cwd.clinic_doctor_id='".$clinicDocInfo->clinic_doctor_id."' and cwd.slot='video call'")->result();
                if(count($weekdaysInfo) > 0){
                    foreach($weekdaysInfo as $value){
                        $this->Generic_model->deleteRecord('clinic_doctor_weekday_slots', array('clinic_doctor_weekday_slot_id'=>$value->clinic_doctor_weekday_slot_id));
                        $this->Generic_model->deleteRecord('clinic_doctor_weekdays', array('clinic_doctor_weekday_id'=>$value->clinic_doctor_weekday_id));
                    }
                }
                ...
                if(count($arr) > 0){
                    foreach($arr as $value){
                        $data['weekday'] = $value['day'];
                        $data['clinic_doctor_id'] = $clinicDocInfo->clinic_doctor_id;
                        $data['status'] = 1;
                        $data['slot'] = "video call";
                        $data['created_by'] = $clinicDocInfo->clinic_doctor_id;
                        $data['created_date_time'] = date('Y-m-d H:i:s');
                        $data['modified_by'] = $clinicDocInfo->clinic_doctor_id;
                        $data['modified_date_time'] = date('Y-m-d H:i:s');
                        $cdw_id = $this->Generic_model->insertDataReturnId('clinic_doctor_weekdays', $data);
                        if(count($value['slots']) > 0){
                            foreach($value['slots'] as $val){
                                    $data1['clinic_doctor_weekday_id'] = $cdw_id;
                                    $data1['from_time'] = $val['from_time'];
                                    $data1['to_time'] = $val['to_time'];
                                    $data1['session'] = $val['session'];
                                    $data1['status'] = 1;
                                    $data1['created_by'] = $clinicDocInfo->clinic_doctor_id;
                                    $data1['created_date_time'] = date('Y-m-d H:i:s');
                                    $data1['modified_by'] = $clinicDocInfo->clinic_doctor_id;
                                    $data1['modified_date_time'] = date('Y-m-d H:i:s');
                                    $this->Generic_model->insertData('clinic_doctor_weekday_slots', $data1);
                                    unset($data1);
                            }
        ...
        //         // echo "inndss";
        //         $arr = json_decode($weekday,true);
        //         $clinincDocInfo = $this->db->query("select * from clinic_doctor where clinic_id='".$clinic_id."' and doctor_id='".$doctor_id."'")->row();
        //         $con['online_consulting_fee'] = trim($consultation_fee, '"');
        //         $this->Generic_model->updateData('clinic_doctor',$con,array('clinic_doctor_id'=>$clinincDocInfo->clinic_doctor_id));
        ...
        //                     {
        //                         // echo $arr[$x]['slots'][$a]['from_time'];
        //                         $getData = $this->db->query("select * from clinic_doctor_weekday_slots where clinic_doctor_weekday_slot_id='".$arr[$x]['slots'][$a]['clinic_doctor_weekday_slot_id']."'")->row();
        //                         if(count($getData)>0)
        //                         {
        //                             $data1['clinic_doctor_weekday_id'] = $getData->clinic_doctor_weekday_id;
        //                             $data1['from_time'] = $arr[$x]['slots'][$a]['from_time'];
        ...
        //                             $data1['created_date_time'] = date("Y-m-d H:i:s");
        //                             $data1['modified_date_time'] = date("Y-m-d H:i:s");
        //                             $this->Generic_model->updateData("clinic_doctor_weekday_slots",$data1,array('clinic_doctor_weekday_slot_id' => $getData->clinic_doctor_weekday_slot_id));
        //                         }
        //                         $this->response(array('code'=>'200','message'=>'Sucess','result'=>'Successfully Updated'));
        //                     }
        //                     else
