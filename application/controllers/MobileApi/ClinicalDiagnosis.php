...
class ClinicalDiagnosis extends REST_Controller1
{
    ...
    public function index_get($id = ' ')
    {
        if(!empty(isset($_GET)))
        {
            ...
            if($id != " ")
            {
                $cdInfo = $this->db->query("select clinical_diagnosis_id,disease_name,code from clinical_diagnosis where disease_name LIKE '%".$this->db->escape(urldecode($id))."%' limit 20")->result();
    ...
    public function androidclinicalDiagnosis_get($id = ' ')
    {
        if(!empty(isset($_GET)))
        {
            ...
            if($id != " ")
            {
                // echo 'select clinical_diagnosis_id,disease_name,code from clinical_diagnosis where disease_name LIKE %'.urldecode($id).'% limit 20';
                // echo 'select clinical_diagnosis_id,disease_name,code from clinical_diagnosis where disease_name LIKE (%'.urldecode($id).'%) limit 20';
